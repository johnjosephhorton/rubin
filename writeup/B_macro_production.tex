...
This formulation is essentially equivalent to the original definition of wage in Equation~\eqref{eq:wage}.
If we multiply each skill cost in Equation~\eqref{eq:new_wage} by its respective base wage rate and redefine the task's skill cost as the resulting product, we arrive at the original formulation given by Equation~\eqref{eq:wage}.
The formulation in Equation~\eqref{eq:new_wage} allows tasks to contribute differently to the job's wage depending on their mode of execution and type of labor that need to perform them.}
...
Suppose the firm solves the long-run cost minimization problem in Equation~\eqref{eq:totalcost_with_handoff} for a given AI quality level $\alpha$, and obtains the optimal AI strategy \( \T \) and job design \( \J \).
...
The condition expressed in Equation~\eqref{eq:task_subst_rate_indep} satisfies the aggregation requirement above.\footnote{In fact, this condition is stricter than necessary. It not only guarantees labor independence across aggregated input types, but also imposes a stronger condition: that the rate of substitution between tasks within the same aggregate input type is independent even of other tasks within the same aggregate type.}
...
\end{equation}
The representation in Equation~\eqref{eq:micro_agg_prod} helps us gain clearer insights into the allocation between manual and AI management labor at the firm level by removing the need to deal with the complex, many-input production function in Equation~\eqref{eq:task_level_prod} at the level of individual tasks.
...
In the first stage, firms solve the long-run cost minimization problem in Equation~\eqref{eq:totalcost_with_handoff}, committing to an AI deployment strategy $\T$ and a job design $\J$ based on their \emph{expected} AI quality levels.
...
In what follows, we demonstrate that the heterogeneity in effective AI quality level can be structured so that micro-level Leontief production functions in Equation~\eqref{eq:micro_agg_prod} aggregate into a macro-level CES production function, in which the CES inputs are aggregates of the micro-level inputs \citep{levhari1968note}.
...
\]
Substituting for $\labor{\manualLetter}$ and $y$ from Equation~\eqref{eq:micro_fixed_proportions} into the profitability condition yields:
\[
...
Moreover, observe from Equation~\eqref{eq:alpha_bar} that for $d_b \geq 0$ the upper bound of $\bar{\alpha}$ is 1 and is achieved when $\alpha \rightarrow 1^{-}$.
Therefore:
\begin{equation}
...
From Equation~\eqref{eq:micro_fixed_proportions}, it directly follows that the AI management labor and manual labor used by this firm are: $\labor{\AIletter} = (\skillAdjustedTimeLetter_{\AIletter}/\bar{\alpha})\,\phi(\bar{\alpha}),$ and $\labor{\manualLetter} = \skillAdjustedTimeLetter_{\manualLetter}\,\phi(\bar{\alpha}).$
...
Substituting the aggregated variables from the micro-level Equations~\eqref{eq:micro_Y}, \eqref{eq:micro_M}, and \eqref{eq:micro_H} into the aggregate production function in Equation~\eqref{eq:macro_agg_prod}, we obtain:
\begin{equation}
...
The derived distribution in Equation~\eqref{eq:phi} characterizes firm-level heterogeneity in the effective AI quality level required to support a macro CES production function of the form in Equation~\eqref{eq:macro_agg_prod}, with economy-wide aggregate AI management and manual labor inputs and parameters $\theta_{\AIletter}, \theta_{\manualLetter}, \rho$, obtained from micro-level Leontief production functions of the form in Equation~\eqref{eq:micro_agg_prod}.
...
In this subsection we describe how to obtain the analytical formula for distribution of heterogeneity $\phi(\bar{\alpha})$ expressed in Equation~\eqref{eq:phi} by extending the method of \cite{levhari1968note}.
...
\end{equation}
which is the formula provided in Equation~\eqref{eq:phi} above.
