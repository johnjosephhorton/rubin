...
\]
Substituting for $\labor{\manualLetter}$ and $y$ from (\ref{eq:micro_fixed_proportions}) into the profitability condition yields:
\[
...
Substituting the aggregated variables from the micro-level equations (\ref{eq:micro_Y}, \ref{eq:micro_M}, \ref{eq:micro_H}) into the aggregate production function (\ref{eq:macro_agg_prod}), we obtain:
\begin{equation}
