...
\end{align}
over economy-wide AI management labor $\aggregateAIlabor$, manual labor $\aggregateManualLabor$, and capital $K$, with $Y$ denoting aggregate output and $\rho < 0$, so that the elasticity of substitution $\sigma = 1/(1-\rho)$ lies below one and the three aggregate inputs are complements.
...
Because that absorption is not invertible, aggregate data cannot reveal workflow structure, so objects such as the fragmentation index must be measured where the arrangement of work is still visible, which is what our empirical exercise in Section~\ref{sec:empirics} does.
...
We therefore round each of the two costs up to a power of $(1+\epsilon')$, for an $\epsilon'$ smaller than $\epsilon$ by a factor of order $m$, which leaves finitely many of them to keep track of.
Because the recursion is indexed by the rounded totals, each step of a job rounds again on top of what earlier steps have already rounded, so the distortions compound along the job rather than being incurred once; shrinking $\epsilon'$ by that factor is what keeps the resulting wage bill within $(1+\epsilon)$ of the true one.
