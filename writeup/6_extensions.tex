...
Let $R(i,\, \skillcost{},\, \timecost{})$ be the minimum cost of completing steps $1$ through $i$ when the worker currently accumulating tasks already holds total skill $\skillcost{}$ and total time $\timecost{}$, the latter including their hand-off, and let $V(i)$ be the corresponding cost when that worker's job closes at step $i$, which is the same recursion evaluated at the job-opening state $(i,\, 0,\, \handofftime{i})$ with the closure option removed, since by Definition~\ref{def:job_design} a job must hold at least one task.
...
\end{align}
with the optimum at $V(m)$, recalling that the final job hands off at no cost.
