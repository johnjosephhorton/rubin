...
The aggregate representation asks more of the economy than the firm-level argument does: capital productivity is common across firms, every firm organizes identically, and realized effective AI quality is the single dimension along which firms differ.
}
...
Two economies with identical production steps, arranged differently within workflows, aggregate to the same CES form in Equation~\eqref{eq:lr_macro_ces} but with different production parameters and input requirements.
...
With the job and its wage fixed, the firm minimizes total expected time in Equation~\eqref{eq:totalcost_shortterm}.
...
\end{align}
Because time costs enter Equation~\eqref{eq:totalcost_shortterm} additively, a stretch's time cost is all that Equation~\eqref{eq:shortrun_recursion} needs from it.
...
Given a workflow with $m$ steps, the AI strategy minimizing Equation~\eqref{eq:totalcost_shortterm} can be calculated in time $O(m^2)$ via dynamic programming.
\end{proposition}

\paragraph{The Long Run Optimization.}
The firm now chooses job boundaries as well, and Equation~\eqref{eq:totalcost_with_handoff} multiplies each job's total skill by its total time.
...
For any $\epsilon > 0$, a pair of AI strategy $\T$ and job design $\J$ whose long-run cost in Equation~\eqref{eq:totalcost_with_handoff} is within a factor $(1+\epsilon)$ of the minimum can be computed in time $O(m^2 \epsilon^{-2} \log^2(mB))$ via dynamic programming.
\end{proposition}
