...
\begin{subfigure}[b]{\textwidth}
    ...
    \caption{Panel (A): Full Workflow (All Tasks)}
...
\begin{subfigure}[b]{\textwidth}
    \centering
    \captionsetup{labelformat=empty}
    \caption{Panel (B): Pruned Workflow (Hourly$+$ $\geq 50\%$)}
    \includegraphics[width=0.86\textwidth]{plots/taskSequence_vs_anthropicIndex/example_pruning/example_allergists_hourly50.png}
\end{subfigure}
\end{center}
\footnotesize{
\emph{Notes:} Panel~(A) reports the full sequenced workflow for Allergists and Immunologists (SOC 29-1229.01), the object used in the main analysis: the occupation's sixteen O*NET tasks in the order assigned by the language model.
