...
The idea behind the optimization is to track both the worker's accumulated skill and accumulated time as the firm decides on how to execute each step one at a time in a dynamic program. 
Because the wage bill is the product of the two totals, it is evaluated only once every step has been assigned.
We then discretize each cost dimension to a geometric grid so that the dynamic program runs in polynomial time subject to an error term than can be arbitrarily small.
...
We introduce an additional ``hand-off'' time cost, to capture the coordination friction when worker pass intermediate outputs between them.
...
The cost minimization of the firm's long-run problem has a similar flavor to the medium-run case, but rather than fixing the steps assigned to the worker it also chooses how tasks are allocated across different workers.
The full characterization of the problem and polynomial-time approximation of its solution are given in the job design appendix (Appendix~\ref{app:jobdesign}).
