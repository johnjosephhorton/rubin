...
Labor is the only input used in production.
...
In the medium run, the boundary of the job remains fixed, but competitive pressure on the worker side and free entry into jobs let the firm do two things at once: optimize the job's time cost \emph{and} adjust the wage to the skill the job actually requires once AI potentially lowers the cost of executing its tasks.
Deploying AI can lower costs on both margins, since an augmented or automated step may demand both less skill and less time than manual execution.
The firm therefore minimizes its total wage bill, and since fixed job boundaries make jobs' costs independent of one another, the problem reduces to minimizing the wage bill of each job separately, as we do below.
Because the worker is paid in proportion to the job's skill for the entire time the job takes, the wage bill of a job is the product of its total skill and its total time.
\begin{align}
\label{eq:totalcost_mediumrun}
\min_{\T} \ \Bigl(\sum_{T_b \in \T} \skillcost{b}\Bigr)\Bigl(\sum_{T_b \in \T} \timecost{b}\Bigr).
\end{align}

In \eqref{eq:totalcost_mediumrun} the firm minimizes the wage bill of the job.
Because this objective is a product of two sums rather than a single sum, it is not additive across tasks, and the short-run dynamic program no longer applies.
...
The solution still relies on dynamic programming: the firm decides how to execute the steps one at a time, and the program tracks the worker's accumulated skill and accumulated time along the way, evaluating the wage bill, the product of the two totals, only once every step has been assigned.
...
We therefore discretize both cost dimensions to a geometric grid, which keeps the running time polynomial while approximating the optimum subject to an error term that can be made arbitrarily small.
...
Narrowing a job lets the firm staff it with workers specializing in fewer tasks, and thus commanding lower wages, but splitting the workflow across more workers introduces coordination costs between them.
...
The firm chooses AI deployment and job design jointly, trading the lower wages of narrower jobs against the hand-off costs of maintaining more of them.
