...
Steps can also be executed in an \emph{automated} mode in which AI performs the step end-to-end with no direct human involvement, and hence at zero direct human cost.
...
The AI chain as a whole can be thought as a single ``composite'' task delegated by the human to AI for execution.
...
An AI chain may consist of a single step, with $\ell = r$: having no automated predecessors, it is simply an augmented step.
...
We define an \emph{AI deployment strategy} (or \emph{AI strategy} for short) as a partition of the step sequence $\mathcal{S}$ into a sequence of contiguous tasks $\mathcal{T}$, each of which is either a manual step or an AI chain.
...
Executing a task requires a worker who possesses the skills it demands, for as long as the task takes to complete.
A worker's wage is proportional to the skills they hold, and the firm pays that wage for the entire time the worker is at work.
What the firm can adjust to lower its labor cost depends on the horizon, and we distinguish three.

In the \emph{short run}, the firm has already hired its workers and fixed both the boundaries of their jobs and the skills they hold.
These skill costs are sunk, since each worker is paid for the skills they hold regardless of how AI is deployed, so the only margin AI deployment can move is the time workers spend.
...
\end{align}
where $\timecost{b}$ is the (expected) time cost of task $T_b$ from Section~\ref{sec:model.tasks} and Table~\ref{tab:model_parameters}.
AI is deployed on a step, or on a run of steps as a chain, whenever doing so lowers the expected time of production.
This short-run problem is the one we solve in the remainder of the body.

In the \emph{medium run}, job boundaries remain fixed, but the firm can also adjust the skills it hires: it can staff a job with a worker trained to exactly the skills its tasks require, performing the same set of tasks at a potentially lower wage.
Deploying AI can then lower cost on two margins at once, since an augmented or automated step may demand less skill, and less time, than manual execution.
Because the worker is paid in proportion to their skill for the whole time they spend on the job, the firm minimizes the product of the job's total skill and its total time,
\begin{align}
\label{eq:totalcost_mediumrun}
\min_{\T} \ \Bigl(\sum_{T_b \in \T} \skillcost{b}\Bigr)\Bigl(\sum_{T_b \in \T} \timecost{b}\Bigr),
\end{align}
the product of the sum of skill costs and the sum of (expected) time costs across the job's tasks.

In the \emph{long run}, the firm additionally chooses the job boundaries themselves, partitioning the workflow into jobs.
Designing a job around fewer tasks lets the firm hire a less skilled, and so lower-paid, worker for it, but splitting the work across more workers introduces coordination (hand-off) costs between them.
The firm trades these two forces off, choosing the AI deployment and the job design jointly.
We develop this long-run problem, which nests the medium-run problem as the special case of fixed job boundaries, in Appendix~\ref{app:jobdesign}.
