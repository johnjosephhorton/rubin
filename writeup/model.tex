...
Since job boundaries are fixed, it suffices to optimize AI deployment for each job separately, so it is without loss to assume in \eqref{eq:totalcost_shortterm} that there is a single job consisting of the entire $m$-step sequence $\mathcal{S}$.
...
Next we turn to solving the medium-run problem.
In \eqref{eq:totalcost_mediumrun} the firm minimizes the wage bill of the job, the product of the job's total skill and its total time.
...
The idea is to track both the worker's accumulated skill and accumulated time as the firm decides on how to execute each step one at a time, discretizing each to a geometric grid so that the dynamic program runs in polynomial time; because the wage bill is the product of the two totals, it is evaluated only once every step has been assigned.


\subsubsection{Long Run: Adjustable Job Boundaries, Wages, and Execution Times}
\label{sec:model.longrun}
In the long run, the firm additionally redraws the boundaries of jobs, reallocating tasks across workers in addition to optimizing the AI strategy in production.
...
The solution of the long-run problem has a similar flavor to the medium-run case, but rather than fixing the steps assigned to the worker it also chooses how tasks are allocated across different workers.
