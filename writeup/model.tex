...
In the \emph{medium run}, the boundary of the job remains fixed, but competitive pressure on the worker side and free entry into jobs let the firm do two things at once: optimize the job's time cost \emph{and} adjust the wage to the skill the job actually requires once AI potentially lowers the cost of executing its tasks.
...
\subsection{Solution to the Firm's Problem and Optimal AI Deployment}
\label{sec:optimization.short}

We now turn to solving the firm's short-run problem~\eqref{eq:totalcost_shortterm}.
...
We turn next to the medium-run problem~\eqref{eq:totalcost_mediumrun}, in which the firm minimizes the wage bill of the job, the product of the job's total skill and its total time.
...
Finally, the long-run problem has a similar flavor to the medium-run case, but rather than fixing the steps assigned to a the worker it also chooses how tasks are allocated across different workers. 
