...
Note that if the final step is a singleton task in $\T_I$ then no task is terminal.
    ...
    This implies that the realized fragmentation is equal to $1$ plus, for each $s_i \in F$ (i.e., each step $s_i$ that fails), a contribution of $\minTime{i}$ plus an additional $1$ in the event that $i > 1$ and the step immediately preceding $s_i$ did not fail.
