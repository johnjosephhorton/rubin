...
In the first two, step $k$ is not the chain's last step, so its output is never verified and it is automated, while in the third it is the last step, hence augmented.
...
We never need to know which chain is optimal, only whether each one, on its own, is cheaper than leaving the step to the human.
\end{sloppypar}
