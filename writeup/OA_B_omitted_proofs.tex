...
Proposition~\ref{prop:fragmentation} then follows by combining the three lemmas.
...
Note that $FI$ depends only on the step sequence $\mathcal{S}$: every step carries its own success probability $q_i$ whatever $\T$ does with it, so the realization of $F$ is unaffected by $\T$.
...
\]
The optimal task sequence chains the steps three at a time: a chain of $k$ steps costs $2^{k/2}$, or $2^{k/2}/k$ per step, which is minimized at $k=3$ with value $\tfrac{2\sqrt{2}}{3} \approx 0.9428$, below both the manual cost $\sqrt{2}$ and the per-step cost of any other chain length, so $OPT \to \tfrac{2\sqrt{2}}{3}\,m$.
...
Moreover, at every reorganization threshold (where the optimal AI strategy changes), $g^*$ does not fall, and at every transversal threshold it jumps upward discontinuously.
\end{lem}
...
If in addition $\alpha_0$ is transversal, then $\phi'(\alpha_0) \neq 0$, which together with $\phi'(\alpha_0) \le 0$ gives $\phi'(\alpha_0) < 0$ and hence $g_\T(\alpha_0) < g_{\T'}(\alpha_0)$: the marginal benefit jumps upward discontinuously.
...
The condition is nonetheless the generic case: a threshold already imposes $\phi(\alpha_0) = 0$, so requiring $\phi'(\alpha_0) = 0$ as well is a second equation in the same unknown, and the time cost vectors $(\manualTime{}, \AItime{})$ for which it holds form a Lebesgue-null set.
...
\begin{itemize}
    ...
    The states with $\skillcost{} = 0$ are exactly the job-opening states $(i,\; 0,\; \handofftime{i})$, whose value is the $V(i)$ defined above; without this restriction the recursion evaluated at such a state would read $V(i) = \min\{V(i),\, \cdot\,\}$, which every value below the other two branches satisfies and which therefore determines nothing.
