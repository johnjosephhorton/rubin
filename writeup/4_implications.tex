...
This does not depend on the predecessor, and is easiest to see when we set $q_{k-1}=1$, so that step $k-1$ becomes ``free'' and its share in \eqref{eq:ca_test} vanishes.
...
Proposition~\ref{prop:ca_local} formalizes this overturning of comparative advantage result and makes a more general claim.
...
Consider a workflow containing four steps built from the AI-hard step of Section~\ref{sec:comparative_advantage} and a version of its AI-easy step that AI completes less reliably, with two steps of each kind.
