...
- **Status**: ✅ **Addressed** 2026-09-04 (working tree). The six "Definition~1" labels in SA.D and SA.E were removed. Four were plain parentheticals; the other two were reworded so the sentence still reads, keeping the cross-reference and the E1/E2 clause. The "Definition~2" variant went with the removal of SA.B.2 under M4. One instance outlived that pass: the Figure SA.D.1 legend was generated from a label hard-coded as "EFI (Definition 1)" in `GPT_task_sequences_overlap.ipynb`, so the number was printed in the exhibit rather than the source. That label and the companion figure title were removed and the panels regenerated. No EFI variant is now cited by number anywhere, in the text or in a figure, so the collision with Definition~1 (Manual Step) and Definition~2 (Augmented Step) is gone; those two remain the only numbered definitions in the PDF.
...
- **Status**: ✅ **Addressed** 2026-09-04 (regenerated in a later pass; an earlier status line credited `b2d11a8` in error). `b2d11a8` rebuilt only the exposure panel, so Panel (b) still carried its 2026-09-01 render and the superseded 328/540 split. It has now been regenerated against the current `occupation_analysis_with_fragmentationIndex_def1.csv` and reads **433 (mean 0.573) / 435 (mean 0.630)**, matching the values this entry derived. The `_count` twin and the AI-execution panel were regenerated with it. The now-false cell-7 comment in `GPT_task_sequences_overlap.ipynb` was corrected in the same pass: the ceiling it describes belonged to the old series, and the current file has median 0.8095 with 0.7% tied. The `>=` tie rule was left as is, since at 0.7% tied it moves about six occupations and changing it would alter the split for a reason unrelated to this entry; that part of the proposed fix is still open as a judgment call.
