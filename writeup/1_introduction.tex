...
This allows us to observe empirical counterparts to all of the model's key objects: AI's ability to perform each step, the sequence in which steps are performed, and the current extent of AI execution.
...
This is what we find in the data, where AI execution operates over consecutive steps.
...
We test this on a second corpus, the Process Classification Framework of the American Productivity \& Quality Center, whose sequences practitioners documented rather than a language model imputed.
Controlling for the share of steps exposed to AI, workflows whose AI-exposed steps are more dispersed across the production sequence are associated with a lower share of their steps executed by AI.
The direction of the effect is the same in the O*NET sample but statistically imprecise.
...
First, we show that the first two predictions are not artifacts of the particular prompt we use to elicit workflow sequences from an LLM (Appendix~\ref{app:gptPrompts_robustness}).
Second, they are not artifacts of the particular way O*NET lists occupation tasks (Appendix~\ref{app:frequency_robustness}).
Third, we test our sequencing approach on two external sources, ordered based on actual workplace data rather than by a language model.
Our approach recovers the orderings in both external sources to a substantial degree, and Predictions~\#1 and~\#3 hold in the practitioner-ordered PCF corpus, while the matching leaves too few steps to test Prediction~\#2 (Appendix~\ref{app:external_validation}).
