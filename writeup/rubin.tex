...
\subsection{Task-Based Production and the Optimal Job Design}
...
This cost can be interpreted as the expense of context switching (or hand-off) between workers in the production process.
...
Thus, the firm's objective is to minimize the following expression, which differs from the previous formulation only by the extra $\handofftime{j}$ term added to the time cost for each job:
\begin{align}
...
Consider a job consisting of only two tasks, indexed $1$ and $2$ (We continue to suppress the job index $j$).
