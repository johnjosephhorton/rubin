...
\]
We refer to any collection of jobs satisfying the above conditions as a ``job design'' for the occupation, and denote it with $\mathcal{J}=\{\mathcal{J}_j\}_{j=1}^{J}$.
...
Note that AI improvements can be disproportionate across tasks (hence, the $ij$ subscript) and thus time and human capital cost savings may vary across occupation tasks.
...
The firm can either stick with its pre-AI job design \(\{\mathcal{J}_j^{(0)}\}\) and enjoy cost reductions enabled by the AI, or pay a redesign fee to reassign tasks optimally given the new cost structure and reconfigure its jobs to \(\{\mathcal{J}_j^*\}\).
We consider each scenario separately.

\paragraph{Scenario 1: No Job Redesign.}
If the firm preserves its pre-AI job design its gains from AI improvements will be:
\begin{equation}
\label{eq:no_redesign_gains}
\text{NoRedesignGains}(\alpha)
:=
\text{TotalCost}(\mathcal{J}^{(0)}; \hccost{}, \timecost{}) - \text{TotalCost}(\mathcal{J}^{(0)}; \widetilde{\hccost{}}, \widetilde{\timecost{}}).
\end{equation}
This is the net gains from time cost savings with the pre-AI job structure, and depends on the improved quality level of the AI ($\alpha$).
Note that by definition, the time cost savings is weakly greater than zero, so the firm always deploys the new technology.\footnote{None of the implications of the model will change if we assume adoption of the new technology comes at a nonzero, fixed cost. Thus, we assume acquiring the advanced AI is costless for the firm for simplicity.}

\paragraph{Scenario 2: Redesigning Jobs.}
If the firm chooses to restructure jobs to the new optimal design \(\{\mathcal{J}_j^*\}\) its cost savings from this change will be:
\begin{equation}
\label{eq:job_redesign_saving_alpha}
\text{RedesignSavings}(\alpha)
:=
...
Reallocating tasks to new jobs, however, is costly for the firm.
...
\text{RedesignGains}(\alpha)
:=
\text{RedesignSavings}(\alpha)\;-\;\text{RedesignCosts}(\mathcal{J}^{(0)}, \mathcal{J}^{*}).
\end{equation}


In the short run, the firm evaluates whether the incremental gains from reorganizing tasks exceed the time‐savings achieved by plugging improved AI into its existing job structure.  
Proposition~\ref{prop:shortrun_redesign} makes this tradeoff precise and provides the condition determining the job redesign decision.  
...
Let $\mathcal{J}^{(0)}$ be the pre–AI optimal partition and $\mathcal{J}^*$ its reoptimized counterpart under improved AI quality.
The firm will switch from $\mathcal{J}^{(0)}$ to $\mathcal{J}^*$ in the short run if and only if
\[
...
\]
Moreover, there exists a unique threshold $\tilde\alpha$ such that for all $\alpha>\tilde\alpha$ the inequality holds and the firm strictly prefers the redesigned partition.
\end{proposition}
See Appendix~\ref{app:shortrun_redesign_proof} for the proof.
Intuitively, Proposition~\ref{prop:shortrun_redesign} states that the short‐run redesign decision balances the efficiency gains stemming from a more optimal task allocation against the one‐off costs of reorganizing jobs.
Moreover, the existence of a unique AI‐quality threshold creates a tipping‐point effect: although AI improvements yield cost savings for modest increases in the AI quality, it is not worthwhile to redesign jobs in the short run until quality exceeds a certain threshold, at which point restructuring becomes optimal.  
...
= 
\sum_{j=1}^J \Biggl[\Biggl(v_L + \sum_{i \in \mathcal{J}_j} \widetilde{\hccost{ij}} \Biggr) \Biggl(\widetilde{\handofftime{j}} + \sum_{i \in \mathcal{J}_j} \widetilde{\timecost{ij}} \Biggr)\Biggr].
\end{align}
...
Define
\[
\Delta\mathrm{Gain}(\alpha)
:=
\mathrm{RedesignGains}(\alpha)\;-\;\mathrm{NoRedesignGains}(\alpha),
\]
the extra payoff from redesign versus pure time‐savings.  By construction the firm redesigns exactly when $\Delta\mathrm{Gain}(\alpha)\ge0$.  
Using the definition of each term, the firm redesigns jobs when:
\begin{align*}
\Delta\text{Gain}(\alpha) 
& =
\text{TotalCost}(\mathcal{J}^{(0)}; \hccost{}, \timecost{}) - \text{TotalCost}(\mathcal{J}^{*}, \widetilde{\hccost{}}, \widetilde{\timecost{}})
-
\text{RedesignCosts}(\mathcal{J}^{(0)}, \mathcal{J}^{*}) \nonumber \\
...
C_{\mathcal{J}}(\alpha)
:=
\mathrm{TotalCost}\bigl(\widetilde t(\alpha),\,\widetilde c(\alpha),\,\mathcal{J}\bigr).
\]
...
Moreover, since $D(\alpha)$ is strictly increasing whenever $C_{\mathcal{J}^{(0)}}-C_{\mathcal{J}^*}$ strictly increases, we have
\[
D(\alpha)
\begin{cases}
<0, & \alpha<\tilde\alpha,\\
...
\end{cases}
\]
Thus $\tilde\alpha$ is the unique AI‐quality threshold above which the firm strictly prefers redesign.
\end{proof}
