...
\begin{document}
\title{The Economics of Generative AI}
...
\begin{abstract}
\noindent The distinctive feature of generative AI is that it generates: code, writing, images, music, videos, and so on.
It does this at essentially zero marginal creation cost.
These generations are made in response to some request or ``prompts,'' but these AI generations only become economic goods if judged suitable for some human need.
This depends on human judgment in evaluation, the skill in crafting the prompt, and the technical capability of the generative model  jointly determines the productivity. 
...
One economic approach to understanding a new technology is to frame it as a change in costs.
This approach has been applied to illumination (electicity), mechanical power (the steam engine), personal tranportation (the automobile), computation (the computer)---all of which made some recognizable economic good much cheaper.
But the approach rests on undestanding what the new technology does and hence what is it making cheaper. 
The problem with generative AI---from an economic modeling standpoint at least---is that it is unclear what this kind of AI \emph{cannot} do even now, never mind over the next 5 or 10 years.
%This is makes it difficult to reason about this technology in terms of ``types'' of tasks.
%There is talk about Artificial General Intelligence (AGI) that might occur in our lifetimes.

One thing that is clear is that is does seem reasonable to analogize generative AI to a kind of labor.
It performs idenfiable human ``tasks'' in a way, say, a steam engine does not.
However, unlike with computers where we can it replaces rote computation, what generative AI does it not so clearly defined.
Stepping back, the distinctive feature of generative AI is, at least in part, right there in the name: it generates.
Writing, code, images, music, videos, and so on---but only in response to a human request, or ``prompt.''
...
What AI generates is not inherently useful, but rather depends on human judgment.
...
1) asking the AI to do that task, and 
2) evaluating the machine's efforts. 
Each of these steps is a human-provided task that can, in turn, be AI-augmented. 

This paper presents a simple task-based model of generative AI. 
A productive process is a collection of tasks.
However, those tasks are assumed to be done in sequence, with interdependecies.
At each step, the task is given to the factor with the \emph{conditional} comparative advantage---though this can depend on what tasks are nearby.
...
To start, imagine a job that consists of just a single task.
...
This $q$ captures the model's capability but also reflects the capability of the human at prompting and judging and the nature of the task.\footnote{
...
To indicate a task being done by a human, we will use the notation $\human{\cdot}$ and we will use $\machine{\cdot}$ for a task done by a machine.
...
The higher the human performance costs for a task, the more likely a worker is to try an AI, even a fairly incapable one (i.e., low $q$).
...
%Note that we are ignoring capital costs---all that are present are labor costs.
Note that with a high capable AI $(q = 1)$ with output that is very cheap to evaluate ($c_m \approx 0$), costs can be very low.

In isolation, raising $q$ has two effects on the kinds of labor that is used.
It displaces direct labor doing tasks, as it is more likely that $c_m/q < c_h$.
But it increases the demand for management-labor of the extensive margin, as more tasks flip from being done by a human to being done by a machine.
...
task 1 by a machine and task 2 by human, $\cost{\machine{1}\human{2}} = c_m/q_1 + c_h$; and task 1 by human and task 2 by machine $\cost{\human{1}\machine{2}} = c_h + c_m/q_2$.
...
Imagine there is a measure 1 of worker/consumers.
Workers supply labor inelastically.
There are $M$ \emph{potential} products in the market which have idiosyncratic chains of tasks and associated costs.
%NB: It might be useful to create a two task economy where all potential products are considered e.g., 11, 12, 22, 21, 22. 
%with $c_1, c_2, q_1, q_2$.
Each product has the same demand curve,
\begin{align}
  d(p) = \max \{ 0, \frac{\bar{p} - p}{\bar{p}} \}.
\end{align}
where $\bar{p}$ is exogenous and $p$ is the product market price of the good.
Note that when $p \ge \bar{p}$, no one buys the good; when $p = 0$, everyone buys the good. 

Let $l_i$ be the labor required to create one unit of good $i$, under the optimal production chain, given $w$.
There are constant returns to scale in production i.e., to product N units of the good, it requires $N l_i$ units of labor.
A unit of the good has a cost of $p_i = l_i w$ where $w$ is the market wage, and so $d(l_i w)$ is how much of good $i$ gets produced.

Market clearing is defined by a wage, $w$ and a set of products, $M' \subseteq M$, that are produced in equlibrium.
The set $M'$ is defined as those with labor requirements such that $\bar{p} < l_i w$.
Total expenditure equals total labor income at market clearing, or
\begin{align}
  w  &= \sum_{i \in M'} p_i d(l_i w).
%  1  &= \sum_{i \in M'} l_i d(l_i w).\\
%  1  &= \sum_{i \in M'} l_i \frac{\bar{p} - w l_i}{\bar{p}}.\\
%  1  &= \sum_{i \in M'} l_i .\\
% \frac{1}{(1 - \frac{w}{\bar{p}})} &= \sum l_i
\end{align}
If we let $\bar{l}$ be the average amount of labor used per good and $|M'|$ is the number of good produced in equilibrium, we have
\begin{align}
  w = \bar{p} \left(1 - \frac{1}{|M'|\bar{l}} \right).
\end{align}

(Not formal - economic intuition - not proven!)
What happens when $q$ increases i.e., AI models become more capable? 
This causes all $l_i$ to fall or at least stay the same, as goods need less labor to produce.
This tends to lower wages (NB: Welfare increases, as total consumption increases. In real terms, I don't think wages fall(?)
This raising of $q$ also causes $|M'|$ to increase, as more goods now become economical to produce.
There are two reasons: technical production costs fall for these marginal costs, pushing them over the limit, plus labor is freed up from all the products previously produced.
These marginal goods are also particularly labor intensive goods, having $l_i \approx \bar{p}/w$---the highest possible labor usage.
This will tend to damped the shock.
NB: This re-creates the Acemoglu ``labor has an advantage in new tasks'' (here, products) type result but for economic reasons rather than by assumption. 


Other thought---if automation chains cause more goes to go to $l_i \approx 0$, further advances in $q$ become irrelevant, meaning a smaller pool of workers are ``freed up'' by advances in $q$. 
...
Unlike the task-in-isolation view, it is possible to reap further benefits from automation even from tasks where humans have an absolute advantage.
...
This chaining builds into the model a consideration pointed out by \cite{bresnahan2020artificial}, who argues there has been little to no task-level substitution with the AI systems we actually see in production:
``the transition to ICT-based production has largely proceeded at the system level, not the task level.''
...
In the model, it is the possibility of chaining that can create large productivity gains. 

The model offers several insights.
First, it predicts what kinds of tasks tend to be amenable to automation in terms of economic fundamentals.
Tasks that are expensive for humans to do are natural candidates for automation.
But a less obvious insight is that the extent of generative AI usage depends on the costs of judging the model's output.
...
Search goods are trivially easy to evaluate, whereas inspection goods and credence goods might have prohibitively expensive evaluation costs. 
These differences will affect the adoption of AI by task.

Endogenizing whether an AI can ``do'' a task as depending on the stakes of the productive context likely allows for insights into the pace and location of adoption. 
...
In the model, it matters what tasks are ``next'' to each other in some productive process, because tasks that have a high ability to be automated can be chained together.

But beyond tasks considered in isolation, the model predicts, qualitatively tasks are likely to be targets for automation---namely those that allow not just one-off task replacement but system-level replacement.

As such, we can get ``automation dispersion'' where tasks are automated in some production settings but not others.

%% The model gives another interpretation of the Acemoglu observation about so-so technologies:
%% ```Examples of so-so technologies include automated customer service, which has displaced human service representatives but is generally deemed to be low quality and thus unlikely to have generated large productivity gains.''
...
The literature on the cost of evaluation of goods---search goods, inspection goods, experience goods, credence goods---gives us an economic way to think about what tasks are are likely to be automated.

(1) The model primitives map to things that can be measured.
...
(4) It also highlights that we are likely to see investment not only in direct model capabilities, but the ways in which they can be prompted and the results evaluated. 
...
Examples include \cite{autor2003skill}'s ``computation substitutes for routine cognitive tasks and complements non-routine cognitive tasks'' is a useful simplification of what computers do. 
...
\section{The economic approach to modeling technology}
%\topic{There are contrasting ways consider the effect of technology on labor markets.}
There are contrasting ways consider the effect of technology on labor markets.
...
There are criticisms of the task view of technology. 
A contrasting view is offered by \cite{bresnahan2020artificial}, who argues there has been no task-level substitution with the AI systems we actually see in production:
``the transition to ICT-based production has largely proceeded at the system level, not the task level.''
It has all been capital deepening.
To the extent there is important labor-capital substitution, it will happen at the system level.
There is a long tradition in studies of ICT to take a ``systems'' view---that ICT requires a reconfiguration of production to reap the benefits \citep{bresnahan2002information}. %(Cite Erik).
...
An argument for a task-based view with respect to generative AI is that it does due a number of distinct \emph{tasks}.
...
The literature to date on AI specifically (as opposed to automation) in the task-based tradition has emphasized AI as a prediction technology \citep{agrawal2019}.
While forecasting and prediction are certainly key firm tasks, relatively few workers have that as a task. 
...
Generative AI models have shown the ability to create output similar to or better than even expert humans can produce over a broad class of knowledge-worker tasks.  
...
A natural starting point for the effects of generative AI on labor markets is to try to classify tasks as being the kinds of the things generative AI can ``do'' or ``not do.''
But this misses two considerations.
First, tasks are not done in isolation and most productive tasks are interpendent.
Second, whether a model's output at some task is sufficient depends largely on what that output will be used for.
For example, for the task ``summarizing a document'': whether the AI can do this task, in an economic sense, depends if the document is being summarized is the Supreme Court or for Steve Caruthers.\footnote{A guy I made up who is just browsing Wikipedia.} 

Economic forces tend to create dependencies between tasks both within in a job and within a firm.
Since economists first recognized the division of labor as an essential consideration, the interdependence between tasks has been noted.
...
\subsection{Using an AI for some task has qualitative similarities to delegating a task to another human or team.}
...
\subsection{AI will create new labor-labor substitution possibilities for ``Rick Rubin'' reasons.}
