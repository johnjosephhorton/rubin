...
Task pairs 1--2 and 2--3 in Figure~\ref{fig:tent_poll} illustrate such configurations.
\begin{figure}[h]
  ...
  \begin{footnotesize}
    ...
    Task 1 has a high time cost and low human‐capital requirement (wide and short), followed by Task 2 with a low time cost and high human‐capital requirement (narrow and tall).  
    Tasks 2 and 3 form another tent‐poll pair with a similar mismatch.
  \end{footnotesize}
\end{figure}
Each task in the graph is represented by a rectangle of width $t$ (time requirement) and height $c$ (human capital requirement).
The sharp contrast between tent-poll tasks creates substantial inefficiencies, discouraging AI chaining and instead pushing firms toward individual task specialization.
...
Some of the hardest areas to automate are not defined by task difficulty alone, but by the awkwardness of their neighbors.
...
To perform each task $i\in\mathcal{J}_j$, a worker must first acquire the required skills, incurring a one‐time human capital cost $\hccost{ij}>0$. 
Once trained, each task takes time $\timecost{ij}>0$ to complete. 
...
For simplicity we set $v_L=0$ in the graph.
In each panel, the numbered, solid‐lined rectangles denote the individual tasks.  
...
\begin{figure}[h]
  ...
  \end{center}
  \begin{footnotesize}
    \emph{Note:} Each panel of the graph shows the geometric representation of the wage bill for a job design of an occupation with two tasks (with $v_L=0$).  
    The shaded areas represent job wage bills: in the left panel, two separate jobs cover tasks [1] and [2]; in the right panel, a single job covers both tasks [1,2].
  \end{footnotesize}
\end{figure}
...
\[
  xy + wz \le (x + w)(y + z),
\]
which means bundling any two tasks cannot reduce total cost.  
Geometrically, for any number of tasks, the sum of the areas of individual rectangles never exceeds the area of the single bounding rectangle that covers two or more tasks assigned to one worker.
...
The hand-off cost is an inherent characteristic of a task and applies only when that task sits at the boundary between two jobs.  
Specifically, when transitioning from the last task of job $j$ to the first task of job $j+1$, the worker in job $j$ incurs a hand‐off time cost $\handofftime{j}\ge0$.  
If the task is not followed by a job boundary, no hand‐off cost applies.  
This reflects that there are no context‐switching costs within a job, since a single worker performs all its tasks.
...
In this section we focus on a single task and drop the job ($j$) and task indices ($i \in \mathcal{J}_j$) for notational simplicity.
...
\end{enumerate}
We defer discussion of task automation to the next section as it only happens in our model when two or more tasks are present.
...
After prompting the worker evaluates the AI's output, incurring an additional time cost $\timecost{e} > 0$.
...
This expression shows how the overall time cost of a task is set by the trade-off between manual execution and AI-augmentation.
...
A task is \emph{automated} if it appears in a sequence of two or more AI-completed tasks and is not the last task in that sequence.  
Consequently, an AI-performed task in isolation (a run of length one) is classified as augmented.  
...
An exogenous improvement in AI quality—captured by in increase in $\alpha$—affects each task $i$ in two ways:
\[
\widetilde{\timecost{ij}}(\alpha)
\;=\;\min\Bigl\{\timecost{ij},\,\tfrac{\machinetime}{q_i(\alpha)}\Bigr\} \leq \timecost{ij},
\qquad
\widetilde{\hccost{ij}}(\alpha)\;\le\;\hccost{ij},
\]
where $q_i(\alpha)$ rises in $\alpha$.  
The AI improvement can potentially also generate entirely new “oversight” tasks (e.g., prompt‐engineering, exception handling) with their own
$\bigl(\hccost{o}(\alpha),\,\timecost{o}(\alpha)\bigr)$.  
Together, the firm’s per‐unit cost becomes:
\begin{equation}
\label{eq:totalcost_ai}
\sum_{j=1}^J 
\Bigl(v_L + \sum_{i\in\mathcal{J}_j}\widetilde{\hccost{ij}}(\alpha)\Bigr)
\Bigl(\handofftime{j}(\alpha) + \sum_{i\in\mathcal{J}_j}\widetilde{\timecost{ij}}(\alpha)\Bigr),
\end{equation}
with $\handofftime{j}(\alpha)$ the (possibly AI‐aided) hand‐off cost.

\paragraph{Short‐run:}  
Job boundaries are fixed at $\{\mathcal{J}_j^{(0)}\}$.  
The firm trades off
\[
\underbrace{\Delta\!\bigl[\text{TotalCost}(\alpha)\bigr]}_{\text{direct wage‐bill savings}}
\quad\text{vs.}\quad
\underbrace{\kappa \sum_j \mathbf{1}\{\mathcal{J}_j^{(0)}\neq\mathcal{J}_j\}}_{\text{re‐design cost}},
\]
where $\kappa$ is a fixed cost of reorganizing each job.  
If savings exceed re‐design costs, the firm renegotiates tasks—otherwise it rides out the old partition but only pays for the human capital still put to use.

\paragraph{Long‐run:}  
Workers can re‐train (redeploy human capital) and firms freely re‐partition tasks.  
Everything is re‐solved as:
\[
\min_{\{\mathcal{J}_j\}}\;
\sum_{j=1}^J 
\Bigl(v_L + \sum_{i\in\mathcal{J}_j}\widetilde{\hccost{ij}}(\alpha)\Bigr)
\Bigl(\handofftime{j}(\alpha) + \sum_{i\in\mathcal{J}_j}\widetilde{\timecost{ij}}(\alpha)\Bigr),
\]
yielding a new optimal design $\{\mathcal{J}_j^*(\alpha)\}$.  
In particular:
\begin{itemize}
  \item As $\alpha$ increases, $\widetilde{\timecost{ij}}(\alpha)$ weakly decreases, and some tasks drop out entirely (full automation).
  \item The hand‐off component of the wage bill
  \[
    \Bigl(v_L + \sum_{i\in\mathcal{J}_j}\widetilde{\hccost{ij}}(\alpha)\Bigr)\,\handofftime{j}(\alpha)
  \]
  may shrink because:
  \begin{itemize}
    \item the human‐capital term falls; and/or
    \item $\handofftime{j}(\alpha)$ declines if AI chains tasks across jobs more efficiently or assists with hand‐offs.
  \end{itemize}
  \item New oversight tasks $\{o\}$ enter $\mathcal{T}$, redirecting effort toward exception handling and quality control. The new tasks typically require less human capital $\widetilde{\hccost{ij}}(\alpha)$ than those they replace.
\end{itemize}

\paragraph{Implications for Workers \& Wages:}
\begin{itemize}
  \item {\bf Short‐run:}  Firms only compensate for $\widetilde{\hccost{ij}}^{\rm used}$, so workers whose skills partly deprecate face wage cuts or redeployment to oversight roles.
  \item {\bf Long‐run:}  Workers optimally invest in the (smaller) set of skills $\{\hccost{ij}^*(\alpha)\}$ that survive alongside AI—leading to a new wage schedule via 
  \[
    w_j^*(\alpha)
    = v_L \;+\;\sum_{i\in\mathcal{J}_j^*(\alpha)} \widetilde{\hccost{ij}}(\alpha).
  \]
\end{itemize}
