...
A single (representative) firm produces all goods under perfect competition.
...
The firm can partition the production process $\mathcal{T}_k$ into a number of jobs, where each job is a contiguous block of tasks performed by a single worker.
With $|\mathcal{T}_k| = \tau_k$ tasks, there are $2^{(\tau_k - 1)}$ unique ways of designing jobs.
For example, if $\tau_k = 3$, the production process of good $k$ consists of three tasks, $\{1,2,3\}$, with $2^{(3-1)}=4$ unique job designs.
...
Suppose the firm breaks down the production process $\mathcal{T}_k$ into $J_k$ jobs.
Let $i_j^k$ denote the first task in job $j \in \{1,\ldots,J_k\}$ in the production process of good $k$.
We have:
\[
1 = i_1^k < i_2^k < \cdots < i_{J_k+1}^k = \tau_k+1.
\]
This implies that the first task, $i_1^k=1$, is the first task of the first job ($j=1$), while the (imaginary) task $\tau_k+1$ is considered the first task of the (imaginary) job $j = J_k+1$.
With this notation, the $j$th job for good $k$ is a collection of consecutive tasks in the production process $\mathcal{T}_k$ given by:
\[
\mathcal{J}_j^k = \{i_j^k, i_j^k+1, \cdots, i_{j+1}^k - 1\}.
\]
Going back to the earlier example with $\tau_k=3$, the four potential job designs can formally be expressed using this notation as follows:
\begin{align*}
\{[1][2][3]\} \Longleftrightarrow \ J_k = 3:  & \quad \mathcal{J}_1^k = \{1\}, \quad  \mathcal{J}_2^k = \{2\}, \quad \mathcal{J}_3^k = \{3\}, \\
\{[1,2][3]\} \Longleftrightarrow \ J_k = 2: & \quad \mathcal{J}_1^k = \{1, 2\},  \quad \mathcal{J}_2^k = \{3\}, \\
\{[1][2,3]\} \Longleftrightarrow \ J_k = 2: & \quad \mathcal{J}_1^k = \{1\}, \quad \mathcal{J}_2^k = \{2, 3\}, \\
\{[1,2,3]\} \Longleftrightarrow \ J_k = 1: & \quad \mathcal{J}_1^k = \{1,2,3\}.
\end{align*}
...
The firm buys all intermediate goods $\{x_1^k, \cdots, x_{J_k}^k\}$ from workers and produces the final good $k$ at no additional cost using a Leontief production function:
\[
x_k(x_1^k, \cdots, x_{J_k}^k) = \min\{x_1^k, \cdots, x_{J_k}^k\}.
\]
This is merely a technical assumption reflecting the fact that to produce one unit of good $k$, each set of tasks in $\mathcal{T}_k$, across all jobs $j \in \{1,\ldots,J_k\}$, must be performed exactly once.
We further assume that tasks in $\mathcal{T}_k\!$ are performed in fixed proportions.
For instance, if task $m \in \mathcal{J}_j^k$ takes 30 minutes and task $n \in \mathcal{J}_j^k$ (with $m \neq n$) takes 45 minutes, then producing one unit of good $k$ requires a total of 75 minutes spent on both task $m$ \underline{and} task $n$ because both must be performed exactly once; it is not possible to perform task $m$ multiple times while performing task $n$ only once.
We formalize this assumption later after introducing the full production model.
As we will see, this helps simplify the equilibrium characterizations significantly.

In order to produce the intermediate good $x_j^k$, a worker must complete the tasks associated with that job, namely $\mathcal{J}_j^k$.
Each task $i \in \mathcal{J}_j^k$ necessitates acquiring human capital $\taskhc{ij}^{k} > 0$ and takes time $\tasktime{ij}^{k} > 0$ to complete.
The worker incurs the human capital cost for task $i$ only once at the beginning of his tenure at job $j$, but spends time $\tasktime{ij}^{k}$ completing task $i$ for producing each unit of good $x_j^k$.
The total human capital acquired by the worker employed in job $j$ in the production process of good $k$ is:
\[
\sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k},
\]
while the total time he spends on tasks in $\mathcal{J}_j^k$ to produce one unit of output is:
\[
\sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}.
\]

Having acquired the necessary human capital, the production of each intermediate good uses labor as the only input.
...
To recoup their training costs and to make them indifferent between working and not working, worker compensation must account for human capital costs.
The firm must pay workers for their time, regardless of the task; hence, the wage bill for job $j$ is:
\[
\text{WageBill}_j^k = \Biggl(v_L + \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}\Biggr) \Biggl(\sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}\Biggr).
\]

Without additional frictions, the firm could minimize the cost of production by assigning a single worker to each task.
What prevents this is a hand-off cost---a cost incurred for a task that, while requiring no extra human capital, does require additional time.
...
The worker can do the hand-off in one of two ways: manually or using an AI.
If he does so manually, he must spend extra time $\tasktime{w_j}^k \geq 0$ to hand off the output of job $j$ in the production process of good $k$.
However, the worker also has access to an AI technology that can perform the same hand-off for a time cost $\tasktime{m_j}^k \geq 0$ with success probability $q_j^k$.
...
\]
If the worker deploys the AI instead of handing off the job manually, he incurs an additional time cost equal to $\alpha_j \geq 0$ times the AI’s hand-off cost.
This represents the cost of AI \emph{management} that the worker must bear, with $\alpha_j$ reflecting the worker's management efficiency.
...
\]
We denote the optimal hand-off time at the end of each job $j$ for good $k$ by:
\[
...
Thus, the total time required for job $j$ to produce one unit of $x_j^k$ is:
\[
\tasktime{j}^k = \tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k},
\]
which implies that, after accounting for the hand-off friction, the wage bill for job $j$ per unit of good $x_j^k$ is:
\[
\text{WageBill}_j^k = \Biggl(v_L + \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}\Biggr) \Biggl(\tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}\Biggr).
\]

The firm's problem has a nice geometric interpretation, as shown in Figure~\ref{fig:job_design}.
The productive process is a sequence of stacked rectangles of height $\taskhc{ij}^k$ and width $\tasktime{ij}^k$.
...
The firm seeks to minimize the total cost of producing good $k$ by optimally designing jobs.
Assuming there are $J_k$ jobs in the optimal design, the overall production cost per unit of good $k$ is:
\[
\text{TotalCost}_k = \sum_{j=1}^{J_k} \Biggl[\Biggl(v_L + \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}\Biggr) \Biggl(\tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}\Biggr)\Biggr].
\]

\paragraph{Goods Prices.}
A worker employed in job $j$ in sector $k$ produces an intermediate good $x_j^k$, which is sold back to the firm in a competitive market.
The production cost per unit of $x_j^k$ can be written as:
\[
\text{WageBill}_j^k = \Biggl(v_L + \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}\Biggr) \Biggl(\tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}\Biggr) = w\,\labor{j}^k,
\]
where $w$ is the wage rate and $\labor{j}^k$ denote the total amount of labor spent on producing a unit of good $x_j^k$.
%\[
%\labor{j}^k = \tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}.
%\]
% is this definition of \labor{}^k correct? i think it's weird because later we fix wage [w = v_L + \sum HC], but wage is actually different across tasks if it's defines as v_L + \sum HC.
Since markets are perfectly competitive, price of intermediate good $j$, $p_j^k$, equals the marginal cost $w\,\labor{j}^k$.
Normalizing $w=1$ yields:
\[
p_j^k = \labor{j}^k.
\]
Furthermore, perfect competition in the final goods market implies that the price of the final good $p^k$ equals the sum of the prices of its constituent intermediate goods:
\[
p^k = \sum_{j=1}^{J_k} p_j^k = \sum_{j=1}^{J_k} \labor{j}^k.
\]
...
\]
so that $p^k = \labor{}^k$.
The parameter $\labor{}^k$ can be interpreted as the overall labor employed in sector $k$ to perform tasks in the production process $\mathcal{T}_k$.
Due to the assumption that tasks are done in fixed proportions, once $\labor{}^k$ is determined, all subsequent $\labor{j}^k$ for $j \in \{1,\ldots,J_k\}$ are pinned down.
Formally, the fixed proportions assumption implies that for some $\gamma_j > 0$:
\[
\frac{\labor{j}^k}{\labor{1}^k} = \gamma_j.
\]
Note that $\gamma_1 = 1$.
This in turn gives:
\[
\labor{}^k = \sum_{j=1}^{J_k} \labor{1}^k \Bigl(\frac{\labor{j}^k}{\labor{1}^k}\Bigr) = \labor{1}^k \sum_{j=1}^{J_k} \gamma_j \Longrightarrow \frac{\labor{}^k}{\labor{1}^k} = \sum_{j=1}^{J_k} \gamma_j,
\]
and thus:
\[
\labor{j}^k = \frac{\gamma_j}{\sum_{j=1}^{J_k} \gamma_j} \labor{}^k.
\]
Therefore, it is without loss of generality to focus our attention on working with $\labor{}^k$, because once that is determined, the labor for each subsequent job is also fixed.

\paragraph{Consumption.}
A representative consumer supplies one unit of labor inelastically and has a CES utility function over the $K$ final goods:
\[
...
\]
where the right hand side is the total labor income (i.e., $w=1$), aggregating income across all jobs and all sectors to which the consumer supplies labor.

\paragraph{Labor Market Clearing.}
The consumer inelastically supplies one unit of homogeneous labor and is indifferent between working in different jobs and sectors, as each compensates him for his foregone leisure and the human capital he has acquired.
...
\]
where $\labor{j}^k$ is the per unit amount of labor allocated to the production of intermediate good $j$ in the production process of good $k$.
Since $p^k = \labor{}^k$ and we have normalized $w=1$, this condition is equivalent to the budget constraint 
\(\sum_{k=1}^K p^k\, x_k = 1\).
...
w = 1, 
& \qquad
p^k = \labor{}^k = \Biggl(v_L + \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}\Biggr) \Biggl(\tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}\Biggr), \\
...
\]
and $L_k^* = \labor{}^k\,x_k^*$ is the labor used in sector $k$.
...
See Appendix~\ref{app:GE} for a step by step derivation of equilibrium prices and allocations. 
Furthermore, in Appendix~\ref{app:GE_comp_stats} we show comparative statics of equilibrium objects with respect to task automation.
...
\section{Sectoral Employment and Equilibrium Allocations}
\label{app:GE}
...
\[
\left(\frac{x_f}{x_g}\right)^{-\tfrac{1}{\sigma}} = \frac{\labor{}^f}{\labor{}^g}\,\cdot\,\frac{\delta_g}{\delta_f},
\quad
\Longrightarrow
\quad
\frac{x_f}{x_g} = \left(\frac{\labor{}^g}{\labor{}^f}\,\cdot\,\frac{\delta_f}{\delta_g}\right)^{\sigma}.
\]
Define
\[
...
\section{Comparative Statics with Respect to Automation}
\label{app:GE_comp_stats}
