...
Equation \ref{eq:wagebill} has a nice geometric representation.
...
Figure~\ref{fig:wage_bill} gives the graphic representation of an occupation with two tasks and thus two potential job designs: [1][2] and [1,2].
The numbered rectangles bounded by solid lines in each panel represent the two tasks.
In the [1][2] design (left panel), each task is assigned to a separate worker.
The wage bill of job 1 in this design is the shaded area bounded by rectangle 1 and the wage bill of job 2 is the shaded area bounded by rectangle 2.
In the [1,2] job design where both tasks are assigned to the same worker (right panel), the wage bill of the only job in the occupation is the area of the shaded rectangle whose width is the sum of time costs of both tasks and whose height is the sum of human capital of the two tasks.
...
\begin{figure}
  ...
  \end{center}
\begin{footnotesize}
  \emph{Note:} This graph shows the geometric representation of the wage bill for an occupation with two tasks. The shaded area in the left panel shows the wage bill of the two jobs in the [1][2] design. The shaded area in the right panel shows the wage bill of the single job comprised by both tasks in the [1,2] design.
\end{footnotesize}
\end{figure}
%%%%%%%%%%%%%%%%% Single Graph Representation %%%%%%%%%%%%%%%%%
...
As it turns out, the minimum cost way of designing jobs is to assign each task to an individual and have each worker specialize in completing a single task. 
This conclusion is not limited to the current example and these specific values.
In fact, for any set of human capital and time cost values the same conclusion will hold.
This is simply because the sum of area of individual rectangles is smaller than the area of any permutation in which one or more rectangles are combined into one job and are assigned to a worker.
...
Without additional frictions, the firm could minimize production costs by assigning a single worker to each task.
This is not a consequence of the chosen parameter values in the specific example of Table~\ref{tab:job_design}, Panel A.
In fact, the same conclusion holds for any arbitrary set of parameters since $xy+wz \leq (x+w)(y+z)$ for any nonnegative $x,y,w,z$.
In other words, for any number of tasks the sum of individual rectangle areas as in Figure~\ref{fig:wage_bill} is always strictly smaller than any job design in which two tasks or more are bundled together and assigned to one worker.

To assume away this trivial arrangement we introduce a source of friction: ``hand-off cost”---a cost associated with job transitions that, while not requiring additional human capital, does take time.
...
The hand-off time cost is an intrinsic characteristic of a task which is only realized when a job boundary falls between that task and the task coming after it (and is to be completed by another worker in another job).

While the firm's cost minimization problem remains unchanged, its solution becomes nontrivial due to the additional hand-off cost incurred after completing the final task in each job.  
The firm's objective is to minimize the following expression, which differs from equation \ref{eq:totalcost_no_handoff} only by the extra $\handofftime{j}$ term added to the time cost term of each job:
\begin{align}
...
\end{array}
\]
Notice that while the earlier optimal job design (i.e., the [1][2][3] design in Panel A) assigned a single worker to each task, in Panel B the cost-minimizing production method becomes the [1,2][3] design after introduction of the hand-off friction.
In the new optimal design, the firm minimizes its cost by bundling tasks 1 and 2 together in a single job [1,2], avoiding the substantial hand-off cost incurred between these tasks in the previously optimal [1][2][3] design.


\subsection{Computational Problem $\rightarrow$ Brendan}
The firm's problem has a nice geometric interpretation as well.
...
On one hand, consolidating tasks into a single job can lead to inefficiencies, illustrated by the cross-diagonal rectangles, that arise from bundling tasks together.  
On the other hand, breaking production into multiple jobs incurs an additional hand-off cost (shown in pink). 
Thus, the firm must trade off these factors to determine the most cost-effective job design.
...
\subsection{Adding AI Automation of Tasks}
...
In the three tasks example, if in the [1][2][3] design the first task was done by AI and the other two by humans, we would write $\machine{1}\human{2}\human{3}$.
...
Furthermore, we suppress the switching cost $\handofftime{}$, which the worker incurs upon completing task $2$, since our focus is solely on tasks performed within a single job (and note that no hand-offs occur between tasks within the same job).
...
Note that its boundaries with the half-automated regions (i.e., \(\machine{1}\human{2}\) and \(\human{1}\machine{2}\)) are sloped, meaning that improvements in one task's AI quality can bring it into the \(\machine{1|2}\) region, without an intervening partial automation step.
...
\begin{proposition}[Recurrence for Valid Arrangements] \label{proposition:num_arrangements}
  Let \(f(n)\) denote the number of valid arrangements of \(n\) tasks in a job, where each task can be either performed by a human (exactly one task per human block) or as part of a machine chain (a consecutive sequence of tasks). Then the following recurrence holds:
  \[
...
We now connect back to the model of production we introduced in \ref{sec:taskbased_prod}, introduce consumption and present the general equilibrium of the model.
...
For ease of exposition, we focus on a representative sector and omit the subscript $k$ from our notation until Section~\ref{sec:household_GE}, where we introduce the household's problem.\footnote{This simplification means that whenever we refer to variables or production processes, they implicitly pertain to a typical final good.}
