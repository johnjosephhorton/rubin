...
The enthusiasm reflects the usefulness of the economist's task-based view of production: many goods are produced by the completion of identifiable tasks, and those tasks often have a recursive sub-structure, allowing them to be decomposed into smaller, simpler tasks.
...
Predictions based on task content seem inherently difficult; as a comparative exercise, our failure to predict the extent of offshoring---arguably a far simpler forecasting problem---should be humbling \citep{ozimek2019overboard}.
...
Understandably, most work to date has addressed this question by focusing on the actual content of the tasks and technical capabilities of machines, as determined by some engineering or expert estimate \citep{frey2017future, felten2021occupational, eloundou2023gpts}; 
This work has also kept with the conceit of task-based models as a bundle of tasks, without considering the structural relationship between tasks \citep{autor2003skill, acemoglu2018automation}.
This is a useful simplification (or perhaps viewed as a question for operations research), but, of course, actual production involves the output of a completed task often being used as input to the next task.

We present asimple task-based model of agentic AI and human labor, but a) is agnostic about the capabilities of AIs now or in the future and b) focuses on the interdependence of tasks in the productive process rather than their content.
As with other task-based models, the focus is on whether labor or capital (here, AI) is assigned to each task.
In the model, the decision to use an AI is based on comparing two costs: the cost of having a human direct and evaluate the AI's output (which depends on the capabilities of the AI at that task) versus the cost of having the human do the task directly.
...
However, it is also possible to create an automation ``chain'' where an AI completing one task passes the output into the next task, without human intervention.
This possibility is supposed to capture the idea of ``agentic AI'' that can complete a sequence of tasks autonomously, albeit with a human providing initial instructions and final evaluation.
This chaining amortizes the cost of the human's supervision over multiple tasks, but with the downside that a larger chain is more likely to have failures and require human supervision or intervention more often.
...
The role humans play in the model as offering judgment is consistent with the emphasis in \cite{agrawal2019exploring}.
...
It can be efficient to use an AI for a task even if humans have a comparative advantage in that task in isolation, if chaining allows for a longer chain of tasks to be fully automated.
A surprising implication of chaining is that improvements in AI for a seemingly unrelated task can cause a task to go directly from being done by human to being done by machine.

In the paper, we explore the $n = 2$ case in detail, which can illustrate the main points of the model, but we also discuss the general $n$-task problem. 
We show that there are $F(2n + 1)$ possible production arrangements, where $F(k)$ is the $k$th Fibonacci number.
We also show how the allocation problem can be solved numerically in $O(n^2)$ time using dynamic programming.

Beyond just allocation, the model provides a way to calculate the return to investing in AI capabilities in specific tasks.
These returns depend on interdependencies between tasks and the state of AI capabilities in other tasks.
For tasks far from economical automation, investments in AI capabilities offer no return because the AI remains inframarginal.
However, once capabilities exceed the threshold where AI replaces humans, further improvements in AI capabilities offer first-order returns.
But beyond the first-oder, investments in the AI doing that task might in turn enable task-chaining.
When that occurs, new investment opportunities emerge in those tasks.
And once tasks are chained, improvements in AI capabilities for one task increase the returns to investing in other tasks in the same chain.

In the $n = 2$ case, we show directly how the time horizon matters in investing in AI capabilities. 
With a short time horizon (or say, a high discount rate), it can be optimal to invest heavily in a single AI capability, leaving the other task to always be done by humans.
But with a long horizon (or low discount rate), the strategy changes. 
As with other optimal growth problems, there's a ``get on the turnpike'' effect: the optimal path is to invest heavily in the lagging task until it too can be automated, after which we have balanced investment across AI tasks.

Another way to think about it is that over short horizons we have task-level substitution, but over long horizons we have system-level substitution.
This feature of the model connects to and potentially reconcile competing views about automation in production. 
...
Whether this increases or decreases the demand for labor even for that task depends on the elasticity of demand for that good in the product market---an often unknown quantity, particualrly for very large changes in costs\footnote{E.g., if an MRI and full expert-level report could be had for \$15, how many more MRIs would be done in the US? This is not a question that can be answered with identification coming from by a 5\% change in medical reimbursement rates.}.
...
When a human performs the task, we denote this as $\human{\cdot}$.
...
Success or failure attempts are assumed to be independent, leading to the following simple Lemma~\ref{lemma:single}.
...
The AI success probabilities are $q_1$ and $q_2$ and $c_h$ and $c_m$ are the same for both tasks.
...
\begin{align}
  C(\mathbf{q}) = \min \left\{2c_h, 2 \frac{c_m}{q_1}, \frac{c_m}{q_1} + c_h, c_h + \frac{c_m}{q_2}, \frac{c_m}{q_1q_2} \right\}
\end{align}

Figure~\ref{fig:two_tasks} shows the cost-minimizing production arrangement for different levels of $q_1$ and $q_2$, with fixed $c_h$ and $c_m$.
It partitions the $q_1$ and $q_2$ space into five regions, corresponding to the five production arrangements.

A few things to note, graphically: the ratio of $\underline{q} := c_m / c_h$ is the threshold for automating a task alone.  
We can see interior to this $\underline{q}$-box, we strictly have human production for both tasks, or $\human{1}\human{2}$.
Investing in $q_1$ and $q_2$ offers no immediate return in the $\human{1}\human{2}$ region.
However, once either task is automated, we gain from increasing AI quaility further, but at a diminishing rate, so long as the other task is still done by humans.
...
The most intestingly-shaped region is $\machine{1|2}$---the cost-minimizing arrangement is to chain the two tasks.
Note that its boundaries with the half-automated regions are sloped, meaning that improvements in one task's AI quality can bring it into the $\machine{1|2}$ region, without an intervening partial automation step.

This curve boundary is the reason for a key implication of the model---namely that chaining can be optimal even when humans have a comparative advantage in one of the tasks in isolation, as formalized in Lemma~\ref{lemma:interdependence}.
...
As one might intuit, because the automation of a focal task depends on the AI capabilities of the other tasks, the returns to investment in AI for a particular task are affected by AI capabilities in other tasks.
Before a task is automated, there is no return to investment in an AI that does that task because it is inframarginal.
...
A natural question is how firms or a social planner should allocate R\&D effort to improve AI capabilities on a task level.
Rather than taking $q_1$ and $q_2$ as given, we can consider the optimal investment paths, or allocation of R\&D effort.
\[
\begin{aligned}
...
I also set the discount rate $\rho = 0$ and the total amount of R\&D resources $r = 1$.
...
We can see that the optimal path genreally has a ``turnpike'' characteristic where we move towards the 45 degree line and then make equal investments in $q_1$ and $q_2$.
However, it does not typicaly go there directly but, if already in a semi-automated region, it will increase capabilities in that task for soem time, taking advantage of the relatively steep slope. 
In the $\machine{1|2}$ region, the optimal path is to directy move towards the 45 degree line.

Figure~\ref{fig:horizon_effects} shows the effect of the time horizon on the optimal path.
As in Figure~\ref{fig:optimal_paths}, a dynamic program is used to find the optimal path that minimizes costs.
However, we vary the number of steps $T$ in the dynamic program.

We can see that this can create fundamental differences in the optimal path.
...
With with a long-enough horizon, we actualy stop work on $q_1$ in begin workign on $q_2$. 
...
\section{Tasks sequences with $n > 2$ tasks}
Now we will consider a job that is a sequence of tasks to be performed $1, 2, \ldots n$.
The number of valid ways to arrange $n$ tasks is given by Lemma~\ref{lem:num_arrangements}.
The number of ways to $n$ tasks is the Fibonacci number $F(2n+1)$ where $F(0) = 1$.
...
\begin{lemma}[Recurrence for Valid Arrangements] \label{lem:num_arrangements}
...
Although the number of possible arrangements grows large, the cost minimization problem is tree traversal.
To see this tree, start with the first task.
...
Figure~\ref{fig:tree} illustrates the tree with $n=3$ tasks.

\begin{figure}
  \caption{The possible production scenarios with an $n=3$ task job}
  \label{fig:tree_extended}
  \begin{tikzpicture}[
    grow=right,
    level 1/.style={sibling distance=20mm},  % dramatically reduced from 100mm
    level 2/.style={sibling distance=20mm},   % dramatically reduced from 45mm
    level 3/.style={sibling distance=5mm},    % reduced from 20mm
    level distance=30mm,                      % kept this to maintain horizontal spacing
    every node/.style={font=\small}
  ]
  \node {Job}
    child {node (AI) {$\langle 1 \rangle$}
      child {node (AIAI) {$\langle 1 \rangle \langle 2 \rangle$}
        child {node {$\langle 1 \rangle \langle 2 \rangle \langle 3 \rangle$}}
        child {node {$\langle 1 \rangle \langle 2|3 \rangle$}}
      }
      child {node (AIAICH) {$\langle 1|2 \rangle$}
        child {node {$\langle 1|2 \rangle \langle 3 \rangle$}}
        child {node {$\langle 1|2|3 \rangle$}}
      }
      child {node (AIAH) {$\langle 1 \rangle (2)$}
        child {node {$\langle 1 \rangle (2) \langle 3 \rangle$}}
        child {node {$\langle 1 \rangle (2) (3)$}}
      }
    }
    child {node (H) {$(1)$}
      child {node (HA) {$(1) \langle 2 \rangle$}
        child {node {$(1) \langle 2 \rangle \langle 3 \rangle$}}
        child {node {$(1) \langle 2 \rangle (3)$}}
      }
      child {node (HH) {$(1) (2)$}
        child {node {$(1) (2) \langle 3 \rangle$}}
        child {node {$(1) (2) (3)$}}
      }
    };
  \end{tikzpicture}
...
With this structure, finding the minimum cost arrangement is a dynamic programing problem.
...
\subsection{Is that what prompting is really like?}
  ...
  It also gives us a single index for the AI's performance, which is a useful simplification.
...
\subsection{Managing an AI is similar to managing people}
...
\subsection{Disappeared occupations often just gave prompting and judging tasks to customers}
...
Asking a model to proofread a memo is not particularly high value;
