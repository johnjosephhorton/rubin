...
However, what prevents this is a hand-off cost: a task that requires no extra human capital but does take time to complete.
This cost can be viewed as the cost of context-switching (or hand-off) between workers in the production process.
Specifically, whenever a job boundary is introduced---i.e., when transitioning from the last task of job $j$ to the first task of job $j+1$---a hand-off cost is incurred by the worker employed to do job $j$.
...
\]
We further assume that tasks in $\mathcal{T}_{k}$ are done in fixed proportions.
For instance, if task $m \in \mathcal{J}_j^k$ takes 30 minutes and task $n \in \mathcal{J}_j^k$ (where $m \neq n$) takes 45 minutes, then producing 1 unit of good $k$ requires a total of 75 minutes spent on $m$ \underline{and} $n$: it is not possible to perform task $m$ multiple times while doing $n$ only once.
Hence, rather than specifying a completion time $\tasktime{}$ for each task separately, it only suffices to determine a single completion time in each job---$\tasktime{j}^k$.\footnote{In fact, we could determine a single completion time parameter for the entire production process; however, for notational simplicity we choose to do so for each job.} % complete footnore for two jobs
...
\]
To ensure that workers recover their training costs and are indifferent between working and not working, the wage for each job must equal the cost associated with the required human capital in a competitive market.
Thus, the wage bill for job $j$ is given by
\[
W_j^k = \Biggl(\sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k} \Biggr) \tasktime{j}^k.
\]
The firm minimizes the total production cost by optimally partitioning the production process $\mathcal{T}_k$ into smaller jobs.
Assuming there are $J_k$ jobs in the optimal job design, the overall production cost per unit of good $k$ is
\[
\text{TotalCost}_k = \sum_{j=1}^{J_k} \Biggl(\sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}\Biggr) \tasktime{j}^k.
\]

\paragraph{Competitive Goods Markets.}

A worker employed in job $j$ produces an intermediate good $j^k$, which is sold in a competitive market to the firm.
The price of intermediate good $j$ is thus
\[
p_j^k = \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}.
\]
Under perfect competition in intermediate goods market, $p_j^k$ also equals the marginal cost $w \tasktime{j}^k$. 
Normalizing the wage to $w=1$ gives
\[
p_j^k \quad = \quad \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k} \quad = \quad \tasktime{j}^k.
\]
Under perfect competition in the final goods market, price of final good $p^k$ equals the sum of price of its constituent intermediate goods:\[
p^k
\quad = \quad \sum_{j=1}^{J_k} \Biggl(\sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k} \Biggr) \quad = \quad \sum_{j=1}^{J_k} p_j^k.
\]
To make the notation in final good market consistent with those in the intermediate good define
\[
\tasktime{}^k \equiv \sum_{j=1}^{J_k} \tasktime{j}^k
\]
so, $p^k = \tasktime{}^k$.
...
\[
\sum_{k=1}^K \Bigg( \sum_{j=1}^{J_k} \tasktime{j}^k \Bigg) x_k = 1.
\]
Since $p_k = \tasktime{}^k$ and $w=1$, this matches the budget constraint 
...
w = 1,
\quad
p_k = l_k = \sum_{j=1}^{J_k} \bigg( \big( \sum_{i \in \mathcal{J}_{jk}} \taskhc{i}^{jk} \big) \big(1 + h_i \big) \bigg),
\quad
...
\]
In this way, the task composition $\mathcal{T}_k$ (and corresponding $c_i^k$s) fixes $p_k$, which then fixes $l_k$, and thus fully determines the consumption bundle $\{x_k^*\}$ and labor allocations $\{L_k^*\}$.

\subsubsection{Comparative Statics with Respect to Automation}

In this task-based model, automation corresponds to reducing $\taskhc{i}^{jk}$ for some tasks (partial automation) or removing them entirely (full automation). 
