...
\centering
\hspace*{3,5cm} Panel (A): Without Hand-off Costs \\[1ex]
\begin{tabular}{|
...
\hline
\textbf{Job Design} & \textbf{Job} & \textbf{Job Tasks} & $v_L+\sum\taskhc{i}$ & $\sum\tasktime{i}$ & \textbf{Job Cost} & \textbf{Total Cost} \\
\hline
\multirow{3}{*}{[1][2][3]} 
  & 1 & $\{1\}$   & 4   & 1    & 4    & \multirow{3}{*}{12.5} \\
\cline{2-6}
  & 2 & $\{2\}$   & 2   & 2    & 4    &  \\
\cline{2-6}
  & 3 & $\{3\}$   & 3   & 1.5  & 4.5    &  \\
...
\begin{minipage}{\textwidth}
\centering
\hspace*{3.5cm} Panel (B): Including Hand-off Costs \\[1ex]
\begin{tabular}{|
  >{\centering\arraybackslash}m{2cm}|
  >{\centering\arraybackslash}m{1cm}|
  >{\centering\arraybackslash}m{2cm}|
...
\hline
\textbf{Job Design} & \textbf{Job} & \textbf{Job Tasks} & $v_L+\sum\taskhc{i}$ & $\tasktime{h} + \sum\tasktime{i}$ & \textbf{Job Cost} & \textbf{Total Cost} \\
\hline
\multirow{3}{*}{[1][2][3]} 
...
\hline
\multirow{2}{*}{[1,2][3]}
  & 1 & $\{1,2\}$ & 5   & 3.5  & 17.5   & \multirow{2}{*}{22} \\
\cline{2-6}
  & 2 & $\{3\}$   & 3   & 1.5  & 4.5    & \\
\hline
\multirow{2}{*}{[1][2,3]}
...
In the next section, we will discuss automation of two or more tasks.

The worker can complete a task in one of two ways: manually or using an AI.  
If performed manually, the worker incurs a time cost of $\tasktime{w} \geq 0$.  
Alternatively, if the worker chooses to use AI, they issue a ``prompt'' to the AI at a time cost of $\tasktime{p} \geq 0$.  
The AI then attempts the task and produces an output, which the worker evaluates at an additional time cost of $\tasktime{e} \geq 0$.  
To capture the overhead associated with supervising and managing the AI’s output, we introduce the management efficiency parameter $\alpha \geq 0$.
In particular, we define the total management time as:
$$\tasktime{m} = \tasktime{p} + \tasktime{e},$$  
so that the effective management cost is given by $\alpha\,\tasktime{m}$.  
The parameter $\alpha$ can be interpreted as either the worker’s management efficiency in working with AI or as the ``AI-ability'' of the task if we assume all workers are equally sophisticated in AI deployment.
A high value of $\alpha$ implies that managing the AI is very costly, whereas a lower value indicates that the AI deployment cost is relatively small (with $\alpha = 0$ meaning that finishing the task is effectively free when using AI).

The AI produces the output with probability $q$ (and fails with probability $1-q$).  
If the AI fails, the worker can provide a new prompt and evaluate a new attempt, incurring the same costs $\tasktime{p}$ and $\tasktime{e}$ again.  
Figure~\ref{fig:flow} illustrates this iterative process.  
...
The decision whether to deploy the AI depends on whether the manual cost exceeds the effective management cost.  
...
$$\underline{q} := \frac{\alpha\,\tasktime{m}}{\tasktime{w}}.$$  
\end{proposition}

\begin{proof}  
By the standard result for the expectation of a geometric distribution, the expected time cost of using the AI is:
$$\tasktime{m} \Biggl( \sum_{\ell=1}^{\infty} \ell\,q\,(1 - q)^{\ell - 1} \Biggr) = \frac{\tasktime{m}}{q}.$$  
Thus, automation is optimal when:
$$\tasktime{w} \geq \frac{\alpha\,\tasktime{m}}{q} \quad \Longleftrightarrow \quad q \geq \frac{\alpha\,\tasktime{m}}{\tasktime{w}} := \underline{q}.$$  
\end{proof}

This criterion has intuitive comparative statics: automation becomes more attractive when the manual time cost $\tasktime{w}$ is high, the effective management cost $\alpha\,\tasktime{m}$ is low, and the AI's success probability $q$ is high.  
One implication is that improvements in prompting and evaluation tools---thereby reducing $\tasktime{p}$ and $\tasktime{e}$---can lower the effective management cost and make automation more appealing, even if $q$ remains unchanged.  
...
For a given management efficiency $\alpha$ and AI success probability $q$, the effective time cost per task---i.e., the time cost parameter $\tasktime{}$ used earlier---is determined by:
$$\tasktime{} = min \Bigl\{\frac{\alpha\,\tasktime{m}}{q},\, \tasktime{w}\Bigr\}.$$  
...
The AI success probabilities are \(q_1\) and \(q_2\), and $\alpha = 1$ and \(\tasktime{w}\) and \(\tasktime{m}\) are the same for both tasks, for simplicity.
With two tasks, there are four possible direct production arrangements:
\begin{align*}
    \cost{\human{1}\human{2}} &= 2\,\tasktime{w} & \text{(fully human)} \\
    \cost{\machine{1}\human{2}} &= \frac{\tasktime{m}}{q_1} + \tasktime{w} & \text{(partial automation)} \\
    \cost{\human{1}\machine{2}} &= \tasktime{w} + \frac{\tasktime{m}}{q_2} & \text{(partial automation)} \\
    \cost{\machine{1}\machine{2}} &= \frac{\tasktime{m}}{q_1} + \frac{\tasktime{m}}{q_2} & \text{(independent automation)}.
\end{align*}
...
\begin{align*}
\cost{\machine{1|2}} &= \frac{\tasktime{m}}{q_1 q_2} & \text{(chained automation)}.
\end{align*}
...
If \(\tasktime{w}\) and \(\tasktime{m}\) are common across tasks, the full cost function for this productive arrangement when costs are minimized is thus: 
\begin{align}
  \min \left\{2\,\tasktime{w},\; 2\,\frac{\tasktime{m}}{q_1},\; \frac{\tasktime{m}}{q_1} + \tasktime{w},\; \tasktime{w} + \frac{\tasktime{m}}{q_2},\; \frac{\tasktime{m}}{q_1 q_2} \right\}.
\end{align}
Note that as each term is linear in \(\tasktime{m}\) and \(\tasktime{w}\), the cost function itself is convex in labor costs. 

Figure~\ref{fig:two_tasks} shows the cost-minimizing production arrangement for different levels of \(q_1\) and \(q_2\), with fixed \(\tasktime{w} = 3\) and \(\tasktime{m} = 1\).
It partitions the \(q_1\) and \(q_2\) space into five regions, corresponding to the five production arrangements.
The ratio 
\[
\underline{q} := \frac{\tasktime{m}}{\tasktime{w}}
\]
...
\end{proposition}
\begin{proof}
Consider \(q_1 = \frac{3}{5}\), \(q_2 = \frac{4}{5}\), \(\tasktime{m} = 1\), and \(\tasktime{w} = \frac{3}{2}\).
For task 1 in isolation, human performance is cheaper: 
\[
\tasktime{w} = \frac{3}{2} < \frac{5}{3} = \frac{\tasktime{m}}{q_1}.
\]
However:
\begin{align*}
    \cost{\human{1}\machine{2}} &= \tasktime{w} + \frac{\tasktime{m}}{q_2} = \frac{3}{2} + \frac{5}{4} = \frac{11}{4}, \\
    \cost{\machine{1|2}} &= \frac{\tasktime{m}}{q_1 q_2} = \frac{25}{12} < \frac{11}{4}.
\end{align*}
...
\begin{proposition}[Comparative Statics] \label{prop:comparative_statics}
  ...
  \[
  \frac{\partial \cost{\machine{1}\machine{2}}}{\partial q_i} = -\frac{\tasktime{m}}{q_i^2}, \quad \frac{\partial^2 \cost{\machine{1}\machine{2}}}{\partial q_1 \partial q_2} = 0.
  \]
  \item In mixed production, only improvements in the specific task matter:
  \[
  \frac{\partial \cost{\machine{1}\human{2}}}{\partial q_1} = -\frac{\tasktime{m}}{q_1^2}, \quad \frac{\partial \cost{\machine{1}\human{2}}}{\partial q_2} = 0.
  \]
  \item In chained production, improvements are complementary:
  \[
  \frac{\partial \cost{\machine{1|2}}}{\partial q_i} = -\frac{\tasktime{m}}{q_i^2 q_j}, \quad \frac{\partial^2 \cost{\machine{1|2}}}{\partial q_1 \partial q_2} = \frac{\tasktime{m}}{q_1^2 q_2^2} > 0,
  \]
  where \(j \neq i\).
  \end{enumerate}
  ...
  \begin{equation}
      \frac{\partial^2}{\partial q_1 \partial q_2} \cost{\machine{1|2}} = \frac{\tasktime{m}}{q_1^2 q_2^2} > 0.
  \end{equation}
...
Consider a scenario with \(n\) tasks, each with the same success probability \(q\) and the same \(\tasktime{m}\) and \(\tasktime{w}\).
To start, all are done by humans, implying that
\[
n\, \tasktime{w} < \frac{\tasktime{m}}{q^n}.
\]
Now suppose we can increase the \(q\) of the first task by \(\delta q\). 
If it is possible that  
\begin{align}
  \delta q > \frac{\tasktime{m}}{n\, q^{n-1}} - q,
\end{align} 
...
Each task \(i \in \{0,\ldots,n-1\}\) can be completed either by a human at cost \(\tasktime{w}[i] \in \mathbb{R}_+\) or attempted by an AI with success probability \(q[i] \in (0,1]\).
Multiple consecutive tasks can be assigned to the AI as a single chain, incurring a fixed management cost \(\tasktime{m} \in \mathbb{R}_+\) per chain. 
When tasks \(i\) through \(j\) are chained, the expected cost is 
\[
\frac{\tasktime{m}}{\prod_{k=i}^j q[k]},
\]
...
V(i) = \min\left\{
\begin{array}{l}
\tasktime{w}[i] + V(i+1), \\[1ex]
\displaystyle\min_{j \geq i} \left\{\frac{\tasktime{m}}{\prod_{k=i}^j q[k]} + V(j+1)\right\}
\end{array}
\right\}
\end{equation}
...
\begin{enumerate}
    \item Assign task \(i\) to human (cost \(\tasktime{w}[i]\)), or
    \item Chain tasks \(i\) through \(j^*\) to AI, where:
    \begin{equation}
        j^*(i) = \arg\min_{j \geq i} \left\{\frac{\tasktime{m}}{\prod_{k=i}^j q[k]} + V(j+1)\right\}.
    \end{equation}
\end{enumerate}
...
\[
\frac{\tasktime{m}}{\prod_{i=1}^n q_i} < \sum_{i=1}^n \frac{\tasktime{m}}{q_i}.
\]
Dividing by \(\tasktime{m} > 0\) and multiplying by \(\prod_{i=1}^n q_i\) yields:
\[
...
If two tasks can proceed in parallel, we can simply model them as having no hand-off costs, of $\tasktime{h} = 0$.
...
However, this does not change the basic structure of the problem much---if we imagine that, say, costs are declining linearly in the number of tasks, say by $\gamma$ each subsequent task, then we would simply have $\tasktime{m} (1-\gamma) / q$ costs instead, with the costs still proportional to $1/q$. 
But in this case, we could just treat $\tasktime{m}$ as being $\tasktime{m} (1-\gamma)$. 
...
Recall that the firm is indifferent to using AI when $\tasktime{m} / \bar{q} = \tasktime{w}$. 
...
the firm's cost-savings will $(\tasktime{w} - \tasktime{m} / q^*) x = C_{q_0}(q^*)$, where $C$ is the one-time cost of improving the technology.
...
Consider that for some occupations, there are necessary tasks such that $\tasktime{w}$ is essentially infinite, or so high that it would be uneconomic for that person to pursue that job because their effective efficiency wage would be lower than their next best alternative.  
...
% Suppose there is some AGI singularity, where all goods cost just $\tasktime{m}$ to produce---the only human labor used is the management of the AI and AI is perfect.
...
However, this is not applicable in our setup because, as soon as task $i$ becomes automated, the corresponding partial derivative changes from 0 to $-\frac{\tasktime{m}}{q_{i}^{2}}$.\footnote{We assume $\tasktime{m}$ and $\tasktime{h}$ are such that some level of automation is preferred over no automation.  
...
Specifically:
\[
\frac{\partial C\{\machine{1|2}\}}{\partial q_{1}}=\frac{\partial C\{\machine{1|2}\}}{\partial q_{2}}\Leftrightarrow-\frac{\tasktime{m}}{q_{1}^{2}q_{2}}=-\frac{\tasktime{m}}{q_{1}q_{2}^{2}}\Leftrightarrow q_{1}=q_{2}.
\]
...
Let $\tilde{T}$ denote the earliest time at which $\frac{\partial C}{\partial q_{1}}=\frac{\partial C}{\partial q_{2}}$ holds.\footnote{We assume that the parameters $\tasktime{m}$ and $\tasktime{h}$ are such that this equality is guaranteed to occur.  
...
\[
\frac{\partial C\{\machine{1|2}\}}{\partial q_{1}}\approx-\frac{\tasktime{m}}{q_{1}^{2}q_{2}}>-\frac{\tasktime{m}}{q_{1}q_{2}^{2}}\approx\frac{\partial C\{\machine{1|2}\}}{\partial q_{2}}.
\]
This simplifies to:
\[
...
\[
q_{1}>q_{2}\Leftrightarrow-\frac{\tasktime{m}}{q_{1}^{2}}>-\frac{\tasktime{m}}{q_{2}^{2}}\Leftrightarrow\frac{\partial C\{\machine{1}\machine{2}\}}{\partial q_{1}}>\frac{\partial C\{\machine{1}\machine{2}\}}{\partial q_{2}}.
\]
...
\[
C\{\machine{1}\machine{2}\}=\frac{\tasktime{m}}{q_{1}}+\frac{\tasktime{m}}{q_{2}}
\]
to 
\[
C\{\machine{1}\human{2}\}=\frac{\tasktime{m}}{q_{1}}+\tasktime{h}.
\]
...
However, on the $\machine{1}\human{2}$ side, the derivatives change to $\frac{\partial C}{\partial q_{1}}=-\frac{\tasktime{m}}{q_{1}^{2}}<0$ and $\frac{\partial C}{\partial q_{2}}=0$.  
...
\begin{equation}
\begin{split}
    & \underbrace{\int_{\hat{T}}^{\tau} e^{-\rho s} \left(\frac{\tasktime{m}}{\hat{q}_{1}} + \tasktime{h}\right) ds}_{\text{from $\hat{T}$ to automating task 2}} 
    + \underbrace{\int_{\tau}^{\tilde{T}} e^{-\rho s} \left(\frac{\tasktime{m}}{\hat{q}_{1}(q_{2,0} + rs)}\right) ds}_{\text{from automation of task 2 to parity of $q$s}} \\
    & \quad + \underbrace{\int_{\tilde{T}}^{\bar{T}} e^{-\rho s} \left(\frac{\tasktime{m}}{\hat{q}_{1}^{2}}\right) ds}_{\text{after parity}} 
    = \underbrace{\int_{\hat{T}}^{\bar{T}} e^{-\rho s} \left(\frac{\tasktime{m}}{\hat{q}_{1} + rs} + \tasktime{h}\right) ds}_{\text{continue investing on $q_{1}$}}.
\end{split}
\label{eq:kink_eq}
\end{equation}
...
whereas:
\[
\frac{\partial C\{\machine{1}\human{2}\}}{\partial q_{1}}=-\frac{\tasktime{m}}{q_{1}^{2}}<0=\frac{\partial C\{\machine{1}\human{2}\}}{\partial q_{2}}.
\]
...
We have:
\begin{align*}
\frac{\partial\hat{T}}{\partial q_{2,0}} & = -\int_{\tau}^{\tilde{T}} e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}(q_{2,0}+rs)^{2}}\right)ds < 0.
\end{align*}
...
\begin{lem}
A sufficient condition for $\frac{\partial\hat{T}}{\partial\rho}<0$ is $\frac{\tasktime{m}}{\tasktime{h}}>Q(\hat{q}_{1})$, where $Q(\hat{q}_{1})=\frac{\hat{q}_{1}^{2}}{1-\hat{q}_{1}}$, the machine-to-human comparative advantage bound, is increasing in $\hat{q}_{1}$.
\end{lem}

\begin{proof}
We take partial derivative of both sides of \eqref{eq:kink_eq} with respect to $\rho$, treating $\hat{T}$ as a variable:
\[
\int_{\hat{T}}^{\tau}e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}}+\tasktime{h}\right)ds+\int_{\tau}^{\tilde{T}}e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}(q_{2,0}+rs)}\right)ds+\int_{\tilde{T}}^{\bar{T}}e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}^{2}}\right)ds=\int_{\hat{T}}^{\bar{T}}e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}+rs}+\tasktime{h}\right)ds.
\]
Differentiating with respect to $\rho$ gives: 
\begin{align*}
-\int_{\hat{T}}^{\tau} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}}+\tasktime{h}\right)ds 
-\int_{\tau}^{\tilde{T}} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}(q_{2,0}+rs)}\right)ds 
-\int_{\tilde{T}}^{\bar{T}} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}^{2}}\right)ds
-\left[\frac{\partial\hat{T}}{\partial\rho}\,e^{-\rho\hat{T}}\left(\frac{\tasktime{m}}{\hat{q}_{1}}+\tasktime{h}\right)\right] \\[1ex]
= -\int_{\hat{T}}^{\bar{T}} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}+rs}+\tasktime{h}\right)ds
-\left[\frac{\partial\hat{T}}{\partial\rho}\,e^{-\rho\hat{T}}\left(\frac{\tasktime{m}}{\hat{q}_{1}+rs}+ \tasktime{h}\right)\right].
\end{align*}
Rearranging terms, we get: 
\begin{align*}
\frac{\partial\hat{T}}{\partial\rho} & =\frac{\int_{\hat{T}}^{\tau} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}}+\tasktime{h}\right)ds+\int_{\tau}^{\tilde{T}} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}(q_{2,0}+rs)}\right)ds+\int_{\tilde{T}}^{\bar{T}} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}^{2}}\right)ds-\int_{\hat{T}}^{\bar{T}} s\,e^{-\rho s}\left(\frac{\tasktime{m}}{\hat{q}_{1}+rs}+\tasktime{h}\right)ds}{e^{-\rho\hat{T}}\left(\frac{\tasktime{m}}{\hat{q}_{1}+rs}-\frac{\tasktime{m}}{\hat{q}_{1}}\right)}
\end{align*}
...
\begin{align*}
 & \underbrace{\int_{\hat{T}}^{\tau} s\,e^{-\rho s} \left(\frac{\tasktime{m}}{\hat{q}_{1}} - \frac{\tasktime{m}}{\hat{q}_{1} + rs}\right) ds}_{>0} 
+ \int_{\tau}^{\tilde{T}} s\,e^{-\rho s} \left(\frac{\tasktime{m}}{\hat{q}_{1}(q_{2,0} + rs)} - \frac{\tasktime{m}}{\hat{q}_{1} + rs} - \tasktime{h}\right) ds \\[1ex]
& + \int_{\tilde{T}}^{\bar{T}} s\,e^{-\rho s} \left(\frac{\tasktime{m}}{\hat{q}_{1}^{2}} - \frac{\tasktime{m}}{\hat{q}_{1} + rs} - \tasktime{h}\right) ds.
\end{align*}
Now we look for sufficient conditions that guarantee the numerator is positive.  
First, notice that $\left(-\frac{\tasktime{m}}{\hat{q}_{1}+rs}-\tasktime{h}\right)$ appears in both the middle and right terms.  
To ensure the numerator is positive, we require:
\[
\frac{\tasktime{m}}{\hat{q}_{1}+rs}+\tasktime{h}\leq \min\left\{\frac{\tasktime{m}}{\hat{q}_{1}^{2}},\frac{\tasktime{m}}{\hat{q}_{1}(q_{2,0}+rs)}\right\}.
\]
In the region where $q_{1}>q_{2}$ and $q_{2}+rs\leq\hat{q}_{1}$ until parity is achieved, the minimum is $\frac{\tasktime{m}}{\hat{q}_{1}^{2}}$.  
Therefore, the condition becomes: 
\begin{align*}
\frac{\tasktime{m}}{\hat{q}_{1}+rs}+\tasktime{h} & <\frac{\tasktime{m}}{\hat{q}_{1}^{2}}.
\end{align*}
At $t=0$ (just after the investment switch), this simplifies to:
\[
\frac{\tasktime{m}}{\hat{q}_{1}}+\tasktime{h}<\frac{\tasktime{m}}{\hat{q}_{1}^{2}},
\]
or equivalently:
\begin{align*}
\frac{\tasktime{m}}{\tasktime{h}}\left(\frac{1}{\hat{q}_{1}^{2}}-\frac{1}{\hat{q}_{1}}\right) & >1,\\[1ex]
\frac{\tasktime{m}}{\tasktime{h}}\frac{1-\hat{q}_{1}}{\hat{q}_{1}^{2}} & >1,\\[1ex]
\frac{\tasktime{m}}{\tasktime{h}} & >\frac{\hat{q}_{1}^{2}}{1-\hat{q}_{1}},\\[1ex]
\frac{\tasktime{m}}{\tasktime{h}} & >Q(\hat{q}_{1}).
\end{align*}
Thus, $\frac{\tasktime{m}}{\tasktime{h}}>Q(\hat{q}_{1})$ is a sufficient condition to ensure $\frac{\partial\hat{T}}{\partial\rho}<0$.  
...
\]
This implies that as $q_{2,0}$ increases, maintaining $\frac{\partial\hat{T}}{\partial\rho}<0$ requires less comparative machine cost advantage (i.e., lower $\frac{\tasktime{m}}{\tasktime{h}}$).
\end{proof}
...
Then:
\[
\text{CostBlock}(i\text{ through }j) = \frac{\tasktime{m}}{\mathbb{P}(\text{all succeed})},
\]
...
Let the production process be represented by a DAG $G = (V, E)$, where each node $i \in V$ is a task requiring cost $\tasktime{h}[i] \in \mathbb{R}_+$ by humans or $\frac{\tasktime{m}}{\prod_{k \in \text{chain}} q[k]}$ if executed in a chained block of tasks.  
...
Let a firm allocate resources to improve AI success probabilities $q_i$ and reduce management costs $\tasktime{m}$ over time, facing cost functions $R(q_i)$ and $S(\tasktime{m})$.  
The total cost per period is:
\[
C(\mathbf{q}, \tasktime{m}) = \min\left\{\text{production costs with } q_i \text{ and } \tasktime{m}\right\}.
\]
If improvements in $q_i$ and $\tasktime{m}$ are complementary (i.e., $\frac{\partial^2 C}{\partial q_i \partial \tasktime{m}} < 0$), the firm will invest in both dimensions simultaneously, rather than prioritizing one exclusively.
\end{proposition}

\subsection{Multiple Humans with Heterogeneous Skills}

\begin{proposition}[Heterogeneous Human Costs]
Suppose there are two human types, $H_1$ and $H_2$, with costs $\tasktime{h1} < \tasktime{h2}$ and management costs $\tasktime{m1} < \tasktime{m2}$.  
...
\end{itemize}
The segmentation of tasks depends on $\frac{\tasktime{m1}}{q_1q_2\cdots q_k}$ versus $\frac{\tasktime{m2}}{q_1q_2\cdots q_k}$, which affects allocation efficiency.
\end{proposition}
...
\[
\sum_{\ell=1}^m \frac{\tasktime{m}}{\prod_{i \in \ell\text{-th subchain}} q[i]}.
\]
