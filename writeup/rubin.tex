...
\]
This assumption implies that, in equilibrium, the amount of good $k$ produced in the economy is the same as the amount of each intermediate good $x_j^k$ used in production of the final good.
...
To recoup their training costs and to make them indifferent between working and not working, worker compensation must account for human capital costs, i.e.,
\[
w_j^k = v_L + \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}.
\]
The firm must pay workers for their time, regardless of the task; hence, the wage bill paid to worker per unit of good $x_j^k$ is:
\[
...
Let $\labor{j}^k$ denote the amount of labor per unit of good $x_j^k$ employed in job $j$ in the production process of $k$.  
That is:  
\[
\labor{j}^k = \tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}.
\]  
The production cost per unit of $x_j^k$ can thus be written as:  
\[
\text{WageBill}_j^k = \Biggl(v_L + \sum_{i \in \mathcal{J}_j^k} \taskhc{ij}^{k}\Biggr) \Biggl(\tasktime{hj}^k + \sum_{i \in \mathcal{J}_j^k} \tasktime{ij}^{k}\Biggr) = w_j^k \,\labor{j}^k,
\]  
where $w_j^k$ is the wage rate for job $j$ in the production of good $k$.  
Since markets are perfectly competitive, the price of intermediate good $j$, $p_j^k$, equals the marginal cost $w_j^k\,\labor{j}^k$:  
\[
p_j^k = w_j^k \labor{j}^k.
\]  
Furthermore, given the Leontief production function, perfect competition in the final goods market implies that the price of the final good $p^k$ equals the sum of the prices of its constituent intermediate goods:  
\[
p^k = \sum_{j=1}^{J_k} p_j^k = \sum_{j=1}^{J_k} w_j^k \labor{j}^k.
\]  
To ensure consistency in notation between the final and intermediate goods markets, define  
\[
w^k \labor{}^k \equiv \sum_{j=1}^{J_k} w_j^k \labor{j}^k,
\]
so that $p^k = w^k \labor{}^k$.  
Let $\labor{}^k$ denote the total amount of time (or labor) spent producing good $k$, that is,  
\[
\labor{}^k = \sum_{j=1}^{J_k} \labor{j}^k.
\]
Thus, $w^k$ can be interpreted as the time-weighted average wage rate paid to workers in the production process of $k$, namely,  
\[
w^k = \frac{\sum_{j=1}^{J_k} w_j^k \labor{j}^k}{\sum_{j=1}^{J_k} \labor{j}^k}.
\]
Here we must note that, due to the assumption that tasks are done in fixed proportions, once $\labor{}^k$ is determined all subsequent $\labor{j}^k$ for $j \in \{1,\ldots,J_k\}$ are pinned down.  
...
This in turn gives:  
\[
\labor{}^k = \sum_{j=1}^{J_k} \labor{1}^k \Bigl(\frac{\labor{j}^k}{\labor{1}^k}\Bigr) = \labor{1}^k \sum_{j=1}^{J_k} \gamma_j 
\quad 
\Longrightarrow
\quad 
\frac{\labor{}^k}{\labor{1}^k} = \sum_{j=1}^{J_k} \gamma_j,
\]  
and thus:  
\[
\labor{j}^k = \Bigg( \frac{\gamma_j}{\sum_{j=1}^{J_k} \gamma_j} \Bigg) \labor{}^k.
\]  
Therefore, it is without loss of generality to focus our attention on working with $\labor{}^k$, because once that is determined, the labor for each subsequent job is specified.
...
\]  
where $I$ is the total labor income earned by supplying labor to different jobs across all sectors.  
Note that when choosing how much of each good to consume, the household takes income as given while it is in fact determined by earnings from producing intermediate goods:  
\[
...
\]
and $L_k^* = \labor{}^k\,x_k^*$ is the total labor employed in sector $k$.
In this way, the job design of the production process $\mathcal{T}_k$ (and the corresponding $\taskhc{ij}^k$s and $\tasktime{ij}^k$s) determines $p^k$, which then fixes $\labor{}^k$, thereby fully determining the consumption bundle $\{x_k^*\}$ and the labor allocations $\{L_k^*\}$.

Appendix~\ref{app:GE} gives the step by step derivation of equilibrium objects. 
Furthermore, in Appendix~\ref{app:GE_comp_stats} we show comparative statics with respect to task automation.
