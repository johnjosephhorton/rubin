...
To recoup their training costs and make zero profit, $w = \sum_{i \in J} c_i$.
...
\begin{figure}[htbp]
  \centering
  \caption{Allocation of workers to tasks when $n = 6$} \label{fig:job_design}
  \includegraphics[width=0.8\textwidth]{plots/job_design.png}
  \footnotesize{Notes: This figure shows a potential job design for a sequence of 6 tasks.
  The total cost of the job is the area of the bounding box, plus the area of the switching cost rectangles.
  The optimal job design is to assign workers to tasks 1, 2, and 3, with a handoff cost at task 4.}
\end{figure}
...
What is \emph{particularly} costly to the firm are tall and narrow tasks right next to short and wide tasks that have high handoff cost boundaries: when the productive process requires lots of work nearly anyone could do next to high-skilled work that only a few can do, but cannot be easily broken up.
For example, professors at business schools are quite highly paid to teach and do reearch, but (the good ones at least) are also paid to clean blackboards. 
...
\subsection{A model of production with AI} \label{sec:model}
