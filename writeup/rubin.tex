...
Writing might appear next to hundreds of different tasks, from ``send your economics paper to a journal'', to ``drop your maintenance report slip into a box'' at the end of a shift driving a bus.
...
%%%%%%%%%%%% Tent Poll Tasks %%%%%%%%%%%%
Imagine two tasks where one takes a lot of time to complete but requires little human capital appears next to a task that takes little time to complete but requires a lot of human capital. 
An example of such combination of tasks is ``business school professor teaching on the board.''
The task of ``teaching'' in this example is one that requires a lot of human capital and requires moderate time to complete.
The ``cleaning of the board'', on the other hand, takes a lot of time to complete without requiring much, if any, human capital.
What determines 

An economic force that pushes us towards specialization is ``hand-off'' or ``switching'' costs. 
This friction leads us to bundle tasks together in a 
Internalized hand-off costs push us towards minimum costs.
Chaining is hard for tent poll tasks. 
%%%%%%%%%%%% Tent Poll Tasks %%%%%%%%%%%%
...
To model how AI affects occupations, here viewed as collections of tasks, we need some account of why those tasks were put together in an occupation in the first place.
...
The firm hires one worker for each job.  
The worker then completes all of the constituent tasks of their assigned job. 
To perform each task $i \in \mathcal{J}_j$, the worker must first acquire the necessary skills to do the task, incurring a one-time human capital cost of $\hccost{ij} > 0$.  
The task itself takes $\timecost{ij} > 0$ time to perform, and this time cost is incurred for each unit of good produced.  
Note that the human capital cost is a fixed investment---a stock of knowledge acquired just once---and is entirely independent of the time required to perform the task.
...
\]
while the total time spent on tasks in $\mathcal{J}_j$ to produce a unit of output is:
\[
...
\end{align}
The firm must pay workers for their time, regardless of the task; hence, the wage bill paid to worker per unit of output is:
\begin{align}
\label{eq:wagebill}
...
Equation \ref{eq:wagebill} has a nice geometric representation as well.
The productive process can be represented as a sequence of stacked rectangles of height $\hccost{ij}$ and width $\timecost{ij}$, as shown in Figure~\ref{fig:wage_bill}.
A job corresponds to the bounding box around a sequence of tasks with its wage bill given by the area of that box.
The wage bill of each job can be thought of as the area of a rectangle whose height is the required human capital to complete the job and whose width is the time cost of the job.
Figure~\ref{fig:wage_bill} gives the graphic representation of an occupation with $\tau=2$ tasks and thus two potential job designs: [1][2] and [1,2].
The numbered rectangles bounded by solid lines in each panel of the figure represent the two tasks.
In the [1][2] design (left panel), each task is assigned to a separate worker.
The wage bill of job 1 in the [1][2] design is the shaded area bounded by rectangle 1 and the wage bill of job 2 is the shaded area bounded by rectangle 2.
In the [1,2] design where both tasks are assigned to the same worker (right panel), the wage bill of the only job in the occupation is the area of the shaded rectangle whose width is the sum of time costs of both tasks and whose height is the sum of human capital of tasks.

\begin{figure}
  \caption{Geometric Representation of the Wage Bill} \label{fig:wage_bill}
  \begin{center}
  \includegraphics[width = \textwidth]{plots/job_design.png} \\
  \end{center}
\begin{footnotesize}
  \emph{Note:} This graph depicts the geometric representation of the wage bill for an occupation with $\tau=2$ tasks. 
  The shaded areas are the wage bills of jobs in each panel. 
  In the left panel, the shaded rectangles give the wage bill for jobs [1] and [2]. 
  In the right panel, the shaded area is the wage bill of the job comprised of both tasks [1,2].
\end{footnotesize}
\end{figure}
%%%%%%%%%%%%%%%%% Single Graph Representation %%%%%%%%%%%%%%%%%
...
Assuming there are $J$ jobs in the optimal allocation of workers to jobs, the overall production cost per unit of the good is:
\begin{align}
...
The minimum cost way of doing this occupation is to assign each task to a single worker and have each individual specialize in completing one task. 
In fact, the same conclusion holds for any arbitrary set of parameter values as for any nonnegative $x,y,w,z$ the following holds: $xy+wz \leq (x+w)(y+z)$.
Another way to put this is to say for any number of tasks the sum of individual rectangle areas in the geometric representation is always strictly smaller than any job design in which two tasks or more are bundled together and are assigned to one worker.
...
\subsection{Why Not Full Division of Labor?}
Without additional frictions, the firm could minimize production costs by assigning one worker to each task.
To assume away this trivial arrangement we introduce the ``hand-off cost”---a cost associated with job transitions that, while not requiring additional human capital, does take time.
This cost can be interpreted as the expense of context switching between workers in the production process.
Specifically, this cost is incurred whenever a job boundary is introduced. That is, when transitioning from the last task of job $j$ to the first task of job $j+1$, the worker in job $j$ incurs a hand-off time cost of $\handofftime{j} \geq 0$.
The hand-off time is an intrinsic characteristic of the task and is only realized when the task falls at the boundary of two consecutive jobs.
...
The firm's cost minimization problem can be interpreted geometrically as well.
In Figure~\ref{fig:job_design} we represent the productive process of the example discussed in Table~\ref{tab:job_design} as a sequence of stacked rectangles of height $\hccost{ij}$ and width $\timecost{ij}$.
\begin{figure}[htbp]
  \begin{center}
...
\end{figure}
Panel (a) of the figure shows the job design problem for the case without hand-off costs (corresponding to Panel A of Table~\ref{tab:job_design}).
Panel (b) of the figure shows the same example including hand-off costs (corresponding to Panel B of Table~\ref{tab:job_design}).
Visually, the firm's objective is to strike an optimal balance between two competing costs.
On one hand, consolidating tasks into a single job can lead to inefficiencies illustrated by the cross-diagonal rectangles, arising from bundling tasks together.
On the other hand, breaking production into multiple jobs incurs an additional hand-off time cost at the transition boundary between jobs.
The extent to which the extra hand-off time cost increases total cost depends on the cumulative human capital of the worker who is doing the hand-off.
That is, the height of the pink hand-off cost rectangle(s) shown in Panel (b) of Figure~\ref{fig:job_design} are determined by the sum of heights of tasks comprising that job.
The firm trades off these two factors against each other to determine the most cost-effective job design.
As discussed earlier, the cost minimizing plan in absence of hand-off costs is [1][2][3] (top left graph of Panel (a)) whereas after inclusion of hand-offs the optimal plan changes to [1,2][3] (bottom left graph of Panel (b)).
...
\subsection{AI Augmentation of Tasks}
...
A task can be done in one of three ways: 1) manually by a human in its entirety, 2) using an AI with human oversight, or 3) using an AI in its entirety.
We refer to the second case as task ``augmentation'' and the third case as task ``automation.''
We defer discussion of automation to the next section as it only happens when two or more tasks are present.

In the remainder of this section, a task is done either fully manually or by AI in an augmented way.
If the task is completed manually by human, the worker incurs a time cost of $\humantime > 0$.
Alternatively, if the worker opts to use AI in an augmented way, they issue a ``prompt'' to the AI at a time cost of $\timecost{p} > 0$.
...
After prompting the worker evaluates the AI's output, incurring an additional time cost of $\timecost{e} > 0$.
After evaluation, the worker learns whether the AI succeeded or failed.
...
Figure~\ref{fig:flow} below illustrates this process graphically.
...
\end{tikzpicture}}
   \end{center}
  \footnotesize{
  \emph{Notes:} This figure illustrates the iterative process of using AI to complete a task.  
...
This criterion has intuitive comparative statics: automation becomes more attractive when the manual time cost $\humantime$ is high, the management cost $\machinetime$ is low, and the AI's success probability $q$ is high (or equivalently, the completion difficulty $d$ is low).
...
For a given AI success probability $q$, the effective time cost per task (the task time cost parameter $\timecost{i}$ used in previous section) is determined by:
$$\timecost{i} = \min \Bigl\{\frac{\machinetime}{q},\, \humantime\Bigr\}.$$
This expression shows how the overall time cost of a task is set by the trade-off between manual execution and AI-assisted completion in an augmented way.
...
First, let us formalize the definition of augmented and automated tasks.
We say that a task is ``augmented'' if it is the last task in a sequence of consecutive tasks that are completed using AI.
We refer to a task as ``automated'' if it belongs to a sequence of two or more consecutive tasks done by AI and the task is not the last task in the sequence.
Note that a task facilitated by AI in a sequence of length 1 (i.e., a single task in isolation) is necessarily an augmented task.
The reason we introduce these definitions is to make a distinction between tasks that are done in their entirety by AI without requiring direct human intervention (automated) and tasks whose outputs are overseen by a human before being passed on (augmented).
...
Furthermore, we suppress the switching cost $\handofftime{}$ which the worker incurs upon completing task $2$, since our focus is solely on tasks performed within a single job (and note that no hand-offs occur between tasks of the same job).
...
\end{align*}
The following tasks in above arrangements are augmented: task 1 in $\machine{1}\human{2}$, task 2 in $\human{1}\machine{2}$, and both tasks 1 and 2 in $\machine{1}\machine{2}$.
...
We call this an ``automation chain.''
...
Since $\humantime$ and $\machinetime$ are common to all tasks, the complete cost function for this productive arrangement, when minimizing costs, is given by:
\begin{align}
...
A task may be optimally automated as part of a chain even if humans have a comparative advantage when performing that task in isolation.
In particular,
\[
...
\section{AI Advancements and Job Re-design}
Once a firm has a job design, if there is an advance in AI capabilities, how does it re-design the job? 

There are two ways an AI advance can affect individual tasks.
It can lower the time it takes to finish the job and/or change the amount of human capital required to complete the task (for now we assume it decreases, i.e., workers need fewer skills to work with the AI.).
AI advances will not only change the firm's cost minimization problem directly by affecting tasks, but also indirectly enters the problem by changing the effective hand-off cost it incurs for switching between jobs, as hand-offs effectively enter the cost minimization through the human capital amount of the worker employed in that job.

Depending on the time horizon, the problem can have different answers.
In the long-run, workers adjust the amount of human capital they acquire for doing each task and everything is re-calibrated.
The firm faces a similar cost minimization problem as before the shock but with modified parameter values.

In the short-run, however, things are a bit different. 
Following an AI advance a part of worker's human capital is deprecated.
The firm is only willing to compensate the worker for the portion of human capital they exploit to complete the tasks assigned to them in the optimal job design.
So the question becomes: how do workers and firms respond?
...
Suppose the firm breaks down the production process $\mathcal{T}_k$ into $J^k$ jobs, each of which consist of a number of consecutive tasks.
Let $\mathcal{J}_j^k$ denote the collection of tasks in job $j \in \{1, 2, \ldots, J^k\}$ in the production process of final good $k$.
With this notation, the four potential job designs in the earlier example with $\tau_k=3$ tasks can formally be expressed as:
\begin{align*}
...
This is merely a technical assumption reflecting the fact that to produce one unit of the final good each set of tasks in $\mathcal{T}_k$, across all $J^k$ jobs in that sector, must be performed exactly once.
