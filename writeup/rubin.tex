...
A surprising implication of chaining is that improvements in AI capabilities for a seemingly unrelated task can cause a focal task to go directly from being done by human to being done by machines without the focal task experiencing direct improvements by the AI.
...
This is evident in Panel A of Table~\ref{tab:job_design}.
We assume this trivial arrangement is prevented by a ``hand-off cost”---a cost associated with job transitions that, while not requiring additional human capital, does take time.
This cost can be interpreted as the expense of context switching between workers in the production process.
Specifically, whenever a job boundary is introduced---that is, when transitioning from the last task of job $j$ to the first task of job $j+1$, the worker in job $j$ incurs a hand-off time cost of $\handofftime{j} \geq 0$.
...
The firm's objective is to minimize the following expression, which differs from the previous equation only by the extra $\handofftime{j}$ term added to the time cost for each job:
\begin{align}
...
\end{array}
\]
Notice that while the earlier optimal job design assigned a single worker to each task (i.e., the [1][2][3] design), after introduction of the hand-off friction, the cost-minimizing production method becomes the [1,2][3] design.
In the new optimal design, the firm minimizes its cost by bundling tasks 1 and 2 together, avoiding the substantial hand-off cost incurred between these tasks.
