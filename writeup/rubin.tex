...
If the task is completed manually, the worker incurs a time cost of $\tasktime{w} > 0$.
Alternatively, if the worker opts to use AI, they issue a ``prompt'' to the AI at a time cost of $\tasktime{p} > 0$.
The AI successfully produces an output with probability $q$ and fails with probability $1-q$.
The worker then evaluates the output, incurring an additional time cost of $\tasktime{e} > 0$.
...
Let $\tasktime{m} = \tasktime{p} + \tasktime{e}$ denote the total ``management'' time cost associated with using the AI for the task.
...
$$\tasktime{w} \geq \frac{\tasktime{m}}{q} \quad \Longleftrightarrow \quad q \geq \frac{\tasktime{m}}{\tasktime{w}} := \underline{q}.$$
\end{proof}

This criterion has intuitive comparative statics: automation becomes more attractive when the manual time cost $\tasktime{w}$ is high, the management cost $\tasktime{m}$ is low, and the AI's success probability $q$ is high.
One implication is that improvements in prompting and evaluation tools---thereby reducing $\tasktime{p}$ and $\tasktime{e}$---can lower the management cost and make automation more appealing, even if $q$ remains unchanged.
...
For a given AI success probability $q$, the effective time cost per task---i.e., the task time cost parameter $\tasktime{i}$ used earlier---is determined by:
$$\tasktime{i} = \min \Bigl\{\frac{\tasktime{m}}{q},\, \tasktime{w}\Bigr\}.$$
