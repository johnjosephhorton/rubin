...
Let $\mathcal{J}^{(0)}$ be the current optimal partition and $\mathcal{J}^*$ its reoptimized counterpart under the improved AI quality.
The firm will switch from $\mathcal{J}^{(0)}$ to $\mathcal{J}^*$ in the short run if and only if the pure efficiency gains from job redesign exceeds the redesign costs:
\[
...
\]
Moreover, there exists a unique threshold $\tilde{\alpha}$ such that for all $\alpha>{\tilde\alpha}$ the inequality strictly holds and the firm strictly prefers to redesign jobs.
\end{proposition}
See Appendix~\ref{app:shortrun_redesign_proof} for the proof.
Proposition~\ref{prop:shortrun_redesign} states that so long as gains from redesigning jobs is larger than its costs the firm will reallocate tasks to workers even in the short run.
Moreover, the existence of a unique AI‐quality threshold implies a tipping‐point effect: although AI improvements yield cost savings for increases in the AI quality, it is not worthwhile to redesign jobs in the short run until quality exceeds a certain threshold, at which point restructuring becomes optimal. 
