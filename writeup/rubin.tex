...
\pscomment{JJH's recommendation on immediately bringing the example table and graphs here?}
...
\pscomment{Is it worthwhile to expand briefly on how heuristics could improve upon the $O(n^2)$ complexity bound? Consider a task that has a significantly large hand-off cost, yet moderate skill and time costs relative to its neighboring tasks. If clear cost inequalities hold---for example, if automating this task in a chain with its immediate successor is consistently cheaper than performing it manually or through augmentation---then this task can be directly automated without explicitly evaluating all alternatives.

Or, if manual execution of a certain task consistently proves cheaper than any alternative, the original optimization problem could effectively split into smaller subproblems around that particular task (Of course, we should still carefully account for job boundaries and associated hand-off costs in such cases.).

Exploiting insights like these might substantially improve the computation beyond the stated bound. But, I can't think of a structured way of incorporating these into the dynamic programming solution, aside from possibly brute-force evaluating different cost inequalities for each task before executing the DP algorithm.
}
...
\pscomment{The following lines implicitly refer to and use the the equalities about derivatives of costs w.r.t. success probabilities (and/or AI quality). Revise a bit or bring the formula here?}
...
\pscomment{To Do: fix the language of job vs. occupation in the proposition (and another proposition earlier?).}
...
\pscomment{I think it's worthwhile to explicitly distinguish between different workers in the notation here. Doing so clarifies that the dynamic programming (DP) approach not only calculates the optimal cost but also determines the job boundaries. If maintaining an additional worker argument $j$ in the $W(.)$ notation complicates things, we should at least clearly mention this distinction in the text---once here and again when explaining Option 1 below.

PS to do: also, remove legacy text that mentions active worker (or just worker) $j$.}
...
\begin{itemize}
    ...
    \item \textbf{Option (3):} The term $\min_{r < i} \ W\left(r,\; \hccost{} + \machinehc{i},\; \timecost{} + \frac{\machinetime{i}}{\prod_{s=r+1}^{i} q_s}\right)$ also corresponds to adding tasks to the active worker's job, but by having them perform task $i$ in an AI-augmented manner. 
...
\pscomment{Two comments re. the calculations above: 

1) Shouldn't the number of powers of $(1+\epsilon)$ in the ranges be inversely proportional to $\epsilon$ (for smaller $\epsilon$s more bits and thus a larger table is required)? The number of powers of $(1+\epsilon)$ in $[a,b]$ can be found via:
\[
a \times (1+\epsilon)^x = b \Longrightarrow x = \frac{\log(\frac{b}{a})}{\log(1 + \epsilon)} \approx \frac{\log(\frac{b}{a})}{\epsilon}.
\]
So, for skill costs in range $[1/M, nM]$ we get $O(\epsilon^{-1} \log(nM^2))$ and for time costs in range $[1/M, 2nM^3]$ we get $O(\epsilon^{-1} \log(nM^4))$.

2) Since $M$ is potentially much larger than $n$ shouldn't we keep its exponents in the $O(.)$ notation? That is, we should \underline{not} simplify $O(\epsilon^{-1} \log(nM^2))$ and $O(\epsilon^{-1} \log(nM^4))$ to $O(\epsilon^{-1} \log(nM))$. So, the overall time complexity of the table will be $O(n \epsilon^{-2} \log^2(nM^4))$ and thus total runtime will be $O(n^2 \epsilon^{-2} \log^2(nM^4))$}
