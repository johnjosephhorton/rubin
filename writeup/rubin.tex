...
Workers value leisure at $1$ and are perfectly inelastic in their labor supply.
To recoup their training costs and be indifferent between working and not working, $w = 1 + \sum_{i \in J} c_i$.
The firm faces a perfectly inelastic labor supply at any job it designs.
...
\subsection{General Equilibrium with Task-Based Production}
...
\sigma > 0.
\]
Here, goods are substitutes if $\sigma > 1$, complements if $0 < \sigma < 1$, and $\sigma = 1$ gives the Cobb-Douglas case. 
The consumer faces a budget constraint 
\[
\sum_{k=1}^K p_k \,x_k = w.
\]

A single (representative) firm produces all $K$ goods under perfect competition, using labor as the only input. 
Each good $k \in \{1,\ldots,K\}$ is produced by completing a specific set of tasks $\mathcal{T}_k$. 
A worker employed in task $i \in \mathcal{T}_k$ must acquire human capital $c_i^k > 0$. 
We assume tasks in $\mathcal{T}_k$ must be done in fixed proportions to yield one unit of good $k$. 
For instance, if task $i \in \mathcal{T}_k$ takes 30 minutes and task $j \in \mathcal{T}_k$ (where $j \neq i$) takes 45 minutes, then producing 1 unit of good $k$ requires a total of 75 minutes.
It is not possible to perform task $i$ multiple times while doing $j$ only once: the tasks have a fixed ratio. 
Hence, rather than specifying labor for each task separately, we let $l_k$ be the total labor required for one unit of good $k$, aggregating over all tasks.

We further assume the firm fully compensates workers for the human capital they acquire. 
Thus, the price of good $k$ is
\[
p_{k} = 1 + \sum_{i \in \mathcal{T}_k} c_{i}^k.
\]
Under perfect competition, $p_k$ also equals the marginal cost $w \,l_k$. 
Normalizing the wage to $w=1$ gives
\[
p_{k} \;=\; l_{k} \;=\; 1 \;+\; \sum_{i \in \mathcal{T}_k} c_i^k.
\]

\subsubsection{Labor Market Constraint}

Because the consumer inelastically supplies one unit of labor, the labor-market clearing condition is
\[
\sum_{k=1}^K l_k \,x_k \;=\; 1.
\]
Since $p_k = l_k$ and $w=1$, this matches the budget constraint 
\(\sum_{k=1}^K p_k\,x_k = 1\).

\subsubsection{Solving for $\{x_k\}$}
...
\forall i,j.
\]
From CES properties, we get
\[
\left(\frac{x_i}{x_j}\right)^{-\tfrac{1}{\sigma}}
=\;
\frac{l_i}{l_j}\,\cdot\,\frac{\delta_j}{\delta_i},
\quad
\Longrightarrow
\quad
\frac{x_i}{x_j}
=
\left(\frac{\delta_i}{\delta_j}\,\cdot\,\frac{\,l_j\,}{\,l_i\,}\right)^{\sigma}.
\]
Define
\[
R_j
...
\frac{x_j}{x_1}
=
\Bigl(\frac{\,l_1\,}{\,l_j\,} \cdot \frac{\delta_j}{\delta_1}\Bigr)^{\sigma},
\]
where $R_1 = 1$. 
Substitute $x_j = R_j \, x_1$ into $\sum_{k=1}^K l_k x_k = 1$:

\[
\sum_{k=1}^K l_k\,\bigl(R_k\,x_1\bigr)
=
x_1\Bigl[\sum_{k=1}^K l_k R_k\Bigr]
=
1,
\]
giving
...
x_1^*
=
\frac{1}{\sum_{k=1}^K l_k\,R_k\,},
\quad
x_j^*
=
R_j\,x_1^*
=
\frac{\,R_j\,}{\,\sum_{k=1}^K l_k\,R_k\,}.
\]

\subsubsection{Sectoral Employment and Equilibrium}

Labor used in good $k$ is $L_k^* = l_k\,x_k^*$. 
Thus the equilibrium can be summarized as
\[
w = 1,
\quad
p_k = l_k = 1 + \sum_{i\in \mathcal{T}_k}c_i^k,
\quad
x_k^* = \frac{R_k}{\sum_{\ell=1}^K l_\ell\,R_\ell},
\quad
L_k^* = l_k\,x_k^*,
\]
where
...
R_k
=
\Bigl(\frac{l_1}{l_k}\,\cdot\,\frac{\delta_k}{\delta_1}\Bigr)^{\sigma}.
\]
In this way, the task composition $\mathcal{T}_k$ (and corresponding $c_i^k$s) fixes $l_k$, which then fixes $p_k$, and thus fully determines the consumption bundle $\{x_k^*\}$ and labor allocations $\{L_k^*\}$.

\subsubsection{Comparative Statics with Respect to Automation}

In this task-based model, automation corresponds to reducing $c_i^k$ for some tasks (partial automation) or removing them entirely (full automation). 
Suppose $\mathcal{A}_1 \subseteq \mathcal{T}_1$ is automated in good~1’s production, reducing $\sum_{i\in\mathcal{T}_1}c_i^1$ and thus lowering $l_1$ and $p_1$. 
The following comparative statics apply:

\paragraph{Price Effects.}
Only $p_1^* = l_1^*$ falls; prices $p_j^*$ for $j\neq 1$ remain unaffected (unless their own tasks change).

\paragraph{Effects on Consumption.}
We find
\[
\frac{\partial x_{1}^*}{\partial l_{1}}
=
-\,
\frac{\,l_{1} \;+\;\sigma\,\bigl(\sum_{k\neq 1}l_{k}R_{k}\bigr)\,}{
       l_{1}\,\Bigl(\sum_{k}l_{k}R_{k}\Bigr)^{2}}
<0,
\qquad
\frac{\partial x_{j}^*}{\partial l_{1}}
=
(\sigma-1)
\,\frac{\,R_{j}\,}{\bigl(\sum_{k}l_{k}R_{k}\bigr)^{2}}
\quad(\text{for }j\neq 1).
\]
Hence, when $l_{1}$ decreases (automation makes good 1 cheaper), $x_{1}^*$ unambiguously rises, while $x_{j}^*$ moves in the opposite direction if $\sigma>1$ (substitutes) or in the same direction if $0<\sigma<1$ (complements).

\paragraph{Effects on Sectoral Labor Shares.}
Similarly, 
\[
\frac{\partial L_{1}^*}{\partial l_{1}}
=
x_{1}^*\,(1-\sigma)\bigl(1 - l_{1}x_{1}^*\bigr),
\quad
\frac{\partial L_{j}^*}{\partial l_{1}}
=
(\sigma-1)\,
\frac{\,l_{j}R_{j}\,}{\bigl(\sum_{k}l_{k}R_{k}\bigr)^{2}}
\quad (j\neq 1).
\]
If $\sigma>1$, a reduction in $l_{1}$ shifts labor towards good~1 and away from the other goods (since $\tfrac{\partial L_j^*}{\partial l_1}>0$, a lower $l_1$ means a lower $L_j^*$), consistent with substitutability. 
If $0<\sigma<1$, goods act more like complements, and lowering $l_{1}$ increases labor usage in other sectors.
In all cases, total labor remains $1$.



One could have two goods, A and B. 
The A good is exclusively produced by human labor; the B task is the two task model. 
In equilibrium, the firm has to earn zero profits price has to equal marginal cost, so $p_A = w t_h$. 
The B good is produced by a chain of tasks, so $p_B = w l_{B}(w)$.
The relatively prices of the two goods will just be their relative labor costs. 


We could have a third good that consumers value at 

\section*{Sketch: Consumer Problem with Utility Function}

We have the utility function:
\[
u(x, y) = \ln\left(x + \frac{1}{\underline{p}}\right) + \ln(y),
\]
subject to the budget constraint:
\[
p_x x + y = B.
\]
Here, \(y\) is the numeraire good (\(p_y = 1\)), \(p_x > 0\) is the price of good \(x\), and \(B > 0\) is the total budget.

\subsection*{Corner Solution: \(x = 0\)}
If \(x = 0\), then the budget constraint implies:
\[
y = B.
\]
The utility in this case is:
\[
u(0, B) = \ln\left(\frac{1}{\underline{p}}\right) + \ln(B).
\]
To determine if the consumer will deviate from \(x = 0\), we compare the marginal rate of substitution (MRS) with the price ratio.

\subsubsection*{Marginal Rate of Substitution}
The marginal rate of substitution is:
\[
\text{MRS}(x, y) = \frac{\frac{\partial u}{\partial x}}{\frac{\partial u}{\partial y}} = \frac{\frac{1}{x + \frac{1}{\underline{p}}}}{\frac{1}{y}} = \frac{y}{x + \frac{1}{\underline{p}}}.
\]
At \(x = 0\) and \(y = B\):
\[
\text{MRS}(0, B) = \frac{B}{\frac{1}{\underline{p}}} = B \, \underline{p}.
\]

\subsubsection*{Condition for Corner Solution}
The slope of the budget line is:
\[
\frac{p_x}{p_y} = p_x.
\]
A corner solution occurs if:
\[
\text{MRS}(0, B) \leq p_x \quad \Longrightarrow \quad B \, \underline{p} \leq p_x.
\]
Thus, the corner solution is:
\[
x^* = 0, \quad y^* = B, \quad \text{if } B \, \underline{p} < p_x.
\]

\subsection*{Interior Solution}
If \(B \, \underline{p} \geq p_x\), the consumer chooses a positive amount of \(x\). The first-order condition is:
\[
\text{MRS}(x, y) = p_x \quad \Longrightarrow \quad \frac{y}{x + \frac{1}{\underline{p}}} = p_x.
\]
This simplifies to:
\[
y = p_x \left(x + \frac{1}{\underline{p}}\right).
\]
Substitute this into the budget constraint:
\[
p_x x + y = B \quad \Longrightarrow \quad p_x x + p_x \left(x + \frac{1}{\underline{p}}\right) = B.
\]
Simplify:
\[
2 p_x x + \frac{p_x}{\underline{p}} = B \quad \Longrightarrow \quad x = \frac{B - \frac{p_x}{\underline{p}}}{2 p_x}.
\]
Substitute \(x\) back into the expression for \(y\):
\[
y = p_x \left(x + \frac{1}{\underline{p}}\right) = p_x \left(\frac{B - \frac{p_x}{\underline{p}}}{2 p_x} + \frac{1}{\underline{p}}\right).
\]
Simplify:
\[
y = \frac{B - \frac{p_x}{\underline{p}}}{2} + \frac{p_x}{\underline{p}} = \frac{B + \frac{p_x}{\underline{p}}}{2}.
\]

\subsection*{Summary of the Solution}
\textbf{Corner Solution:} If \(B \, \underline{p} < p_x\),
\[
x^* = 0, \quad y^* = B.
\]

\textbf{Interior Solution:} If \(B \, \underline{p} \geq p_x\),
\[
x^* = \frac{B - \frac{p_x}{\underline{p}}}{2 p_x}, \quad y^* = \frac{B + \frac{p_x}{\underline{p}}}{2}.
\]
