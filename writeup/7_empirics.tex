...
The two panels do not give the same answer, and we read the difference as one of scope rather than of statistical precision.
O*NET is designed to cover the full breadth of occupations in the economy, and many of its task lists describe a repertoire of activities rather than a process with an intrinsic order, so the sequence has to be imputed.
The PCF instead documents operational business processes, which practitioners record as ordered flows because that is what they are.
Of the three tests, Prediction~\#3 is the one that rests most heavily on the ordering, since the fragmentation index is computed from where the AI-able steps sit rather than from how many of them there are.
It is therefore the test we would expect to speak most clearly on a corpus whose sequences are documented rather than inferred, and least clearly on material that is not strongly sequential to begin with.
On the PCF corpus we find the workflow-level channel the model predicts, and on O*NET occupations we are unable to detect it.

Two caveats govern how we read all three results.
First, the exercise is observational throughout, and we make no causal claim.
...
Second, the PCF evidence rests on exposure and execution labels carried over from O*NET, and that transfer is lossy.
Only $12\%$ of PCF steps carry an AI-able label after the match, against $44\%$ in O*NET, both because the crosswalk drops material and because the framework's scope is narrower than O*NET's.
Labels that thin make inference on that corpus harder rather than easier, so we read the PCF coefficient as corroborating the mechanism rather than as settling it.
Taken together, the directions we observe are consistent with what the model predicts, which is the claim we make and also its limit.
