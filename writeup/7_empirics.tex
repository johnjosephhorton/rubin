...
The PCF is more focused than O*NET.
It documents operational business processes such as procurement, order management, after-sales service, inventory, and regulatory compliance, and APQC's member working groups record the elements of each process in the order practitioners perform them.
For a test that turns on the arrangement of a workflow, an ordering taken from practice rather than inferred by a language model is what the exercise requires.
Its $525$ process groups cover a smaller part of economic activity than O*NET's occupational structure, so the fragmentation channel is documented on that part of the economy rather than across it, and its AI labels are carried over from O*NET by a lossy match, which leaves $12\%$ of PCF steps AI-able against $44\%$ in the main O*NET sample.

Throughout, we document associations and not causal effects.
Firms choose how much of their work to hand to AI at the same time as they choose how that work is arranged, and nothing in this section isolates an exogenous change in either.
What we report is that these associations run in the direction the model implies in both samples, and that on the corpus whose sequences are documented they are precise enough to measure.
