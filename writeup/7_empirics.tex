...
Two features of this test shape how we read its results.

The first is that a workflow-level test asks more of an ordering than a step-level one does, and it does so in a particular way.
A step placed one position too early or too late shifts only the immediate neighborhood on which Predictions~\#1 and~\#2 are built, moving a handful of adjacency pairs and leaving the rest of the workflow as it was.
That same misplacement can split a run of AI-able steps in two, which raises $r_w$ and moves the fragmentation index of the entire workflow.
The index is a property of the whole arrangement, so it inherits every error in that arrangement and not only the local ones, and it is for this reason that we bring the PCF alongside the main sample.
APQC's member working groups record the elements of a process in the order practitioners perform them, across operational business processes such as procurement, order management, after-sales service, inventory, and regulatory compliance.
Those $525$ process groups are a far narrower slice of economic activity than O*NET's coverage of the occupational structure, so the PCF complements the main sample rather than standing in for it.

The second is that the exercise is observational throughout, and we make no causal claim.
Firms choose how much of their work to hand to AI at the same time as they choose how that work is arranged, and nothing in this section isolates an exogenous change in either, so what we report are associations whose directions agree with the mechanism the model implies rather than estimates of its effect.
The PCF evidence carries a further qualification, since its exposure and execution labels are carried over from O*NET and that transfer is lossy, leaving only $12\%$ of PCF steps with an AI-able label against $44\%$ in O*NET.
Labels that thin make inference on that corpus harder rather than easier.
We therefore read the fragmentation evidence as corroborating the mechanism rather than as settling it, while noting that the corpus whose sequences were documented rather than imputed is the one that delivers the sign and the significance the model predicts.
