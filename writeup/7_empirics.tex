...
The three predictions we test are: (1) AI-executed steps tend to appear next to each other in the production sequence, forming AI chains; (2) the AI-execution status of a step's neighbors affects the likelihood that the step itself is executed by AI; and (3) workflows in which AI-exposed steps are more dispersed have fewer of those steps actually executed by AI.
...
To create this dataset, we combine data from five sources:
\begin{enumerate}
...
\end{enumerate}
All of these are existing datasets apart from the task ordering, which we generate ourselves.
...
It maps millions of anonymized conversations between users and Claude, Anthropic's AI assistant, to O*NET tasks and classifies each conversation by whether the interaction augments or automates the task, providing a task-level measure of realized AI usage.
...
We bring this corpus in because that ordering is documented rather than imputed, which matters most for the workflow-level test of Subsection~\ref{sec:fragmentation_prediction}.
...
Appendices~\ref{app:data_prep} and~\ref{app:external_validation} give the construction details of the main O*NET sample and APQC sample, respectively.
...
The immediate neighbor effect attenuates from $0.12$ in the baseline to between $0.04$ and $0.06$ but stays statistically and economically meaningful, while the effects two positions away fall to between $-0.01$ and $0.01$ and lose significance.
...
\subsection{Prediction \#3: Dispersion of AI-able Steps in Workflow Lowers AI Execution}
