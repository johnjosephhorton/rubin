...
The two corpora trade against each other, and for this particular test the trade is one worth making.
The PCF is the narrower of the two, covering operational business processes such as procurement, order management, after-sales service, inventory, and regulatory compliance rather than the full occupational structure.
What its $525$ process groups offer in return is the thing a workflow-level test needs most, an ordering that APQC's member working groups recorded in the order practitioners perform the work, rather than one a language model inferred.
The weaker half of the trade is the AI content, since the exposure and execution labels are carried over from O*NET by a lossy match that leaves only $12\%$ of PCF steps AI-able against $44\%$ in the main sample, and the estimates deserve a corresponding measure of caution.
Neither corpus, in any case, supports a causal reading.
Firms choose how much of their work to hand to AI at the same time as they choose how that work is arranged, and nothing in this section isolates an exogenous change in either, so what we document are associations rather than effects.
Those associations run in the direction the model implies in both samples, and on the corpus built around documented sequence they are sharp enough to measure.
