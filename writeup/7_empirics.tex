...
That same misplacement can split a run of AI-able steps in two, however, changing the implied fragmentation of AI-able steps over the entire workflow rather than of one part of it.
...
We therefore work with the PCF corpus from APQC, whose ordering of activities practitioners documented rather than a language model imputed, alongside the main O*NET sample.
...
\end{equation}
where \emph{$\text{ai\_execution}_{w}$} is the share of steps executed by AI in workflow $w$, \emph{$\text{ai\_exposure}_{w}$} is the share of AI-exposed steps, \emph{$\text{empirical\_fragmentation\_index}_{w}$} is the index defined above, and \emph{$\text{num\_ai\_able\_steps}_{w}$} is the number of AI-able steps it contains.\footnote{Controlling for workflow length $m_w$ instead, or dropping the count control altogether, moves the fragmentation coefficient by at most $0.04$ standard deviations and changes no verdict.}
...
The PCF corpus that carries this result is narrower than the O*NET sample in a way worth stating plainly.
...
Firms choose how much of their work to hand to AI at the same time as they choose how that work is arranged, and nothing in this section isolates an exogenous change in either, so what we report are associations whose directions agree with the mechanism the model implies rather than causal estimates.
...
Even so, it is the corpus whose sequences were documented rather than imputed that delivers the sign and the significance the model predicts.
