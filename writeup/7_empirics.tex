...
A workflow-level test asks more of an ordering than a step-level one does, and it does so in a particular way.
...
That same misplacement can split a run of AI-able steps in two, which changes the measured fragmentation of the entire workflow rather than of one part of it.
Because the measure is a property of the whole arrangement, it inherits every error in that arrangement and not only the local ones.
We therefore work with the PCF corpus, whose ordering practitioners documented rather than a language model imputed, alongside the main O*NET sample.
...
The PCF corpus that carries this result is narrower than the main sample in a way worth stating plainly.
...
The exercise is also observational throughout, and we make no causal claim.
