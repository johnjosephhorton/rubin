...
That the channel is sharpest in the PCF is what a test built on arrangement would lead one to expect.
The fragmentation index reads the ordering of a workflow as a whole, so it stands its best chance of detection where that ordering is recorded rather than inferred.
This is what APQC provides, since its member working groups document the elements of each process in the order practitioners perform them, across operational business processes such as procurement, order management, after-sales service, inventory, and regulatory compliance.
The $525$ process groups of APQC cover a smaller part of economic activity than O*NET's occupational structure, and their AI labels are carried over from O*NET by a lossy match that leaves $12\%$ of PCF steps AI-able against $44\%$ in the main O*NET sample, so the channel is documented on that part of the economy rather than across it.

What we document in both samples are associations that speak to the mechanism the model describes, and not causal effects.
Measuring the latter would require exogenous variation in how much of a workflow is handed to AI and in how that workflow is arranged.
Firms choose the two together, and nothing in this section separates them.
