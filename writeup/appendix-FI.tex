...
In this appendix we prove Proposition~\ref{prop:fragmentation}, which relates the cost of the optimal AI deployment strategy for a step sequence to its fragmentation index.
...
We can relax the assumption in Proposition~\ref{prop:FI.lower.bound.4} that $\manualSkill{i}\manualTime{i} \geq 1$ for all $s_i$, but at the cost of a worse approximation factor.
...
But, as we now show, the factor is strictly greater than the factor $4$ from Proposition~\ref{prop:FI.lower.bound.4}, so the assumption that $\manualSkill{i}\manualTime{i} \geq 1$ for all $s_i$ is necessary for Proposition~\ref{prop:FI.lower.bound.4} to hold.
