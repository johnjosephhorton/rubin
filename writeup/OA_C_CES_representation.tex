...
Throughout Sections~\ref{sec:shortrun}--~\ref{sec:longrun} the model focused on individual firm decision-making: how a firm allocates production steps among automation, augmentation, and manual execution, and how its micro-level choices of task assignment and job design determine internal productivity and costs.
However, improvements in AI quality not only alter how individual firms organize production but also affect the composition of inputs used across the economy and the aggregate relationship between labor, capital, and output.
Yet the firm-level optimization framework developed above prevents direct analysis of these aggregate effects and also makes our approach distinct from the large literature in which production is modeled directly at the task level and then aggregated to the economy level (e.g., \cite{acemoglu2018, acemoglu2022tasks, acemoglu2025simple}).
...
These two types of labor demand different compensations: executing an AI chain with skill and time cost of one demands $w_{\AIletter}$, while completing a manual task with similar skill and time requirements commands a compensation of $w_{\manualLetter}$.
...
In order to produce, the firm hires manual labor to perform manual tasks and AI management labor for AI chains.
...
A task's time requirement for a single attempt inherits its mode from $E(b)$ and is written $\timeCostLetter^{E(b)}_{b}$: it is $\manualTime{b}$ when task $b$ is a manual step, and $\AItime{b}$, the cost of verifying the chain's augmented endpoint, when it is an AI chain.
...
The condition expressed in \eqref{eq:task_subst_rate_indep} satisfies the aggregation requirement above.\footnote{In fact, this condition is stricter than necessary. It not only guarantees labor independence across aggregated input types, but also imposes a stronger condition: that the rate of substitution between tasks within the same aggregate input type is independent even of other tasks within the same aggregate type.}
...
\end{equation}
The representation in \eqref{eq:micro_agg_prod} helps us gain clearer insights into the allocation between manual and AI management labor at the firm level by removing the need to deal with the complex, many-input production function in \eqref{eq:task_level_prod} at the level of individual tasks.
...
Throughout, we assume that $\rho<0$, so that the elasticity of substitution is $\sigma<1$, the case usually taken in this literature.
...
Let $\phi(\bar{\alpha})$ be the density of output across firms according to their effective AI quality level, so that aggregate output is the integral of $\phi$ over the range of producing firms.
Only the product of the density of firms over $\bar{\alpha}$ and the output of a firm of type $\bar{\alpha}$ enters the aggregate quantities below, so we normalize the former to one and let $\phi$ carry the latter: a firm with effective AI quality $\bar{\alpha}$ produces output $y = \phi(\bar{\alpha})$.
For brevity we refer to $\phi$ below as the distribution of effective AI quality, always in this sense of output across quality levels rather than of firms across quality levels.
...
\begin{align}
Y & = \int_{\frac{w_{\AIletter}\,\skillAdjustedTimeLetter_{\AIletter}}{1 - w_{\manualLetter}\,\skillAdjustedTimeLetter_{\manualLetter}}}^{1}\,\phi(\bar{\alpha})\,d\bar{\alpha}, \label{eq:micro_Y}\\
...
\begin{sloppypar}
Substituting the aggregated variables from the micro-level Equations~\eqref{eq:micro_Y}, \eqref{eq:micro_M}, and \eqref{eq:micro_H} into the aggregate production function in \eqref{eq:macro_agg_prod}, we obtain:
\end{sloppypar}
...
The derived distribution in \eqref{eq:phi} characterizes firm-level heterogeneity in the effective AI quality level required to support a macro CES production function of the form in \eqref{eq:macro_agg_prod}, with economy-wide aggregate AI management and manual labor inputs and parameters $\theta_{\AIletter}, \theta_{\manualLetter}, \rho$, obtained from micro-level Leontief production functions of the form in \eqref{eq:micro_agg_prod}.


Two features of this construction are worth making explicit.
First, because $\skillAdjustedTimeLetter_{\manualLetter}$ is common to all firms, Equation~\eqref{eq:micro_H} gives $\aggregateManualLabor = \skillAdjustedTimeLetter_{\manualLetter}\,Y$ at every wage vector, while capital is normalized to $K=1$.
...
Equation~\eqref{eq:macro_agg_prod} should accordingly be read as a representation valid along that locus rather than as an identification of the economy's technology over arbitrary $(\aggregateAIlabor, \aggregateManualLabor, K)$: it captures how the economy substitutes between AI management and manual labor as the participation threshold moves, and says nothing testable about the manual-input-per-output or capital margins, which are fixed by construction.
...
By showing how heterogeneous firms with step-by-step Leontief technologies and endogenous task boundaries can be represented by a CES aggregator, our framework offers a micro-founded rationale for using aggregate CES production functions to study labor demand and productivity in economies adopting new AI technologies.
...
In this subsection we describe how to obtain the analytical formula for distribution of heterogeneity $\phi(\bar{\alpha})$ expressed in \eqref{eq:phi} by extending the method of \cite{levhari1968note}.
...
It ties the weights of the aggregate production function to $\skillAdjustedTimeLetter_{\AIletter}$ and $\skillAdjustedTimeLetter_{\manualLetter}$, which are themselves determined by the AI strategy and job design the firm has committed to, so the organization of work is what fixes the CES parameters rather than the two being free of one another.
