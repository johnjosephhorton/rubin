...
Only the product of the density of firms over $\bar{\alpha}$ and the output of a firm of type $\bar{\alpha}$ enters the aggregate quantities below, so we normalize the former to one and let $\phi$ carry the latter.
A firm with effective AI quality $\bar{\alpha}$ then produces output $y = \phi(\bar{\alpha})$.
...
It captures how the economy substitutes between AI management and manual labor as the participation threshold moves, and says nothing testable about the manual-input-per-output or capital margins, which are fixed by construction.
