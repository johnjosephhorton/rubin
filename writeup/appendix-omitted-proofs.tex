...
\begin{proof}
The two strategies agree on every step outside the block $k, \dotsc, r$.
In the $k, \dotsc, r$ block, their costs are
\[
...
\]
which is~\eqref{eq:pred_chain}.
This is the marginal test of Proposition~\ref{prop:ca_local} with the single step $k$ replaced by the contiguous pair $\{k-1, k\}$, carrying joint success probability $q_{k-1}q_k$: the failure risk $1-q_k$ becomes the pair's joint failure risk $1-q_{k-1}q_k$.
By~\eqref{eq:pred_chain} the chain is strictly cheaper, so the described strategy is not cost-minimizing.
\end{proof}
...
Suppose that some cost-minimizing strategy performs step $k$ manually.
We construct a feasible alternative, chaining the whole tail $k, \dotsc, m$, that condition~\eqref{eq:ca_sufficient} makes strictly cheaper, contradicting optimality.
Throughout, write $\mathrm{OPT}(I)$ for the minimum cost of executing a block of consecutive steps $I$ in isolation.

\emph{Cost of the manual strategy.} Because step $k$ is performed manually, no chain crosses it, so the sequence splits into three independently costed pieces: steps $1, \dotsc, k{-}1$, the manual step $k$, and steps $k{+}1, \dotsc, m$.
The strategy's total cost is
\begin{equation}\label{eq:ct_manual}
...
\begin{equation}\label{eq:ct_candidate}
\mathrm{OPT}(1{:}k{-}1) \;+\; \frac{\AIskill{m}\AItime{m}}{\prod_{i=k}^{m} q_i}.
\end{equation}
This candidate is feasible, so the assumed-optimal manual strategy is no more expensive than it; the manual cost~\eqref{eq:ct_manual} is therefore at most the candidate cost~\eqref{eq:ct_candidate}.
Taking the difference between~\eqref{eq:ct_candidate} and~\eqref{eq:ct_manual} and cancelling the shared term $\mathrm{OPT}(1{:}k{-}1)$,
\begin{equation*}
\frac{\AIskill{m}\AItime{m}}{\prod_{i=k}^{m} q_i} \;\ge\; \manualSkill{k}\manualTime{k} + \mathrm{OPT}(k{+}1{:}m). \tag{$\ast$}
\end{equation*}

\emph{Bounding $\mathrm{OPT}(k{+}1{:}m)$.} Executing the block $k{+}1, \dotsc, m$ costs at least as much as the single task that covers its final step $m$.
That task must end at $m$, since no chain extends past the last step, so it is either step $m$ on its own (manual or augmented) or a chain ending at $m$:
\[
\underbrace{\min\!\left\{\manualSkill{m}\manualTime{m},\ \frac{\AIskill{m}\AItime{m}}{q_m}\right\}}_{\text{step } m \text{ alone}}
\qquad\text{or}\qquad
\underbrace{\frac{\AIskill{m}\AItime{m}}{q_m} \;\le\; \frac{\AIskill{m}\AItime{m}}{\prod_{i=\ell}^{m} q_i}}_{\text{chain ending at } m}.
\]
...
\]
which contradicts~\eqref{eq:ca_sufficient}.
Hence no cost-minimizing strategy performs step $k$ manually: in every optimum, step $k$ is automated.
\end{proof}
