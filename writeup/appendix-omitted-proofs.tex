...
This appendix collects the proofs of all propositions stated in the main body of the paper.
...
The two strategies agree on every step outside the block $k, \dotsc, r$. In the $k, \dotsc, r$ block, their costs are
\[
...
\]
The second is the smaller exactly when the \emph{marginal} cost of folding step $k$ in the chain (i.e., the difference of the two chain terms) falls below the manual execution cost $\manualSkill{k}\manualTime{k}$:
\[
...
\subsection{Proof of Proposition~\ref{prop:ca_predecessor}}

The proof is fairly mechanical. The two strategies agree on every step outside the block $k-1, k, k+1$, and both reach the endpoint $k+1$ as an augmented step, paying $\AIskill{k+1}\AItime{k+1}/q_{k+1}$ for its management. Within the block, the described strategy costs
\[
...
\]
while running the three steps as one AI chain augmented at $k+1$ costs $\AIskill{k+1}\AItime{k+1}/(q_{k-1}q_k q_{k+1})$, paying nothing to manage the predecessor or step $k$. The chain is cheaper exactly when the \emph{marginal} cost of folding the pair $\{k-1, k\}$ in, the difference of the two chain terms, falls below the cost of doing that pair the old way. Crediting back the common endpoint term $\AIskill{k+1}\AItime{k+1}/q_{k+1}$ from both sides,
\[
...
\]
which is~\eqref{eq:pred_chain}. This is the marginal test of Proposition~\ref{prop:ca_local} with the single step $k$ replaced by the contiguous pair $\{k-1, k\}$, carrying joint success probability $q_{k-1}q_k$: the failure risk $1-q_k$ becomes the pair's joint failure risk $1-q_{k-1}q_k$. By~\eqref{eq:pred_chain} the chain is strictly cheaper, so the described strategy is not cost-minimizing.

Splitting the pair's failure risk along the chain isolates the predecessor's role,
\[
1 - q_{k-1}q_k \;=\; \underbrace{(1-q_{k-1})}_{\text{predecessor's share}} \;+\; \underbrace{q_{k-1}(1-q_k)}_{\text{step }k\text{'s share}},
\]
so the pair's marginal cost splits into a predecessor term $\AIskill{k+1}\AItime{k+1}(1-q_{k-1})/(q_{k-1}q_k q_{k+1})$ and step $k$'s own term $\AIskill{k+1}\AItime{k+1}(1-q_k)/(q_k q_{k+1})$, the latter being exactly the left-hand side of the second inequality in~\eqref{eq:pred_local}. Conditions~\eqref{eq:pred_local}, together with $\manualSkill{k}\manualTime{k} < \AIskill{k}\AItime{k}/q_k$, make this a reversal rather than a routine substitution: absent the predecessor, the two-step workflow $\{k, k+1\}$ is solved with step $k$ manual, since step $k$'s own term already exceeds its manual cost. The predecessor's term is the only new ingredient, and it is small precisely when the predecessor is AI-reliable; that the cheap standalone predecessor cost it displaces still covers it is what tips step $k$ into the chain.

\subsection{Proof of Proposition~\ref{prop:ca_threshold}}

We prove the proposition by contradiction. Suppose that some cost-minimizing strategy performs step $k$ manually. We construct a feasible alternative, chaining the whole tail $k, \dotsc, m$, that condition~\eqref{eq:ca_sufficient} makes strictly cheaper, contradicting optimality. Throughout, write $\mathrm{OPT}(I)$ for the minimum cost of executing a block of consecutive steps $I$ in isolation.

\emph{Cost of the manual strategy.} Because step $k$ is performed manually, no chain crosses it, so the sequence splits into three independently costed pieces: steps $1, \dotsc, k{-}1$, the manual step $k$, and steps $k{+}1, \dotsc, m$. The strategy's total cost is
\[
\mathrm{OPT}(1{:}k{-}1) \;+\; \manualSkill{k}\manualTime{k} \;+\; \mathrm{OPT}(k{+}1{:}m).
\]

\emph{Cost of the candidate.} As an alternative, execute steps $1, \dotsc, k{-}1$ optimally and run the whole block $k, \dotsc, m$ as a single AI chain augmented at $m$, for a total cost of
\[
\mathrm{OPT}(1{:}k{-}1) \;+\; \frac{\AIskill{m}\AItime{m}}{\prod_{i=k}^{m} q_i}.
\]
This candidate is feasible, so the assumed-optimal manual strategy is no more expensive than it. Cancelling the shared term $\mathrm{OPT}(1{:}k{-}1)$,
\begin{equation*}
\frac{\AIskill{m}\AItime{m}}{\prod_{i=k}^{m} q_i} \;\ge\; \manualSkill{k}\manualTime{k} + \mathrm{OPT}(k{+}1{:}m). \tag{$\ast$}
\end{equation*}

\emph{Bounding $\mathrm{OPT}(k{+}1{:}m)$.} Executing the block $k{+}1, \dotsc, m$ costs at least as much as the single task that covers its final step $m$. That task must end at $m$, since no chain extends past the last step, so it is either step $m$ on its own (manual or augmented) or a chain ending at $m$:
\[
\underbrace{\min\!\left\{\manualSkill{m}\manualTime{m},\ \frac{\AIskill{m}\AItime{m}}{q_m}\right\}}_{\text{step } m \text{ alone}}
\qquad\text{or}\qquad
\underbrace{\frac{\AIskill{m}\AItime{m}}{\prod_{i=\ell}^{m} q_i} \;\ge\; \frac{\AIskill{m}\AItime{m}}{q_m}}_{\text{chain ending at } m}.
\]
In either case, and since every other task in the block adds nonnegative cost, the minimum-cost way of performing steps $k+1, \cdots, m$ will be weakly greater than optimally executing the last step alone,
\[
\min\!\left\{\manualSkill{m}\manualTime{m},\ \frac{\AIskill{m}\AItime{m}}{q_m}\right\} \;\le\; \mathrm{OPT}(k{+}1{:}m).
\]

\emph{Contradiction.} Substituting this lower bound for $\mathrm{OPT}(k{+}1{:}m)$ into~$(\ast)$ gives
\[
\frac{\AIskill{m}\AItime{m}}{\prod_{i=k}^{m} q_i}
\;\ge\;
\manualSkill{k}\manualTime{k} \;+\; \min\!\left\{\manualSkill{m}\manualTime{m},\ \frac{\AIskill{m}\AItime{m}}{q_m}\right\},
\]
which contradicts~\eqref{eq:ca_sufficient}. Hence no cost-minimizing strategy performs step $k$ manually: in every optimum, step $k$ is automated.
...
We now introduce some notation: for each $s_i$, write $\minTime{i} = \min\left\{\manualSkill{i}\manualTime{i}, \frac{\AIskill{i}\AItime{i}}{q_i}\right\}$.  That is, $\minTime{i}$ is the minimum cost to implement step $s_i$ individually (whether manually or through AI augmentation).
...
The optimal policy automates all three together for an expected cost of $\AIskill{3}\AItime{3}/(q_1 q_2 q_3) = 1/(1 \cdot \tfrac12 \cdot 1) = 2$, whereas the fragmentation index is $\tfrac12 \times 1 + \tfrac12 \times (1+2+1) = \tfrac52$. The ratio $FI/OPT$ is therefore $\tfrac54$.
\end{example}
...
Within a regime where $\T$ is optimal, $g^*(\alpha) = g_\T(\alpha) = -C_\T'(\alpha) \ge 0$, and the convexity of $C_\T$ makes it non-increasing in $\alpha$ (when $\T$ uses no chain $g^*(\alpha) = 0$ but is otherwise strictly decreasing). The returns to a fixed configuration in a regime thus weakly diminish. This proves the first claim of the proposition.

Next, let $\alpha_0 \in (0,1)$ be a threshold where the optimum switches from $\T$ (optimal just below $\alpha_0$) to $\T'$ (just above $\alpha_0$).\footnote{We assume this threshold exists. The only scenario where no such threshold exists is the case in which all steps are executed manually for all $\alpha \in (0,1)$. This is the trivial, uninteresting case and is assumed away.} Define
\[
...
\phi(\alpha_0^{+}) \le 0.
\]
A differentiable function with such a sign change has nonpositive derivative in the local neighborhood of $\alpha_0$. So $\phi'(\alpha_0) \le 0$, or equivalently, $C_{\T'}'(\alpha_0) \le C_\T'(\alpha_0)$. By the definition $g_\T = -\,\mathrm{d}C_\T/\mathrm{d}\alpha$, this is
\[
