...
Figure~\ref{fig:efi_regression_exposure_robustness} plots the equivalents of estimated coefficients reported in Table~\ref{tab:fragmentation_index_regression_exposure} for the main variables across the three specifications (baseline, SOC major group fixed effects, and SOC minor group fixed effects) for the exposure-based empirical fragmentation index (Definition~1).
...
Similarly, Figure~\ref{fig:efi_regression_execution_robustness} plots the equivalents of estimates reported in Table~\ref{tab:fragmentation_index_regression_execution} for the ex-post, execution-based EFI (Definition~2) using alternative prompts.
...
\begin{figure}[ht!]
  ...
  The graphs correspond to columns (1)\textendash(3) of Table~\ref{tab:fragmentation_index_regression_execution}.
}
\end{figure}
