...
%     Blue markers indicate occupations below the median empirical fragmentation index (Definition~1) in the full sample, while red markers indicate occupations above the median.
%     }
% \end{figure}
% \begin{figure}[ht!]
%     \begin{center}
...
Here, we examine how sensitive the average AI chain length statistic is to the choice of GPT prompt.
...
In particular, the mean across alternative prompts (the dashed colored line) matches the main sample estimate from the main prompt, which lies in the extreme tail of the placebo distributions in Figure~\ref{fig:aiChains_graphs_def1}.
\begin{figure}[ht!]
...
\end{center}
\raggedright \footnotesize{
\emph{Notes:} This figure shows that the average AI chain length measure is robust to alternative prompt formulations.
...
Here, we re-estimate Equation~\eqref{eq:fragmentation_index_regression} using task orderings generated by the alternative prompts and report the analogs of the estimates in Table~\ref{tab:fragmentation_index_regression}.

Figure~\ref{fig:efi_regression_exposure_robustness} plots the equivalents of estimated coefficients reported in columns (1)\textendash(3) of Table~\ref{tab:fragmentation_index_regression} for the main variables across the three specifications (baseline, SOC major group fixed effects, and SOC minor group fixed effects) for the exposure-based empirical fragmentation index (Definition~1).
...
The estimates on occupation-level AI exposure are positive and statistically different from zero throughout, while the EFI coefficients are consistently negative and statistically different from zero.

Similarly, Figure~\ref{fig:efi_regression_execution_robustness} plots the equivalents of estimates reported in columns (4)\textendash(6) of Table~\ref{tab:fragmentation_index_regression} for the ex-post, execution-based EFI (Definition~2) using alternative prompts.
Again, similar to the results obtained from the main prompt, the estimated coefficient on AI exposure is positive and the estimated coefficient on the empirical fragmentation index is negative.
...
\begin{figure}[ht!]
  \begin{center}
  \caption{Robustness of Table~\ref{tab:fragmentation_index_regression} Exposure-Based (Definition~1) Estimates to GPT Prompts} 
  ...
  The graphs report coefficients on AI exposure (top row, blue) and EFI (bottom row, gold) for the exposure-based EFI (Definition~1).
  Error bars denote 90\% confidence intervals.
  The dashed colored lines report the mean across all 11 prompts.
  In each graph, the Prompt~0 estimate is highlighted in red and corresponds to the estimate reported in Table~\ref{tab:fragmentation_index_regression} using the main prompt.
  The graphs correspond to columns (1)\textendash(3) of Table~\ref{tab:fragmentation_index_regression}.
}
\end{figure}

\begin{figure}[ht!]
  \begin{center}
  \caption{Robustness of Table~\ref{tab:fragmentation_index_regression} Execution-Based (Definition~2) Estimates to GPT Prompts} 
  ...
  \end{center}
  \raggedright \footnotesize{\emph{Notes:} This figure shows that the estimated relationships between occupation-level AI execution and both AI exposure and the empirical fragmentation index are robust to alternative prompt formulations.
  The graphs report coefficients on AI exposure (top row, blue) and EFI (bottom row, gold) for the execution-based EFI (Definition~2).
  Error bars denote 90\% confidence intervals.
  The dashed colored lines report the mean across all 11 prompts.
  In each graph, the Prompt~0 estimate is highlighted in red and corresponds to the estimate reported in Table~\ref{tab:fragmentation_index_regression} using the main prompt.
  The graphs correspond to columns (4)\textendash(6) of Table~\ref{tab:fragmentation_index_regression}.
}
\end{figure}
