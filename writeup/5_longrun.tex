...
Both components of a chain's pair are therefore inherited from its augmented endpoint: the chain demands only the skill required to verify that step's output, and an automated step inside the chain demands no skill of anyone, since no human ever handles it directly.
...
We therefore attach the hand-off cost to the step at which the break occurs rather than to the worker or to the number of jobs as in \citet{becker1992division}, so that \emph{where} the firm divides the workflow matters and not merely how often it divides it.
