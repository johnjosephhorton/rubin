...
Here we relax this assumption and allow the firm to restructure its production fully around the capabilities of AI.

Over longer horizons the firm can change its organizational structure, redrawing the boundaries between jobs and with them the allocation of steps to workers, while optimizing its use of AI.
Moreover, free entry in the labor market lets the firm pay workers according to the actual skill requirements of the jobs it designs, rather than at the rate the short run held fixed, effectively endogenizing wages.
...
We thus introduce skills to capture this dimension of work.

Accordingly, we extend Definitions~\ref{def:manual_step}--\ref{def:ai_chain} to incorporate the skill cost of steps as well: 
the skill and time cost of performing step $i$ manually is $(\manualSkill{i},\, \manualTime{i})$, while verifying the output of one AI attempt at it costs $(\AIskill{i},\, \AItime{i})$.
The two costs need not coincide, since doing the work oneself calls for knowing how to do it, whereas verifying what an AI returns may call for knowledge of a different kind.
...
An AI chain therefore demands only the skill required to verify the output of its augmented step, and an automated step inside a chain demands no skill of anyone, since no human ever handles it directly.
...
Panel~B of Table~\ref{tab:notation} in Appendix~\ref{app:theory_tables} collects the skill, job, and wage notation this section adds.
...
Because the firm chooses those boundaries when it solves \eqref{eq:totalcost_with_handoff}, the answer runs through the trade-off that fixes them, between the skill a broad job demands and the coordination a narrow one requires.
We set that trade-off out first, and then ask how AI moves it.

The wage bill \eqref{eq:wagebill} is a product of a skill and a time, so it reads naturally as an area, which is what makes the trade-off visible.
...
Bundling therefore buys the removal of a hand-off at the price of idle skill, and where those two prices balance is the firm's degree of specialization.
...
That cuts the idle skill a bundled job pays for, which makes wider jobs cheaper to sustain and pushes the firm toward consolidation.
...
The second channel is the hand-off cost itself.
We have treated $\handofftime{i}$ as a primitive, but AI plausibly lowers it, by absorbing the peripheral coordination a transfer requires, such as summarizing what was done or drafting the documentation that accompanies it.\footnote{
...
What the framework delivers here is therefore not a prediction about job breadth but an account of what determines it.
AI presses on both sides of the specialization trade-off at once, and which way jobs move depends on the relative size of the skill it strips out and the coordination it leaves behind.
