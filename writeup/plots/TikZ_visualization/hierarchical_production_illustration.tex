% Local override for draft_mert: step order updated (1 manual, 2 augmented, 3 manual,
% 4-5 automated, 6 augmented, 7 manual). The parent ../../../plots/TikZ_visualization/
% copy keeps the old order for the legacy writeup/main.tex and is left untouched.
...
\begin{adjustbox}{center,trim=0pt 0pt 0pt 0pt,clip,margin=-2cm 0cm}
  \begin{tikzpicture}[scale=0.73, transform shape,
    ...
    \draw[->, thick] (S1) -- (S2);
    \draw[->, thick, red, decorate, decoration={snake, amplitude=.5mm, segment length=3.05mm}] (S2) -- (S3);
    \draw[->, thick, red, decorate, decoration={snake, amplitude=.5mm, segment length=3.05mm}] (S3) -- (S4);
    \draw[->, thick] (S4) -- (S5);
    \draw[->, thick] (S5) -- (S6);
    \draw[->, thick] (S6) -- (S7);
