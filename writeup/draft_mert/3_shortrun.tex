...
Such an attempt succeeds independently with some (step-dependent) probability $q_i=\alpha^{d_i}$, where $\alpha$ is the quality of the general-purpose AI technology and $d_i$ represents how ``difficult'' step $i$ is for AI, and it must be repeated until it succeeds, for a total expected time cost of $\AItime{i}/q_i$.
Definitions~\ref{def:manual_task}--\ref{def:automated_task} formalize the three modes of step execution.
...
\end{definition}
In the top layer of Figure~\ref{fig:steps_tasks}, Step~1, for example, is executed manually and hence is a manual step.
...
\end{definition}
In Figure~\ref{fig:steps_tasks}, Step~2 is an instance of an augmented step performed by the AI and verified by a human.
...
\end{definition}
In Figure~\ref{fig:steps_tasks}, Steps~4 and~5 are instances of automated steps. They are fully performed by the AI without requiring any direct human oversight.

Automation is the mode that makes AI more than a faster way of doing the same work, but it is not freely available.
...
\end{definition}
In Figure~\ref{fig:steps_tasks}, Steps~4--6 form an AI chain, automated at Steps~4 and~5 and augmented at Step~6, and Step~2 forms an AI chain of length one. The human prompts the AI for, and verifies the output of, only the chain's augmented step, which for Steps~4--6 is Step~6; Steps~4 and~5 are performed without any direct human intervention and are hence automated.
Next, we turn to how the firm's placement of these chains determines the tasks its workers hold.
...
An AI deployment strategy (or AI strategy for short) is a partition of the step sequence $\mathcal{S}$ into contiguous blocks, each of which is either a single manual step or an AI chain.
...
Table~\ref{tab:notation} in Appendix~\ref{app:theory_tables} collects the notation used throughout the paper; Panel~A covers the steps, tasks, and AI deployment introduced here and used again in Section~\ref{sec:results}.
...
This linkage gives rise to rich dynamics in how AI is deployed across a workflow, and hence in how automation affects it.
