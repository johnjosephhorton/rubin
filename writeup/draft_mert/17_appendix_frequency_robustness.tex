...
\begin{subfigure}[b]{\textwidth}
    ...
    \subcaption{Next Task ($t{+}1$)}
\end{subfigure}
\par\vspace{\fignotesskip}
\begin{minipage}{1\linewidth}
\begin{spacing}{0.2}
{\fontsize{9.5pt}{10pt}\selectfont Notes: Each cell reports the average marginal effect of having the immediately previous task ($t{-}1$, Panel~(a)) or next task ($t{+}1$, Panel~(b)) be AI-executed on the focal task's probability of AI execution, from re-estimating regression~\eqref{eq:DWA_regression_ai} on the corresponding frequency cut.
...
\begin{minipage}{1\linewidth}
\begin{spacing}{0.2}
{\fontsize{9.5pt}{10pt}\selectfont Notes: Panel~(a) reports the average marginal effect of having the previous task ($t{-}1$) be AI-executed, and Panel~(b) the corresponding effect for the next task ($t{+}1$).
