...
Table~\ref{tab:model_parameters} collects the notation used throughout the paper; Panel~A covers everything needed here and in Section~\ref{sec:results}.
...
\midrule
\addlinespace
\multicolumn{2}{@{}l}{\textbf{Panel A. Short run: steps, tasks, and AI deployment (Sections~\ref{sec:model.steps}--\ref{sec:results})}} \\
\addlinespace
...
\midrule
\addlinespace
\multicolumn{2}{@{}l}{\textbf{Panel B. Long run: skills, jobs, and wages (Section~\ref{sec:longrun.jobdesign})}} \\
\addlinespace
...
\midrule
\addlinespace
\multicolumn{2}{@{}l}{\textbf{Panel C. Long run: aggregation (Section~\ref{sec:longrun.aggregation})}} \\
\addlinespace
$\skillAdjustedTimeLetter_{\AIletter},\ \skillAdjustedTimeLetter_{\manualLetter}$ & Skill-adjusted time the firm requires per unit of output, of AI management labor and of manual labor; hand-offs enter $\skillAdjustedTimeLetter_{\manualLetter}$ \\
$\labor{\AIletter},\ \labor{\manualLetter}$ & AI management labor and manual labor the firm employs, in the same units \\
$\bar{\alpha}$ & Firm's realized effective AI quality, a time-weighted average of the success rates of the chains it runs \\
$x$ & Firm output \\
\addlinespace
$\aggregateAIlabor,\ \aggregateManualLabor$ & Economy-wide AI management labor and manual labor \\
$K,\ X$ & Aggregate capital and aggregate output \\
$\theta_{\AIletter},\ \theta_{\manualLetter},\ \rho$ & CES weights and substitution parameter in \eqref{eq:lr_macro_ces} \\
\bottomrule
\end{tabularx}

    \begin{minipage}{1\linewidth}
\begin{spacing}{0.2}
{\fontsize{9.5pt}{10pt}\selectfont Notes: This table collects the notation used throughout the paper. Step-level primitives are indexed by $i$, task-level objects by $b$, and jobs by $j$. Superscripts $M$ and $A$ denote the manual and AI modes of executing a step, and $H$ denotes a hand-off between workers. Panel~A is all the short-run problem requires: with the whole workflow assigned to one worker at a fixed wage, execution time is the only margin, and no notion of skill is needed. The skill, job, and wage notation of Panel~B enters only once the firm chooses how to divide the workflow among workers, and Panel~C only once firm-level production is aggregated to the economy.}
\end{spacing}
\end{minipage}
\end{table}
