...
What that rate compensates is the total skill the worker's assignment demands of them; because the assignment is fixed here, so is the rate, and it can be normalized away.
This can reflect the short-run dynamics of the economy, in which the firm has already determined the set of tasks for a worker, workers have acquired the skills their roles require, and the firm has hired them at a pre-specified wage rate, so that what the firm optimizes is the time production takes.
Section~\ref{sec:longrun} lets the firm decide how to assign steps across multiple workers, at which point the skill an assignment demands, and hence what it pays, becomes an additional margin the firm chooses in order to optimize its organizational structure and lower production costs further.
...
We define each of these objects formally below.
