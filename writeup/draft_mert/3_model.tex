...
Section~\ref{sec:longrun} lets the firm decide how to allocate steps across multiple workers, at which point the skill a job assignment demands, and hence what it pays, becomes endogenous, and the firm lowers production costs by deploying AI in tandem with optimizing workers' job structure.
...
The firm can therefore walk the workflow from left to right, and at each step face only one question, namely whether to perform that step manually or to consider an AI chain that ends there, and in the latter case how far back the chain reaches.
...
We formulate the firm's problem recursively and solve it by dynamic programming: for each $k$ in turn we compute the cheapest way of completing just the first $k$ steps, which is possible precisely because of the additivity noted above.
...
Second, note how little the problem requires: with the boundary of the worker's job, and hence their wage, held fixed, AI acts purely as a productivity-improving tool, lowering the time it takes to execute a fixed set of steps.