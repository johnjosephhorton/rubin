...
Because chaining makes the cost of automating a step depend on the arrangement it sits in, the firm cannot decide step by step and must instead choose among whole arrangements, of which there are exponentially many in $m$.
The problem is nonetheless exactly solvable: the cost-minimizing strategy is the solution to a dynamic program that walks the workflow from left to right, at each step either performing it manually or closing an AI chain that ends there, and optimizing over where that chain began.
Appendix~\ref{app:shortrun_dp} states the result and gives the recursion.
...
The wage bill in \eqref{eq:totalcost_mediumrun} multiplies two sums rather than adding across tasks, so a task's contribution to cost depends on the skill and the time the rest of the job already carries, and the left-to-right argument of the short run no longer goes through.
The problem remains tractable, but only approximately: Appendix~\ref{app:omitted_proofs} gives an algorithm that comes within any desired factor of the optimum.
...
Section~\ref{sec:longrun} takes up this problem, which nests the medium run as the special case of fixed job boundaries, and uses it to close the model.
