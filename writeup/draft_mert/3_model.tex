...
In this section and the next we fix a worker's job boundary, that is, the set of work activities assigned to them, and introduce the production framework and how the firm's AI deployment determines which tasks are left for the worker to perform.
The entire production sequence is allocated to a \emph{single worker} whose wage rate is fixed and normalized to $1$.
Section~\ref{sec:longrun} then lets the firm decide how to assign steps across workers in order to minimize production costs.
The distinction is between a short run, in which the boundary is given and AI can only make the worker faster within it, and a long run, in which the firm redraws the boundary itself; Table~\ref{tab:short_long_run} summarizes the two.
