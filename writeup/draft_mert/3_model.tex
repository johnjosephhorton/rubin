...
Completing a step takes time and demands skill, and both differ across the three modes of execution.
...
Figure~\ref{fig:steps_tasks}, for instance, illustrates an AI strategy in which seven steps constitute five tasks.

In this section and the next we fix a worker's job boundary, that is, the set of steps assigned to them.
It is therefore without loss to assume that the entire production sequence is allocated to a \emph{single worker} whose wage rate is fixed and normalized to $1$.
What the wage rate compensates is the total skill the worker needs to possess in order to perform all activities in their job.
Because the assignment is fixed here, so is the rate, and we normalize it away.
This characterization reflects the short-run dynamics of the economy, in which the firm has already determined the set of tasks for a worker, workers have acquired the skills their roles require, and the firm has hired them at a pre-specified wage rate, so what the firm optimizes is how to deploy AI in order to minimize the time the worker spends to perform their job.
...
Problem \eqref{eq:totalcost_shortterm} is where chaining does its work.
Note that the firm cannot solve this optimization it step by step.
...
Despite this, the firm's problem is exactly solvable, and cheaply.
