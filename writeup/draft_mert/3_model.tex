...
\midrule
\addlinespace
\multicolumn{2}{@{}l}{\textbf{Panel C. Aggregation to the macroeconomy (Section~\ref{sec:extensions.aggregation})}} \\
\addlinespace
...
What makes it tractable is that \eqref{eq:totalcost_shortterm} adds up time costs over tasks, so that once the firm has decided where a task ends, the cost of everything before it is a separate problem.
Section~\ref{sec:extensions.computation} builds a recursion on this observation and shows that the cost-minimizing AI strategy can be computed in time $O(m^2)$ by dynamic programming.

The argument behind it says something about the economics of the problem, not merely its computation.
