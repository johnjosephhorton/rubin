...
Throughout this section and the next we assume that production is carried out by a \emph{single worker}, to whom every step of the workflow is assigned.
The firm has already hired this worker and their wage is set, so we treat it as fixed throughout.
This is the short run of the firm's problem: with the worker in place and their pay settled, the only margin through which AI can affect production is by raising the worker's productivity on the set of tasks allocated to them.
Minimizing production cost accordingly reduces to minimizing the time the workflow takes, and the firm's problem is to deploy AI so as to compress it.
Section~\ref{sec:longrun} relaxes the assumption and studies the firm's long-run problem, in which the firm also chooses how to allocate tasks across workers.
Changing that allocation changes what each worker's assignment demands of them, so wages adjust in step with the organizational structure, and the firm redraws both together to lower its overall wage bill.
Table~\ref{tab:short_long_run} summarizes how the two horizons differ.
...
The single worker of our short-run assumption carries out the entire step sequence $\mathcal{S} = (s_1, \dotsc, s_m)$, which the AI strategy organizes into the task sequence $\T$, and is paid for the whole time it takes them.
...
Labor is the only cost of production, so the firm's cost is the fixed rate its worker is paid times the duration \eqref{eq:job_duration} of the workflow.
Since that rate is owed for as long as the work runs no matter how AI is deployed, the firm minimizes cost by minimizing duration, and chooses its AI deployment strategy $\T$ to solve
\begin{align}
...
It is worth noting how little the problem requires: because the wage is fixed, time savings are the whole of it, and the model to this point needs no notion of worker skill at all.
