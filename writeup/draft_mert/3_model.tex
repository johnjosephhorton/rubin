...
Where the firm deploys AI is what groups contiguous subsequences of steps into \emph{tasks}, as Figure~\ref{fig:steps_tasks} illustrates.
...
Its worker carries out the entire step sequence $\mathcal{S} = (s_1, \dotsc, s_m)$, which the AI strategy organizes into the task sequence $\T = (T_1, \dotsc, T_n)$, and is paid at their fixed rate for the whole time this takes.
Production therefore costs that rate times the total expected time requirement of the workflow's tasks, so the firm's problem is to choose the AI deployment strategy that makes the workflow shortest,
\begin{align}
