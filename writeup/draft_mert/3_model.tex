...
Figure~\ref{fig:steps_tasks} illustrates an example of how AI can be deployed to bundle steps into tasks.
We define each component shown in the figure formally below.

In the short run, the boundaries of jobs, that is, the sets of steps assigned to each worker, are already set, workers have acquired the skills their roles require, and the firm has hired them at wages agreed in advance.
The only available margin to the firm is thus how to deploy AI on the steps each worker already holds to lower the time cost of production per unit of output.
...
Moreover, since AI deployed within one worker's job leaves the cost of every other job untouched, it is without loss to focus on a single worker to whom the entire production sequence is allocated.
In Section~\ref{sec:longrun} we allow the firm to also choose how to divide the steps among workers, so the skill a job demands, and hence the wage it commands, becomes an endogenous object.
%There AI deployment lowers costs not only by shortening the time performing steps take but also by changing the boundaries of jobs.
...
Steps can also be executed in an \emph{automated} mode in which AI performs the step end-to-end with no direct human involvement, and hence at zero direct human cost.\footnote{That execution of automated steps do not incur any cost to the firm is consistent with the AI subscription business model, where the firm pays an upfront fixed cost to an AI provider and uses AI tools at zero marginal cost afterward. The main forces discussed in the model remain at play even if we assume each time the firm deploys AI on a step it has to incur a cost. We deliberately avoid modeling this to prevent complicating the model.}
...
What the firm does in the short run is lowering its expected time of production per unit of output by optimally creating AI chains, and it does so while the content of the worker's job, and hence the rate at which they are paid, are held fixed.
AI is therefore a purely productivity-improving tool here, compressing the time performing a fixed set of steps takes.
Section~\ref{sec:results} works through the implications of chaining, and Section~\ref{sec:extensions.computation} shows that the problem can be stated recursively in the position of the last step of the workflow, and therefore solved exactly by dynamic programming.