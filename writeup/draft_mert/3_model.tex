...
Throughout this section and the next we assume that the entire production sequence is allocated to a \emph{single worker}, and that this worker's wage rate is fixed, which we normalize to $1$.
This is the short run of the firm's problem: the worker has already been hired and their pay settled, so the only margin through which AI can affect production is by raising their productivity on the set of tasks allocated to them.
With the wage rate normalized, the cost of production is simply the time the workflow takes, and the firm's problem is to deploy AI so as to compress it.
...
Its worker carries out the entire step sequence $\mathcal{S} = (s_1, \dotsc, s_m)$, which the AI strategy organizes into the task sequence $\T = (T_1, \dotsc, T_n)$, and is paid at the normalized rate of $1$ for the whole time this takes.
