...
This characterization reflects the short-run dynamics of the economy, in which the firm has already determined the boundaries of workers' jobs, workers have acquired the skills their roles require, and the firm has hired them at pre-specified wage rates, so what the firm optimizes is how to deploy AI in order to minimize the time each worker spends on their job.
Section~\ref{sec:longrun} lets the firm decide how to allocate steps across multiple workers, at which point the skill a job assignment demands, and hence what it pays, becomes endogenous, and the firm lowers production costs by deploying AI in tandem with its organizational structure.
...
The worker carries out the entire step sequence $\mathcal{S} = (s_1, \dotsc, s_m)$, which the firm's AI strategy organizes into the task sequence $\T = (T_1, \dotsc, T_n)$, and is paid at the normalized rate of $w = 1$ for the whole time this takes.
...
Note that the firm cannot solve this optimization it step by step due to chaining.
...
What makes it tractable is that \eqref{eq:totalcost_shortterm} adds up over tasks: once the firm has decided where a task ends, the cost of everything before it is a separate problem.
