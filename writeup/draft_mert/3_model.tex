...
Throughout this section and the next, a single worker executes the whole workflow: the firm hires one worker, and the entire step sequence $\mathcal{S} = (s_1, \dotsc, s_m)$, which the AI strategy organizes into the task sequence $\T$, is theirs to carry out.
Section~\ref{sec:longrun} lets the firm divide the workflow among several workers instead, and it is there that how work is apportioned becomes something the firm chooses.

The worker is paid for the entire time the workflow takes to complete, at a rate the firm cannot change by deploying AI differently, since it was settled when the worker was hired.
The duration of the workflow is the total expected time requirement of its tasks,
\begin{align}
...
Labor is the only cost of production, so the firm's cost is the rate its worker is paid times the duration \eqref{eq:job_duration} of the workflow.
Because that rate is settled and is owed for as long as the work runs no matter how AI is deployed, the firm takes it as given, and cost minimization reduces to compressing the time the workflow takes.
...
The problem above describes how a firm adapts to AI in the short run, when who does what is already settled.
Having already hired a worker to carry out the workflow, the firm cannot reallocate steps across workers or change what that work pays, so its only lever is where in the workflow to deploy AI, and it pulls that lever to compress the time production takes.
...
Section~\ref{sec:longrun} takes up the longer horizon, in which AI changes not only how fast a worker gets through their assigned steps but which steps are assigned to them, and it is there that what that work pays becomes a margin the firm chooses; Table~\ref{tab:short_long_run} summarizes how the horizons differ.
