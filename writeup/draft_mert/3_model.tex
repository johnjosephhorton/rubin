...
In this section and the next we work in the short run.
The boundaries of jobs, that is, the sets of steps assigned to each worker, are already drawn, workers have acquired the skills their roles require, and the firm has hired them at wages agreed in advance.
What remains open is only how AI is deployed on the steps each worker already holds.
...
And since AI deployed within one worker's job leaves the cost of every other job untouched, nothing is lost by studying a \emph{single worker} to whom the entire production sequence is allocated.

Section~\ref{sec:longrun} reopens both.
There the firm also chooses how to divide the steps among workers, so the skill a job demands, and hence the wage it commands, becomes something the firm decides, and AI lowers costs by reshaping jobs as well as by shortening the time they take.
...
Section~\ref{sec:results} works through what its solution implies for how AI is deployed across a workflow, and Section~\ref{sec:extensions.computation} shows how the problem can be solved via dynamic programming.
