...
Completing a step takes worker time and demands skill, and both differ across the three modes of execution.
...
This problem is where chaining shows its bite.
The firm cannot solve it step by step: whether automating a step lowers expected time depends on the arrangement the step sits in, since a step costs nothing directly once it is absorbed into a chain but adds its failure risk to every attempt at that chain, so the firm must choose among whole arrangements rather than making a separate decision at each step.

What the firm is doing is lowering its production costs by grouping consecutive steps into AI chains, and it does so with the content of the worker's job, and hence the rate at which they are paid, held fixed.
AI is therefore a purely productivity-improving tool here, compressing the time a fixed set of steps takes.
Section~\ref{sec:extensions.computation} shows how the problem can be solved via dynamic programming, and Section~\ref{sec:results} works through what its solution implies for how AI is deployed across a workflow.
