...
In this section and the next we fix a worker's job boundary, that is, the set of steps assigned to them, and introduce the production framework and how the firm's AI deployment determines which work units are assigned to humans and to AI.
The entire production sequence is allocated to a \emph{single worker} whose wage rate is fixed and normalized to $1$.
What that rate compensates is the skill the worker's assignment demands of them; because the assignment is fixed here, so is the rate, and it can be normalized away.
This can reflect the short-run dynamics of the economy, in which the firm has already determined the set of tasks for a worker and hired them at a pre-specified wage rate, so that what it optimizes is the time required to complete them.
Section~\ref{sec:longrun} then lets the firm decide how to assign steps across multiple workers, at which point the skill an assignment demands, and hence what it pays, becomes a margin the firm chooses.
