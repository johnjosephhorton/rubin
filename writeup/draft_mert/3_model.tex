...
The firm deploys AI by assigning one of the modes to each step, and that assignment determines the set of \emph{tasks} the human worker actually handles.
Figure~\ref{fig:steps_tasks} illustrates an AI strategy in which seven steps constitute five tasks.

In this section and the next we fix a worker's job boundary, that is, the set of steps assigned to them.
The entire production sequence is thus allocated to a \emph{single worker} whose wage rate is fixed and normalized to $1$.
What that rate compensates is the total skill the worker's assignment demands of them.
Because the assignment is fixed here, so is the rate, and we normalize it away.
This characterization reflects the short-run dynamics of the economy, in which the firm has already determined the set of tasks for a worker, workers have acquired the skills their roles require, and the firm has hired them at a pre-specified hourly wage rate, so what the firm optimizes is how to use AI to minimize the time production takes.
Section~\ref{sec:longrun} lets the firm decide how to assign steps across multiple workers, at which point the skill an assignment demands, and hence what it pays, becomes endogenous, and the firm lowers production costs by deploying AI in tandem with its organizational structure.
We now define each of these objects formally, beginning with steps.
