...
The only available margin to the firm is thus how to deploy AI on the steps each worker already holds.
Two simplifications follow.
A worker's wage compensates the skills their job demands, so with those demands settled the wage is a constant, which we normalize to $1$.
And since AI deployed within one worker's job leaves the cost of every other job untouched, it is without loss to focus on a \emph{single worker} to whom the entire production sequence is allocated.

Section~\ref{sec:longrun} relaxes both.
There the firm also chooses how to divide the steps among workers, so the skill a job demands, and hence the wage it commands, becomes an endogenous object the firm decides on, and AI lowers costs by reshaping jobs as well as by shortening the time they take.

Below we define each of the model's components in the short run problem formally in turn.
...
Section~\ref{sec:results} works through what the solution implies for how AI is deployed across a workflow, and Section~\ref{sec:extensions.computation} shows how the problem can be solved via dynamic programming in polynomial time.