...
\begin{example}
\label{ex:FI.tight}
Consider a three-step workflow with unit skill (recall that $\AIskill{i}\AItime{i} = 1$ for all $i$). The first and last steps always succeed, while the second succeeds with probability $1/2$ and is twice as costly to perform manually, so the steps differ only in
\[
...
Consider a sequence of $m$ identical steps, each with manual cost $(\manualSkill{i},\manualTime{i}) = (1,\sqrt{2})$, AI management cost $(\AIskill{i},\AItime{i}) = (1,1)$, and success probability $q_i = 1/\sqrt{2}$.
...
Consider a sequence that alternates between two step types, with $K > 1$ such pairs (so $2K$ steps in total), where $\epsilon$ is vanishingly small. AI management costs $(\AIskill{i},\AItime{i}) = (1,1)$ at every step, so the two types differ only in
\[
\begin{aligned}
  \text{Odd } i:\quad  && (\manualSkill{i},\manualTime{i}) &= (1,0), &\quad q_i &= 1/\sqrt{2}-\epsilon,\\[2pt]
  \text{Even } i:\quad && (\manualSkill{i},\manualTime{i}) &= (1,1), &\quad q_i &= 1.
\end{aligned}
\]
