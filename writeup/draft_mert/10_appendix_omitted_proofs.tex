...
Sections~\ref{app:shortrun_dp} and~\ref{app:mediumrun_dp} record the sense in which the firm's short- and medium-run cost minimization problems of Section~\ref{sec:model.jobs} can be solved, which the main text states without proof.

\subsection{Short-Run AI Deployment}
\label{app:shortrun_dp}

The short-run problem \eqref{eq:totalcost_shortterm} minimizes a sum over tasks, and this additivity is what makes it exactly solvable despite the exponentially many arrangements available to the firm.
...
\subsection{Medium-Run AI Deployment}
\label{app:mediumrun_dp}

The medium-run objective \eqref{eq:totalcost_mediumrun} is a product of two sums rather than a single sum, so it is not additive across tasks and the short-run recursion no longer applies.
