...
Section~\ref{app:shortrun_proof} proves Proposition~\ref{prop:shortrun_dp} of Section~\ref{sec:extensions.computation}, and Section~\ref{app:mediumrun_dp} treats an intermediate case, in which wages adjust to the skill a job requires but job boundaries are still given, that the long-run proof of Appendix~\ref{app:jobdesign.longrun} builds on.
...
This subsection treats an intermediate case that the proof of Proposition~\ref{prop:totalcost_optimization_dp} builds on: wages adjust to the skill a job requires, but job boundaries are still given.
Holding the wage fixed is what reduces cost minimization to minimizing time in Section~\ref{sec:jobs_wages}.
Once it adjusts, deploying AI pays on both margins at once, since an augmented or automated step may demand less skill as well as less time, and the firm minimizes the wage bill of the job, the product of its total skill and its total time,
\begin{align}
...
The program decides how to execute the job's steps one at a time, tracking the worker's accumulated skill and time along the way and evaluating the wage bill only once every step has been assigned.
Since each execution choice moves both coordinates, the state is two-dimensional and can take exponentially many values, so we discretize both to a geometric grid.
