...
Section~\ref{app:shortrun_dp} characterizes the solution to the firm's AI deployment problem of Section~\ref{sec:model.jobs}, which the main text states without proof.
Section~\ref{app:mediumrun_dp} then extends it to the case in which wages adjust to the skill a job requires.

\subsection{AI Deployment with a Fixed Wage Schedule}
\label{app:shortrun_dp}

The deployment problem \eqref{eq:totalcost_shortterm} minimizes a sum over tasks, and this additivity is what makes it exactly solvable despite the exponentially many arrangements available to the firm.

\begin{proposition}[AI Deployment with a Fixed Wage Schedule]
...
Section~\ref{sec:model.jobs} holds the wage rate fixed, which is what reduces cost minimization to minimizing time.
...
Objective \eqref{eq:totalcost_mediumrun} is a product of two sums rather than a single sum, so it is not additive across tasks and the recursion of Appendix~\ref{app:shortrun_dp} no longer applies: a task's contribution to cost depends on the skill and the time the rest of the job already carries.
