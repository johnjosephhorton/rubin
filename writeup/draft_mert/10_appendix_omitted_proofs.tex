...
Section~\ref{app:shortrun_proof} proves Proposition~\ref{prop:shortrun_dp} of Section~\ref{sec:extensions.computation}; the companion long-run result, Proposition~\ref{prop:totalcost_optimization_dp}, is proved in Appendix~\ref{app:jobdesign.longrun}.
...
\begin{spacing}{0.2}
{\fontsize{9.5pt}{10pt}\selectfont Notes: The two-step workflow of Example~\ref{ex:nonmonotone}, in which $\manualTime{1}=1$, $\manualTime{2}=2$, $\AItime{1}=\AItime{2}=1$, and the AI succeeds at step $i$ with probability $q_i = \alpha^{d_i}$ for $d_1 = 6$ and $d_2 = 1$, so that $q_1 = \alpha^{6}$ and $q_2 = \alpha$.
