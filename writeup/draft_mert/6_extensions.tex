...
We distinguish two types of labor: \emph{manual labor}, which performs manual steps, and \emph{AI management labor}, by which we mean the labor that verifies the AI's output at the end of a chain.\footnote{
...
\end{align}
over economy-wide AI management labor $\aggregateAIlabor$, manual labor $\aggregateManualLabor$, and capital $K$, with $X$ denoting aggregate output.
...
Appendix~\ref{sec:aggregation} obtains that dispersion from the order in which a firm has to commit, settling on its AI strategy and job design before it learns how effectively it can put AI to use, and derives in closed form the distribution of realized AI effectiveness under which \eqref{eq:lr_macro_ces} holds.\footnote{
...
That is, two economies whose production steps are identical, differing only in how those steps are arranged within workflows, reach the same functional form expressed by \eqref{eq:lr_macro_ces} but with different weights and input requirements while looking alike on any step-level measures.
...
This is not a computationally trivial problem, as the firm cannot solve its optimization problem step by step due to chaining. 
...
Let $R(i,\, \skillcost{},\, \timecost{})$ be the minimum cost of completing steps $1$ through $i$ when the worker currently accumulating tasks, the \emph{active worker}, already holds total skill $\skillcost{}$ and total time $\timecost{}$, the latter including their hand-off, and let $V(i) = R(i,\, 0,\, \handofftime{i})$ be the corresponding cost when that worker's job closes at step $i$.
