...
Table~\ref{tab:notation} in Appendix~\ref{app:theory_tables} summarizes the notation used throughout the theory sections.
...
Chaining steps in this way lowers the probability that an attempt succeeds, since the AI must complete every step in the run, but it economizes on human time, since the output is verified only once.
We call such a contiguous run of AI-executed steps an \emph{AI chain}.

Formally, an AI chain spans steps $(s_\ell, \dotsc, s_r)$, where the worker verifies only the output of the final step $s_r$.
One attempt at the chain therefore costs the same as verifying step $s_r$ on its own, namely time \AItime{r}.
Assuming independent failures across steps, an attempt succeeds with probability $\prod_{i=\ell}^{r} q_i$, and the chain is retried until it succeeds, for a total expected time of $\AItime{r}/\prod_{i=\ell}^{r} q_i$.
