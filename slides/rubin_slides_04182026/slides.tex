% PS: the weird shadows on the title page, proposition, and definition environments appear only on Overleaf. Once you download the pdf everything looks normal.
...
\newcommand{\myemail}[0]{nicimm@microsoft.com}
\newcommand{\conftitle}[0]{}
\newcommand{\confdate}[0]{March 17, 2026}
...
John J. Horton (MIT)\\
Nicole Immorlica (Yale \& Microsoft)\\
...
\end{itemize}
\item The paper relates to AI and labor economics literatures.
\end{itemize}
\end{frame}
%
\section{Model}
...
\begin{tikzpicture}[node distance=1.8cm]
    ...
    %node[midway, above] {$\manualTime{} + \handofftime{}$} 
    (task2.west);
...
\begin{enumerate}
\item Skill $\skillcost{i}$ $\rightarrow$ human capital required to perform the
step
\item Time $\timecost{i}$ $\rightarrow$ time it takes to complete the step
\end{enumerate}
\item Production can be viewed as stacked rectangles of height $\skillcost{i}$
and width $\timecost{i}$:
\end{itemize}
\begin{figure}[h!]
 \begin{center}
...
time costs for a manual step are denoted by $(\manualSkill{i},\manualTime{i})$.
\end{defn}
\end{frame}
%
...
augmented step are denoted by $(\AIskill{i},\text{\ensuremath{\tfrac{\AItime{i}}{q_{i}}}})$,
...
$(\AIskill{i},\tfrac{\AItime{i}}{\prod_{r=\ell}^{i}q_{r}})$, where $q_{r}$
...
\end{frame}
%
\begin{frame}{What's Special about AI?}
\begin{itemize}
\item AI is a general-purpose technology: it can impact multiple points in the production chain (albeit disproportionately).
\item AI also enables ``chaining'' of contiguous steps via prompting.
\end{itemize}
\vspace{0.5cm}
...
\begin{figure}
  \begin{tikzpicture}[scale=0.7, transform shape,
    ...
    % Top layer (Steps)
    \node[manual] (S1) {Step 1};
    \node[manual, right=of S1] (S2) {Step 2};
    \node[manual, right=of S2] (S3) {Step 3};
    \node[manual, right=of S3] (S4) {Step 4};
    \node[manual, right=of S4] (S5) {Step 5};
    \node[manual, right=of S5, fill=pink] (S6) {Step 6};
    \node[manual, right=of S6] (S7) {Step 7};
...
\begin{figure}
  \begin{tikzpicture}[scale=0.7, transform shape,
    ...
    % Top layer (Steps)
    \node[manual] (S1) {Step 1};
    \node[manual, right=of S1, fill=yellow!50] (S2) {Step 2};
    \node[manual, right=of S2, fill=yellow!50] (S3) {Step 3};
    \node[manual, right=of S3] (S4) {Step 4};
    \node[manual, right=of S4] (S5) {Step 5};
    \node[manual, right=of S5] (S6) {Step 6};
    \node[manual, right=of S6] (S7) {Step 7};
    ...
    \node[dashedbox, draw=darkyellow!50, fit=(S2)(S3)] (DS2-3) {};
  \end{tikzpicture}
\end{figure}
\end{frame}
%
...
\begin{figure}
  \begin{tikzpicture}[scale=0.7, transform shape,
    ...
    % Top layer (Steps)
    \node[manual] (S1) {Step 1};
    \node[automated, right=of S1] (S2) {Step 2};
    \node[automated, right=of S2] (S3) {Step 3};
    \node[augmented, right=of S3] (S4) {Step 4};
    \node[manual, right=of S4] (S5) {Step 5};
    \node[augmented, right=of S5] (S6) {Step 6};
    \node[manual, right=of S6] (S7) {Step 7};
...
into tasks ($\textit{\textit{AI deployment strategy}}$), and tasks are
...
tasks $\mathcal{T}=(T_{1},\ldots,T_{n})$ where $\timecost{b}=(s_{i},\ldots,s_{i+\ell})$
for some $i$ and $\ell$.
\item Firm also partitions contiguous subsequences of tasks into jobs $\mathcal{J}=(J_{1},\ldots,J_{p}).$
\item We call $\mathcal{T}$ the firm's $\textit{\textit{AI deployment strategy}}$
...
\begin{itemize}
\item $(\manualSkill{i},\manualTime{i})$ if human,
\item $\left(\AIskill{i},\frac{\AItime{i}}{\prod_{r=\ell}^{i}q_{r}}\right)$
if AI chain.
\end{itemize}
\end{itemize}
\vspace{0.5cm}
...
\begin{itemize}
\item $\text{Wage}_{j}=\sum_{\text{task }b\in j}\skillcost{b}$
\item $\text{\ensuremath{\text{Employment Duration}_{j}}}=\sum_{\text{task }b\in j}\timecost{b}.$
\end{itemize}
\item Workers paid wage rates for every unit of time being employed. Thus,
wage bill of job $j$ is:
\[
\text{WageBill}_{j}=\left(\sum_{\text{task }b\in j}\skillcost{b}\right)\left(\sum_{\text{task }b\in j}\timecost{b}\right).
\]
\textcolor{white}{The term $\handofftime{}(j)$ is the hand-off time cost associated
...
\begin{itemize}
\item $\text{Wage}_{j}=\sum_{\text{task }b\in j}\skillcost{b}$
\item $\text{\ensuremath{\text{Employment Duration}_{j}}}={\color{red}\handofftime{}(j}\mathclose{\color{red})}+\sum_{\text{task }b\in j}\timecost{b}.$
\end{itemize}
\item Workers paid wage rates for every unit of time being employed. Thus,
wage bill of job $j$ is:
\[
\text{WageBill}_{j}=\left(\sum_{\text{task }b\in j}\skillcost{b}\right)\left({\color{red}\handofftime{}(j}\mathclose{\color{red})}+\sum_{\text{task }b\in j}\timecost{b}\right).
\]
The term \textcolor{red}{$\handofftime{}(j)$} is the hand-off time cost associated
...
\[
\text{TotalCost}=\sum_{j=1}^{J}\text{WageBill}_{j}=\sum_{j=1}^{J}\left[\left(\sum_{\text{task }b\in j}\skillcost{b}\right)\left(\handofftime{}(j)+\sum_{\text{task }b\in j}\timecost{b}\right)\right].
\]
\end{enumerate}
\end{itemize}
\begin{figure}[htbp]
...
\begin{enumerate}
\item Long-run: job design $\mathcal{J}$ and AI deployment strategy $\mathcal{T}$
can flexibly adjust:
\[
\min_{\mathcal{T}}\ \min_{\mathcal{J}}\ \text{TotalCost}(\mathcal{J};\mathcal{T})=\sum_{J_{j}\in\mathcal{J}}\text{WageBill}_{j}=\sum_{J_{j}\in\mathcal{J}}\Biggl[\Biggl(\sum_{\timecost{b}\in J_{j}}\skillcost{b}\Biggr)\Biggl(\handofftime{}(J_{j})+\sum_{\timecost{b}\in J_{j}}\timecost{b}\Biggr)\Biggr].
\]
\item Short-run: job design $\mathcal{J}$ and worker wages fixed, AI deployment
strategy $\mathcal{T}$ adjusts within each job:
\[
\min_{\mathcal{T}}\;\sum_{\timecost{b}\in\mathcal{T}}\timecost{b}.
\]
\end{enumerate}
\end{itemize}
\end{frame}
...
\begin{frame}{Short-Term Optimization: AI Strategy Design}
\begin{itemize}
...
\item Goal is to find the optimal AI deployment strategy $\mathcal{T}$
...
\end{frame}
%
\begin{frame}{Short-Term Optimization: AI Strategy Design - Continued}
\begin{prop}
Given $m$ steps organized into a single job, the time-optimal AI
deployment strategy can be calculated in time $O(\aggregateAIlabor^{2})$ via dynamic
programming.
\end{prop}
\begin{proof}
...
with $C[k]$. We have:
\[
C[k]=\min\ \Biggl\{ C[k-1]+\manualTime{k},\ \min_{\ell<k}\Biggl\{ C[\ell]+\frac{\timecost{k}^{\AIletter}}{\prod_{i=\ell+1}^{k}q_{i}}\Biggr\}\Biggr\}.
\]
\begin{itemize}
...
$k=1,2,...,m$ yields the value of $C[m]$ in $O(\aggregateAIlabor^{2})$ time.
\end{proof}
\end{frame}
%
...
pair of AI deployment strategy $\mathcal{T}$ and job design $\mathcal{J}$
minimizing the long-term production cost to within a factor of $(1+\epsilon)$
can be computed in time $O(\aggregateAIlabor^{2}\epsilon^{-2}\log^{2}(mB))$ by dynamic
programming.
\end{prop}
\end{frame}
%
...
time cost parameters: $(\manualSkill{},\manualTime{})=(1,\tfrac{3}{2})$ and $(\AIskill{},\AItime{})=(1,1)$,
...
Consider a job consisting of $m(=2k)$ steps. Each step has $\manualTime{}=5$
and $\AItime{}=1$, but the steps vary in how difficult they are for an
...
steps, \textit{even as part of a chain}. The optimal AI deployment
strategy is to employ AI-augmentation on the odd-numbered steps (for
a cost of 1 each) and perform even-numbered steps manually (for a
cost of 5 each), resulting in overall time cost of $3m$.
\end{exampleblock}
\end{frame}
%
\begin{frame}{Fragmentation Index - Motivating Examples}
...
completed with probability 0.1:\begin{figure}
\begin{tikzpicture}[
    scale=0.8,
    node distance=0.5cm,
    every node/.style={
...
\end{tikzpicture}
\end{figure}The optimal strategy chains together the first $\frac{m}{2}$ steps
...
cost of an optimal AI deployment strategy for the job. 
\end{itemize}
\begin{prop}
Fix a single job with a sequence of $m$ steps $\mathcal{S}=\{s_{1},\dotsc,s_{m}\}$,
each with $\AItime{i}=1$. Let $FI$ denote its fragmentation index
and let $OPT$ denote the minimum time cost over all AI deployment
strategies. Then $\tfrac{1}{8}OPT\leq FI\leq\tfrac{5}{4}OPT$. If
we further assume $\manualTime{i}\geq1$ for all $i$, then $\tfrac{1}{4}OPT\leq FI\leq\tfrac{5}{4}OPT$.
\end{prop}
\begin{itemize}
\item Key takeaways: 
\begin{itemize}
...
time and skill requirements $(\manualSkill{},\manualTime{})=(5,5)$. Suppose that,
under AI augmentation, this step has $(\AIskill{},\AItime{})=(1,1)$ and probability
...
\begin{exampleblock}{Example 4}
 Consider a two-step production process, with costs $(\manualSkill{1}, \manualTime{1},\handofftime{1})=(2,4,5)$
and $(\manualSkill{2}, \manualTime{2})=(4,2)$. That is, the first step is low-skill
...
via AI to yield effective skill and time costs of $(\AIskill{},\AItime{})=(2,2)$
...
and time costs $(\manualSkill{1}, \manualTime{1})=(\manualSkill{2}, \manualTime{2})$ and
hand-off cost $\handofftime{1}=5$.
...
Suppose AI augmentation can allow each step to be completed with $(\AIskill{},\AItime{})=(2,1/4)$,
...
$(\manualSkill{1}, \manualTime{1},\handofftime{1})=(5,1,1)$. Task 1's AI difficulty
...
$(\AIskill{1}, \AItime{1})=(5,1)$. The second step is low-skill and timely
to execute manually, but easy for AI: $(\manualSkill{2}, \manualTime{2})=(2,4)$
and $(\AIskill{2}, \AItime{2})=(2,1)$ with AI difficulty score $1$. 
\end{exampleblock}
\begin{figure}[h!]
 \begin{center}
...
\item From
\[
x=\min\left\{ \frac{\labor{1}}{\skillAdjustedTimeAI{1}\,\alpha^{-d_{1}}},\,\cdots,\,\frac{\labor{k}}{\skillAdjustedTimeAI{k}\,\alpha^{-d_{k}}},\,\frac{\labor{k+1}}{\skillAdjustedTimeManual{k+1}},\,\cdots,\,\frac{\labor{n}}{\skillAdjustedTimeManual{n}},\,\frac{\labor{n+1}}{\skillAdjustedTimeHandoff{}(J_{1})},\,\cdots,\,\frac{\labor{n+p-1}}{\skillAdjustedTimeHandoff{}(J_{p-1})}\right\} ,
\]
to
\[
...
\]
where $\skillAdjustedTimeLetter_{\AIletter}$ and $\skillAdjustedTimeLetter_{\manualLetter}$ are firm-level aggregate management
...
\begin{itemize}
\item We empirically test three implications of the model. \pause \label{Data_Sources}
\end{itemize}
\vspace{0.5cm}
\begin{itemize}
\item Construct a task-level dataset containing three attributes for each
task: (1) exposure to AI, (2) realized model of execution, (3) position
in production sequence by combining:
\begin{enumerate}
\item O{*}NET occupations and tasks (May 2023)
\begin{itemize}
\item Every O{*}NET task is mapped to a step in our theoretical framework.
\end{itemize}
\item Human-generated labels for AI exposure of O{*}NET tasks (Eloundou
et al., 2024)
\item AI execution outcomes of O{*}NET tasks from Anthropic's Economic Index
(Handa et al., 2025)
\item GPT-produced sequence of tasks within occupation \pause
\end{enumerate}
\end{itemize}
\vspace{0.5cm}
\begin{itemize}
...
\begin{itemize}
\item We follow Anthropic's approach in labeling augmented and automated
tasks:
\end{itemize}
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.96\linewidth]{plots/anthropic_executionMode_examples.png}
    \end{center}
\end{figure}
\end{frame}
%
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/all_labels_histogram.png}
    \end{center}
\end{figure}
\end{frame}
%
...
\item We calculate the same statistics in two different placebo datasets
constructed via:
\begin{enumerate}
...
\item Takeaways: 
\begin{itemize}
\item Occurrence of AI tasks in clusters in original data not not random,
...
of job-level AI adoption; where AI-able steps sit in the workflow
also matters for execution outcomes. \pause
\item Empirical fragmentation index (EFI) is measured by number of \uline{potential
chains} divided by all steps.
\item Example: consider an occupation with five O{*}NET tasks:
\begin{itemize}
\item If none of 5 are exposed to AI $\rightarrow$ EFI = 5 / 5 = 1
\item If two contiguous tasks are exposed to AI $\rightarrow$ EFI = 4 /
5 = 0.8
\item If two non-contiguous tasks are exposed to AI $\rightarrow$ EFI =
5 / 5 = 1\pause
\end{itemize}
\item In a sense, EFI measures the number of hand-offs between tasks in
the extreme case where all AI-exposed tasks are executed by AI.\pause
\item We calculate two versions of EFI:
\begin{enumerate}
\item Only considering E1 tasks as AI-exposed (14\% of tasks) -- a strict
measure
\item Considering both E1 and E2 tasks as AI-exposed (44\% of tasks)
\end{enumerate}
\end{itemize}
\end{frame}
%
\begin{frame}{Pred. \#2: AI-able Task Fragmentation Matters - Test}
\begin{itemize}
\item Prediction \#2: Step-level AI exposure is not the only determinant
of job-level AI adoption; where AI-able steps sit in the workflow
also matters for execution outcomes.
\item Regression:
\[
\text{ai\_execution}_{o}=\beta_{0}+\beta_{1}\,\text{ai\_exposure}_{o}+\beta_{2}\,\text{empirical\_fragmentation\_index}_{o}+\varepsilon_{o}
\]

\begin{itemize}
\item $\text{ai\_execution}_{o}$: fraction of AI-executed tasks in occupation
$o$
\item $\text{ai\_exposure}_{o}$: fraction of AI-exposed tasks (E1) in occupation
$o$
\item $\text{empirical\_fragmentation\_index}_{o}$: measure of how fragmented
AI-exposed tasks are in occupation $o$
\end{itemize}
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/fragmentation_regression.png}
    \end{center}
\end{figure}

\hyperlink{A5:EFI_execution-based}{\beamergotobutton{Ex post EFI measures}}\label{EFI_exposure-based}
\end{frame}
%
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/fragmentation_regression_explanation1.png}
    \end{center}
\end{figure}\hyperlink{A5:EFI_execution-based}{\beamergotobutton{Ex post EFI measures}}
\end{frame}
%
\begin{frame}[noframenumbering]{Pred. \#2: AI-able Task Fragmentation Matters - Results}
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/fragmentation_regression_explanation2.png}
    \end{center}
\end{figure}\hyperlink{A5:EFI_execution-based}{\beamergotobutton{Ex post EFI measures}}
\end{frame}
%
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/fragmentation_regression_explanation3.png}
    \end{center}
\end{figure}\hyperlink{A5:EFI_execution-based}{\beamergotobutton{Ex post EFI measures}}
\end{frame}
%
\begin{frame}{Pred. \#3: Local Context Matters in Step's AI Execution Mode}
\begin{itemize}
\item Prediction \#3: The same step may be executed differently across occupations
...
\end{frame}
%
\begin{frame}{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Practical
Considerations}
\begin{itemize}
...
\item We treat tasks in the same DWA as the ``same step'' appearing across
different occupations.
\end{itemize}
\end{frame}
%
\begin{frame}{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Example
DWAs}
\begin{itemize}
\item Example DWAs and tasks associated with them:\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/sameDWA_examples.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
%
\begin{frame}{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Regression}
\begin{itemize}
...
\item Regression:
\begin{align*}
\Pr\!\left(\text{is\_ai}_{t}=1|X_{t}\right) & =\Lambda\!\Big(\beta_{0}+\beta_{1}\,\mathds{1}_{\{(t-2)\ \text{Task is AI}\}}+\beta_{2}\,\mathds{1}_{\{(t-1)\ \text{Task is AI}\}}\\
 & \ \ \ \ \ \ \ \ +\beta_{3}\,\mathds{1}_{\{(t+1)\ \text{Task is AI}\}}+\beta_{4}\,\mathds{1}_{\{(t+2)\ \text{Task is AI}\}}+\varepsilon_{t}\Big)
\end{align*}

\begin{itemize}
\item $\Lambda$ is logistic CDF.
\item $\text{is\_ai}_{t}=1$ if task $t$ is AI (augmented or automated).
...
\end{frame}
%
\begin{frame}{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Results}
\begin{itemize}
\item Average marginal effect of immediate neighbors being AI on AI execution
of task positive and robust.
\item Average marginal effect of distant neighbors being AI on AI execution
of task weaker and disappear after occupational family and DWA FEs.
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
%
\begin{frame}[noframenumbering]{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Results}
\begin{itemize}
\item Average marginal effect of immediate neighbors being AI on AI execution
of task positive and robust.
\item Average marginal effect of distant neighbors being AI on AI execution
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression_explanation1.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression_explanation2.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression_explanation3.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression_explanation4.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
%
\begin{frame}{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Robustness
Test}
\begin{itemize}
\item The magnitudes are hard to interpret as there is no benchmark for
these effects.
\item But could these have happened by chance? \pause No!
\begin{itemize}
...
similar mean and shape) \begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/AME_full_is_ai_no_fe_no_dwa.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
Test Results}
\begin{itemize}
\item Observation \#2: effect of immediate neighbors (middle columns) stronger
...
Test Results}
\begin{itemize}
\item Observation \#3: effect of farther neighbors (side columns) weaker
than in randomized-position placebos \begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/AME_full_is_ai_no_fe_no_dwa.png}
    \end{center}
...
Test Results}
\begin{itemize}
\item Observation \#4: effect of farther neighbors disappears after controlling
for occupational families but remains positive for immediate neighbors
(SOC major group FEs)\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/AME_full_is_ai_major_fe_no_dwa.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
\item Observation \#4: effect of farther neighbors disappears after controlling
for occupational families but remains positive for immediate neighbors
(SOC minor group FEs)\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/AME_full_is_ai_minor_fe_no_dwa.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
\item Observation \#4: effect of farther neighbors disappears after controlling
for occupational families but remains positive for immediate neighbors
(DWA FEs)\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/AME_full_is_ai_no_fe_with_dwa.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
\item Observation \#4: effect of farther neighbors disappears after controlling
for occupational families but remains positive for immediate neighbors
(\# of DWA tasks in occupation FEs)\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/AME_full_is_ai_no_fe_no_dwa_withTaskDWACount.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
...
\item Observation \#4: effect of farther neighbors disappears after controlling
for occupational families but remains positive for immediate neighbors
(\# of DWA tasks in occupation + DWA FEs)\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/AME_full_is_ai_no_fe_with_dwa_withTaskDWACount.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
%
\begin{frame}[noframenumbering]{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Further
Robustness Tests}
\begin{itemize}
\item As further robustness: \label{Pred3_robustness}
\begin{itemize}
\item Ask GPT-5-mini to pick the tasks within DWAs that are most similar
in terms of execution method and required skills, and keep only one
task per occupation. \hyperlink{A4:GPT_DWAsimilarTasksPrompt}{\beamergotobutton{GPT Prompt}}
\begin{itemize}
\item Results quantitatively the same. \hyperlink{A6:DWA_regression_ai_GPT}{\beamergotobutton{GPT-filtered Sample}}
\end{itemize}
\item Test contribution of neighbors on likelihood of automation (instead
of AI execution):
\begin{itemize}
\item Results quantitatively attenuated but qualitative in the same direction.
\\  \hyperlink{A7:DWA_regression_automated}{\beamergotobutton{Automation (Main Sample)}}\\ \hyperlink{A8:DWA_regression_automated_GPT}{\beamergotobutton{Automation (GPT-filtered Sample)}}
\end{itemize}
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{prop}
Given a fixed sequence of tasks $\mathcal{T}=(T_{1},\dots\skillcost{}, \timecost{}_{n})$
with skill and time costs $c=(c_{1},\dotsc,c_{n})$ and $t=(t_{1},\dots\skillcost{}, \timecost{}_{n})$,
...
\end{frame}
%
\begin{frame}[noframenumbering]{Long-Term Optimization: AI Strategy and Job Design}
\begin{itemize}
...
\item Let $W(i,\skillcost{}, \timecost{})$ be the minimum cost of completing steps 1 through
$i$, as well as some additional (but unspecified) set of tasks with
cost profile $(\skillcost{}, \timecost{})$ that have already been assigned as a single
...
\end{frame}
%
\begin{frame}[noframenumbering]{Long-Term Optimization: AI Strategy and Job Design - Continued}
\begin{prop}
...
= \min\Biggl\{ 
& \skillcost{}\,\timecost{} \; + \; V(i), \; \; \; \; W(i-1,\; \skillcost{} + \manualSkill{i},\; \timecost{} + \manualTime{i}), \\
& \ \min_{r < i} \ W\left(r,\; \skillcost{} + \AIskill{i},\; \timecost{} + \frac{\AItime{i}}{\prod_{s=r+1}^{i} q_s}\right)
\Biggr\}.
\end{align*}

The cost of the optimal joint AI strategy and job design for a given
...
\begin{itemize}
\item Assume $\AItime{i}=1$ for all steps.
...
\[
w(C_{j})=min\{1,\;\sum_{i\in C_{j}}\manualTime{i}\}.
\]
\item Given a realization of $C$ and $F$, the realized fragmentation is:
\[
\sum_{i\in F}\min\,\{\manualTime{i},\frac{\AItime{i}}{q_{i}}\}+\sum_{C_{j}\in\mathcal{C}}w(C_{j}).
\]
...
\item Different types of labor have different base wage rates: $w_{\AIletter}$
for AI management and $w_{\manualLetter}$ for human labor.
\item Updated job wage equation:
\[
\text{Wage}_{j}=w_{\manualLetter}\left(\sum_{\manualTime{b}\in J}\manualSkill{b}\right)+w_{\AIletter}\left(\sum_{\AItime{b}\in J}\AIskill{b}\right).
\]
\item Skill-adjusted time requirement of task $b$ is:
\[
\tau_{b}=\frac{w_{\manualLetter}\left(\sum_{T_{\ell}^{\manualLetter}\in J(b)}\manualSkill{\ell}\right)+w_{\AIletter}\left(\sum_{T_{\ell}^{\AIletter}\in J(b)}\AIskill{\ell}\right)}{w_{E(b)}}\,\timecost{b},
\]
where $E(b)=\manualLetter$ if $b$ is a manual task and $E(b)=AIletter$ if $b$ is an AI chain.
\item Wage bill paid to task $b$ is thus $w_{\manualLetter}\tau_{b}$ if executed manually
and $w_{\AIletter}\tau_{b}\alpha^{-d_{b}}$ if executed via AI.
\end{itemize}
\end{itemize}
\end{frame}
%
...
\begin{itemize}
\item Fix AI deployment strategy $\mathcal{T}$ and job design $\mathcal{J}$.
...
\[
\skillAdjustedTimeLetter_{\AIletter}=\sum_{b=1}^{k}\tau_{b}^{\AIletter},\qquad\qquad\skillAdjustedTimeLetter_{\manualLetter}=\sum_{j=1}^{p-1}\skillAdjustedTimeHandoff{}(J_{j})+\sum_{b=k+1}^{n}\tau_{b}^{\manualLetter},\qquad\qquad\bar{\alpha}=\frac{\sum_{b=1}^{k}\tau_{b}^{\AIletter}}{\sum_{b=1}^{k}\tau_{b}^{\AIletter}\alpha^{-d_{b}}},
\]
...
\begin{itemize}
\item Stage 1: firms decide on organization structure (i.e., AI deployment
...
\begin{itemize}
\item Normalize the output price $p=1$, so that $w_{\AIletter}$ and $w_{\manualLetter}$ are
...
\item In equilibrium:
\[
x=\frac{\bar{\alpha}\,\labor{\AIletter}}{\skillAdjustedTimeLetter_{\AIletter}}=\frac{\labor{\manualLetter}}{\skillAdjustedTimeLetter_{\manualLetter}}.
\]
Substituting for $\labor{\manualLetter}$ and $x$ in terms of $\skillAdjustedTimeLetter_{\AIletter}$ and $\skillAdjustedTimeLetter_{\manualLetter}$
we obtain:
\[
...
\[
\gamma=\frac{w_{\AIletter}\,\skillAdjustedTimeLetter_{\AIletter}}{1-w_{\manualLetter}\,\skillAdjustedTimeLetter_{\manualLetter}}\leq\bar{\alpha}\leq1.
\]
\item Let $\phi(\bar{\alpha})$ be the dist. of output in terms of effective
AI quality level of firms. By definition:
\[
X=\int_{\gamma}^{1}\,\phi(\bar{\alpha})\,d\bar{\alpha},\qquad M=\int_{\gamma}^{1}\,\skillAdjustedTimeLetter_{\AIletter}\frac{\phi(\bar{\alpha})}{\bar{\alpha}}\,d\bar{\alpha},\qquad H=\int_{\gamma}^{1}\,\skillAdjustedTimeLetter_{\manualLetter}\,\phi(\bar{\alpha})\,d\bar{\alpha}.
\]
\end{itemize}
\end{frame}
%
...
\[
X=\left(\theta_{\AIletter}\,\aggregateAIlabor^{\rho}+\theta_{\manualLetter}\,\aggregateManualLabor^{\rho}+(1-\theta_{\AIletter}-\theta_{\manualLetter})\right)^{\frac{1}{\rho}}.
\]
...
\end{itemize}
\begin{itemize}
\item \textit{...Conjecture...}: For certain parameter values $(\theta_{\AIletter},\theta_{\manualLetter},\rho)$
a first-order stochastic shift in effective AI quality level $\bar{\alpha}$
will increase the ratio of $\tfrac{\AIletter}{\manualLetter}$, implying that (relatively)
more AI management labor will be employed in the aggregate economy.
\item \textit{...Caveat...}: We've fixed AI deployment strategy $\mathcal{T}$
...
\end{frame}
%
\label{A5:EFI_execution-based}
\begin{frame}[noframenumbering]{Pred. \#2: AI-able Task Fragmentation Matters - Ex-post EFI Measures}
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/fragmentation_regression_appendix.png}
    \end{center}
\end{figure}\hyperlink{EFI_exposure-based}{\beamergotobutton{Return}}
\end{frame}
%
...
\begin{figure}[htbp]
    ...
    \end{center}
\end{figure}\hyperlink{Pred3_robustness}{\beamergotobutton{Return}}
\end{frame}
%
\label{A6:DWA_regression_ai_GPT}
\begin{frame}{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Example
DWA Task Selection}
\begin{itemize}
\item Example DWAs and tasks surviving the GPT filtering procedure:\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=\linewidth]{plots/sameDWA_examples_red.png}
    \end{center}
\end{figure}
\end{itemize}
\end{frame}
%
\begin{frame}[noframenumbering]{Pred. \#3: Local Context Matters in Step's AI Execution Mode - GPT
Sample AI Execution}
\begin{itemize}
\item Same results hold if ask GPT to filter the DWA tasks:\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression_GPT.png}
    \end{center}
\end{figure}\hyperlink{Pred3_robustness}{\beamergotobutton{Return}}
\end{itemize}
\end{frame}
%
\label{A7:DWA_regression_automated}
\begin{frame}[noframenumbering]{Pred. \#3: Local Context Matters in Step's AI Execution Mode - Main
Sample AI Automation}
\begin{itemize}
\item Effects attenuated but directionally the same if study AI automation
instead of AI execution:\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression_automation.png}
    \end{center}
\end{figure}\hyperlink{Pred3_robustness}{\beamergotobutton{Return}}
\end{itemize}
\end{frame}
%
\label{A8:DWA_regression_automated_GPT}
\begin{frame}[noframenumbering]{Pred. \#3: Local Context Matters in Step's AI Execution Mode - GPT
Sample AI Automation}
\begin{itemize}
\item Effects attenuated but directionally the same if study AI automation
and use the GPT-filtered sample:\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.6\linewidth]{plots/sameDWA_regression_automation_GPT.png}
    \end{center}
\end{figure}\hyperlink{Pred3_robustness}{\beamergotobutton{Return}}
\end{itemize}
\end{frame}
%
\begin{frame}{Pred. \#2: AI-able Task Fragmentation Matters - Results (EFI Def. 2)}
...
\hline\hline
 & (1) & (2) & (3) \\
\hline
AI Exposure                   & 0.23*** & 0.11*** & 0.10*** \\
                              & (0.02)  & (0.02)  & (0.03) \\
Empirical Fragmentation Index & -0.80***& -0.70***& -0.67*** \\
                              & (0.03)  & (0.03)  & (0.03) \\
\hline
R-squared                     & 0.84    & 0.87   & 0.90 \\
Adj. R-squared                & 0.84    & 0.87   & 0.88 \\
Observations                  & 872     & 872    & 872 \\
SOC Group Fixed Effect        &         & Major  & Minor \\
\hline\hline
\multicolumn{4}{l}{\footnotesize Variables z-scored; clustered SEs in parentheses.} \\
