...
Economic models on the effects of technology have to make simplifying assumptions about what that technology does, economically.
The ``problem'' with generative AI, from an economic modeling standpoint, is that is completely unclear what AI \emph{cannot} do even now, never mind over the next 5 or 10 years. 
It seems prudent to move away from what they cannot do and instead try to identify what is likely to be a constant, no matter how great the capabilities advance.
...
\paragraph{The recursive nature of tasks}
...
All productive processes have a recursive structure. 
Every task can be further divided into smaller sub-tasks.
It is tasks all the way down. 
But we can broken down can also be aggregated up. 

\paragraph{Generative AI = Information goods factory}
The distinctive feature of generative AI is that it works with us to produce information goods. 
...
Generative AI models have shown the ability to create output similar to or better than what even expert humans can produce over a wide class of knowledge-processing tasks.  
...
\paragraph{The logic of comparative advantage and costs of communication tend to create serial dependencies between tasks within jobs.}
...
\paragraph{Entrpreneurial innovation will focusing reducing AI management tasks as well as increasing model abilities}
...
Because AI marginal production costs are \emph{de facto} zero, we will likely see parallelism in task generation. 

\paragraph{AI will create new labor-labor substitution possibilities}
...
\paragraph{More capable models will require more expressive domain-specific languages}
Scratch
...
$q = Pr(a \subset A)$.
If every point in $A$ in reachable with a prompt, the expressiveness of the prompt language has to be as large as the space of answers. 
What is the pre-image of $A$?

\paragraph{Generative AI substitutes ``maker labor'' and complements ``manager labor'' in the short-run.}
In the model, there is just two broad ``kinds'' of labor: 
labor that does tasks, and labor that manages an AI doing that task. 
While we might think there is some correlation between the cost of the machine doing the task and the human doing the task, this seems like an imprudent assumption.
...
All else equal, more capable management increases the number of tasks performed by an AI, holding AI capabilities fixed. 
...
\paragraph{A automation chain is when constituent tasks are chained together without a human requesting an output or evaluating that output interstitially}

\paragraph{Jobs disappear when every constituent task is subsumed by an automation chain.}
...
\paragraph{What we know about generative AI}
...
Imagine a job as a sequence of tasks to be performed, in sequence, $1, 2, \ldots n$.
We might think each task produces some intermediate good that feeds into the next task. 
Now imagine it might be possible to use generative AI to perform a task instead.
...
Suppose it costs $c_h$ for the human to do the focal task.
There is an AI that can do the task instantly at zero cost, but it only succeeds with probability $q$, given optimal effort.
...
The expected cost of using the AI is $(c_p + c_e)/q$ and so the worker will use the AI if 
\begin{align}
    q > \frac{c_m}{c_h}. 
\end{align}

We will use the notation $\human{\cdot}$ to indicate a task done by a human, $\machine{\cdot}$ as done by an AI.
Think curved parenthesis captures the greater flexibility of humans. 

\begin{definition}
 For a given task, $\cost{\machine{1}} < \cost{\human{1}}$ if $c_m / q < c_h$.
\end{definition}

\subsection{Chaining}
AI tasks can be chained together without a human in the loop. 
Within a chunk, a $|$ indicates chained-together tasks. 
Only AI tasks are capable of being chained.
Consider a 5 task process. 
For example, $\machine{1|2}\human{3}\human{4}\machine{5}$ would be a process with 5 tasks, with tasks 1 and 2 done in a chain by a machine.  
Tasks 3 and 4 would be done by humans, while task 5 would be done by an AI. 
Total costs would be $\frac{c_m}{q_1 q_2} + c_{h(3)} + c_{h(4)} + \frac{c_m}{q_5}$.
...
One might think that evaluation itself could be the domain of the AI, but if the model could have improved the chance the output met would be evaluated positively, that process would have already been incorporated into the model. 
...
A prompt ``write me a haiku about economics'' is about the same effort as ``write me an epic poem about economics'' but the $c_h$ differs radically. 
...
\end{lemma}
\begin{proof}
We can see this with a simple counterexample.
...
Let the human performance cost for the second task be $\infty$, and so it has to be automated. 
...
\subsection{Improvements in model capabilities increase viability of productive chains}
...
Assigning a task to someone else requires (a) asking them to do it, at a cost of $c_p$
...
Assume we just have two tasks, $q_1$ and $q_2$.
One possibility is that we put them together; another is that we split them apart and have a human manager at each step.
...
\begin{lemma}
If $q_1 + q_2 > 1$, then $C\{ \machine{1|2} \} < C\{\machine{1}\machine{2}\}$.  
\end{lemma}
...
\begin{lemma}
 If a task be combined with either a more or less reliable chain, if it should be combined, it should be combined with the lower cost chain.
